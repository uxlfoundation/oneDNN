...
bool matmul_amx_blocking_params_macro_t::set_blocking_parameters() {
    ...
    auto calc_vertical = [&](size_t k_blk_v) {
        if (rnd_up(n_per_thread, n_decomposition) * (nthr_n_ - 1) > (size_t)N) {
            ...
            const size_t l2_ways = 16;
            const size_t l2_ways_threshold = size_t(l2_ways * 0.75);
            const size_t l2_sets = 2048;
            const size_t l2_line_size = l2_sets * 64;

            // If D matrix in vertical traversal suffers from set issues in
            // the L2, then go horizontally.
            // First, calculate the stride on the N dim within a page
            size_t cache_stride = N * c_dt_sz % page_size;
            if (cache_stride == 0) { cache_stride = page_size; }
            // Second, calculate the number of ways D will use in each set.
            // If higher than the threshold, useful cache lines in L2 will be
            // evicted.
