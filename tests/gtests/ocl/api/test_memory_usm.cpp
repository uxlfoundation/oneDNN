...
void fill_data(void *usm_ptr, memory::dim n, const engine &eng) {
    auto alloc_kind = xpu::ocl::usm::get_pointer_type(eng.get(), usm_ptr);
    if (alloc_kind == xpu::ocl::usm::kind_t::host
            || alloc_kind == xpu::ocl::usm::kind_t::shared) {
        for (int i = 0; i < n; i++)
        ...
        auto s = stream(eng);
        xpu::ocl::usm::memcpy(
                s.get(), usm_ptr, host_ptr.data(), n * sizeof(float));
        s.wait();
    }
}
...
usm_unique_ptr_t allocate_usm(size_t size, const engine &eng) {
    return usm_unique_ptr_t(xpu::ocl::usm::malloc_shared(eng.get(), size),
            [&](void *ptr) { xpu::ocl::usm::free(eng.get(), ptr); });
}
...
HANDLE_EXCEPTIONS_FOR_TEST(ocl_memory_usm_test_t, DeviceMapUnmap) {
    test_usm_map_unmap(xpu::ocl::usm::malloc_device, xpu::ocl::usm::free);
}

HANDLE_EXCEPTIONS_FOR_TEST(ocl_memory_usm_test_t, SharedMapUnmap) {
    test_usm_map_unmap(xpu::ocl::usm::malloc_shared, xpu::ocl::usm::free);
}
