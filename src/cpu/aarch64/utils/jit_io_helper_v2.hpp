 ...
 * (as part of tail_conf_t and saturation_conf_t respectively), if you need to
 * use tails or saturation then you *must* initialise them first with the
...
public:
    ...
    void init_saturation(data_type_t dt, bool force = false);
    void init_tail(bool force = false);
...
private:
    ...
    // use-cases.
    data_type_t saturation_prepared_dt_ = data_type::undef;
    ...
    bool is_tail_ok(bool is_tail) const;
    bool is_saturation_ok(data_type_t dt) const;
    ...
    // ASIMD needs no further tail preparation. SVE must be manually
    // initialised.
