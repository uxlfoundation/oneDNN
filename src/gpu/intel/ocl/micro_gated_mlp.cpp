...
status_t micro_gated_mlp_t::pd_t::init_microkernels(impl::engine_t *engine) {
    ...
    problem.Ta = problem.Ta_ext
            = gemm::jit::convert_dnnl_to_kernel_type(src_md()->data_type);
    problem.Tb = problem.Tb_ext
            = gemm::jit::convert_dnnl_to_kernel_type(W_gate_md()->data_type);
    ...
    problem_wgu.A.setAlignment(gemmstone::alignmentForLD(oc_sz(*d) * problem.Ta)); // OC lda? for wgu?
    //problem_wgu.A.setAlignment(gemmstone::alignmentForLD(ic_sz(*d) * problem.Ta));
    ...
    reqs_wgu.push_back(gemmstone::StrategyRequirement::UnrollM == config->unroll_m_gwu);
    reqs_wgu.push_back(gemmstone::StrategyRequirement::UnrollN == config->unroll_n_gwu);
...
status_t micro_gated_mlp_t::init(impl::engine_t *engine) {
    ...
    kernel_ctx.define_int("SIZE_MB", mb_sz(*pd()->desc()));
    ...
    kernel_ctx.define_int("WD_ALIGN", gemmstone::alignmentForLD(int(ldwd)));
    kernel_ctx.define_int("A_ALIGN", gemmstone::alignmentForLD(int(lda)));
    ...
    auto *d = pd()->desc();
    //TODO:  mb,oc,ic full
    bool s_full = ((ic_sz(*d) % tile_wgu_m) == 0); //todo: check m?
    bool b_full = (mb_sz(*d) == pd()->b_max()); //todo: exhaust MB??
    //bool b_full = (mb_sz(*d) % tile_ic_m == 0);
    ...
    if (b_full) {
        ...
        kernel_ctx.define_int("REMAINDER_WGU", (ic_sz(*d) % tile_wgu_n) != 0);
    } else if (pd()->arch() >= compute::gpu_arch_t::xe_hpc) {
        auto vbytes = mb_sz(*d) * W_down_mdw.data_type_size();
...
void micro_gated_mlp_t::pd_t::init_scratchpad() {
    auto *d = desc();
    const size_t size = mb_sz(*d) * oc_sz(*d); // TODO: just intermediate mm or reduction instead? much greater memory demands
