...
dnnl_dim_t mb_sz(const gated_mlp_desc_t &d) { return d.src_desc.dims[0]; }
dnnl_dim_t ic_sz(const gated_mlp_desc_t &d) { return d.src_desc.dims[1]; }
dnnl_dim_t oc_sz(const gated_mlp_desc_t &d) { return d.W_gate_desc.dims[1]; }
...
status_t micro_gated_mlp_t::pd_t::init_microkernels(impl::engine_t *engine) {
    ...
    problem.Ta_ext = gemm::jit::convert_dnnl_to_kernel_type(W_gate_md()->data_type);
    problem.Tb_ext = gemm::jit::convert_dnnl_to_kernel_type(src_md()->data_type);
    problem.Ta = problem.Tb = gemmstone::Type::f16;
    problem.Tc = problem.Tc_ext = gemmstone::Type::f32; //TODO: return fp16? or is Tc_ext in SLM
    problem.Ts = problem.Tc;

    auto problem_wgu = problem;
    // Transpose A and B for C^T = B^T * A^T, allowing slm B to be shared for both matrix multiplies
    problem_wgu.A.layout = gemmstone::MatrixLayout::N;
    problem_wgu.B.layout = gemmstone::MatrixLayout::Pr;
    problem_wgu.C.layout = gemmstone::MatrixLayout::T;

    problem_wgu.A.setAlignment(gemmstone::alignmentForLD(oc_sz(*d) * problem.Ta_ext));
    ...
    bool wgu_common_scales = with_quantize_common(d->wts_gate_scales, DNNL_ARG_WTS_GATE);
    bool wgu_common_zp     = with_quantize_common(d->wts_gate_zero_points, DNNL_ARG_WTS_GATE);

    if(with_wts_gate_scales() && !wgu_common_scales) {
        auto scale_dt = wts_gate_scales_dt();
        problem_wgu.Ta_scale = gemm::jit::convert_dnnl_to_kernel_type(scale_dt); //TODO: * whatever is lda
        ...
        problem_wgu.A_scale.layout = gemmstone::MatrixLayout::N; //TODO: T? or N?, data is row major //umar says always n
    }
    if(with_wts_gate_zp()) {
       ...
       problem_wgu.AO.layout = gemmstone::MatrixLayout::N; //N?
    ...
    reqs_wgu.push_back(gemmstone::StrategyRequirement::UnrollM == config->unroll_m_gwu); //affects A
    reqs_wgu.push_back(gemmstone::StrategyRequirement::UnrollN == config->unroll_n_gwu); //affects B
...
status_t micro_gated_mlp_t::init(impl::engine_t *engine) {
    ...
    int wts_gate_scales_mask = (static_cast<int>(pd()->with_wts_gate_scales()) << 1)
        | static_cast<int>(with_quantize_common(d->wts_gate_scales, DNNL_ARG_WTS_GATE));
    int wts_up_scales_mask = (static_cast<int>(pd()->with_wts_up_scales()) << 1)
        | static_cast<int>(with_quantize_common(d->wts_up_scales, DNNL_ARG_WTS_UP));
    int wts_down_scales_mask = (static_cast<int>(pd()->with_wts_down_scales()) << 1)
        | static_cast<int>(with_quantize_common(d->wts_down_scales, DNNL_ARG_WTS_DOWN));
    ...
    int wts_gate_zp_mask = (static_cast<int>(pd()->with_wts_gate_zp()) << 1)
        | static_cast<int>(with_quantize_common(d->wts_gate_zero_points, DNNL_ARG_WTS_GATE));
    int wts_up_zp_mask   = (static_cast<int>(pd()->with_wts_up_zp()) << 1)
        | static_cast<int>(with_quantize_common(d->wts_up_zero_points, DNNL_ARG_WTS_UP));
    int wts_down_zp_mask = (static_cast<int>(pd()->with_wts_down_zp()) << 1)
        | static_cast<int>(with_quantize_common(d->wts_down_zero_points, DNNL_ARG_WTS_DOWN));
    ...
    //TODO:  mb,oc,ic full
    kernel_ctx.define_int("REMAINDER_SRC", mb_sz(*d) % tile_wgu_n); //todo: %n instead??
...
void micro_gated_mlp_t::pd_t::init_scratchpad() {
    auto *d = desc();
    const size_t size = mb_sz(*d) * oc_sz(*d);
...
status_t micro_gated_mlp_t::execute(const exec_ctx_t &ctx) const {
    ...
    const dim_t MB = mb_sz(*pd()->desc());
    const dim_t IC = ic_sz(*pd()->desc());
    const dim_t OC = oc_sz(*pd()->desc());
    ...
    //gemm_args[DNNL_ARG_A] = tmp_reduce.get();
    gemm_args[DNNL_ARG_A] = ctx.args().at(DNNL_ARG_SRC);
    //gemm_args[DNNL_ARG_A] = ctx.args().at(DNNL_ARG_WTS_SRC);
    gemm_args[DNNL_ARG_B] = ctx.args().at(DNNL_ARG_WTS_GATE);
    gemm_args[DNNL_ARG_C] = ctx.args().at(DNNL_ARG_DST);
