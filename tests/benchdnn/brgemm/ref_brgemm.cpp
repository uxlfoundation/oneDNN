...
void compute_ref_brgemm(const prb_t *prb, const args_t &args) {
    ...
    const auto acc_m = args.find(DNNL_ARG_DST_1).get_host_f32_handle();
    ...
    dnn_mem_t dst_tmp_mem(dst_m, dnnl_f32, tag::abx, dst_m.engine());
    ...
    if (!generate_skip_accumulation) {
        benchdnn_parallel_nd(M, N, [&](int64_t m, int64_t n) {
            ...
            float &dst = dst_tmp[dst_off_f(prb, m, n)];
            float acc = acc_m[dst_off_f(prb, m, n)];
    ...
    benchdnn_parallel_nd(M, N, [&](int64_t m, int64_t n) {
        ...
        float wei_scale = 1.f;
        if (has_wei_scale) wei_scale = wei_scales[wei_scale_mask > 0 ? n : 0];
        float tmp = dst_tmp[dst_off] * src_scale * wei_scale;
