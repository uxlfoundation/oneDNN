...
stream_impl_t::stream_impl_t(
    ...
    : impl::stream_impl_t(flags), context_(context), allocated_(true) {
...
stream_impl_t::~stream_impl_t() {
    for (const auto &e : events_) {
        func_zeEventHostSynchronize(e.second, UINT64_MAX);
        func_zeEventDestroy(e.second);
        func_zeEventPoolDestroy(e.first);
    }
    if (allocated_) {
        wait();
...
ze_event_handle_t stream_impl_t::create_out_event() {
    ...
    func_zeEventCreate(event_pool, &event_desc, &event);

    events_.push_back({event_pool, event});
...
status_t stream_impl_t::copy(const impl::memory_storage_t &src,
    ...
    ze_event_handle_t out_event = create_out_event();
