...
Information about primitive cache hits and misses can be used for debug purposes. That information is part of the verbose output when any of ``profile_create``, ``profile``, or ``all`` values are used (:ref:`Verbose Mode <doxid-dev_guide_verbose>`).
