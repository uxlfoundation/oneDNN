Quantized MatMul Fusions {#dev_guide_graph_quantized_matmul_fusions}
===========================================================

## Overview

oneDNN supports various quantized MatMul fusion patterns to optimize
performance and reduce memory bandwidth requirements. This document describes
the supported fusion patterns for quantized MatMul.

## MatMul patterns

oneDNN supports quantized MatMul and its optimization through Graph API [1]
by defining the graph, getting partition from the graph, and optimizing the kernels
underneath. In general, a quantized MatMul pattern is defined as a directional
acyclic graph (DAG) using oneDNN Graph API.

### Quantized MatMul patterns

oneDNN defines quantized MatMul patterns as follows.
The blue parts are required when defining a quantized MatMul pattern while the
...
2. The MatMul performs MatMul between src and weights tensor.
...
3. The post-op subgraph is optional and can be constructed with the following operations:
   ...
   2. [Select](@ref dev_guide_op_select) must precede binary/unary operations
      and can only appear once.
   3.  [Reshape](@ref dev_guide_op_reshape), if present, must precede or follow
       [Transpose](@ref dev_guide_op_transpose).
   4.  [Reorder](@ref dev_guide_op_reorder), if present, must follow
       [Transpose](@ref dev_guide_op_transpose).
...
4. The data type conversion subgraph #2 is optional and is used to convert output
   ...
   ![dtype_conversion_subgraph_2_4](images/dtype_conversion_subgraph_2_4.png) ![dtype_conversion_subgraph_2_5](images/dtype_conversion_subgraph_2_5.png)
