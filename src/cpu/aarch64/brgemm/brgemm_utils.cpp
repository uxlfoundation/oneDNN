...
status_t init_brgemm_conf(brgemm_desc_t *brg, cpu_isa_t isa,
    ...
    brg->is_sme = brg->is_sme_feat && brg->is_f32 && (data_type::f32 == dt_b)
            && (data_type::f32 == dt_a)
            && ((0.f == brg->beta) || (1.f == brg->beta)) && (1.f == brg->alpha)
            && (brgemm_addr == brg->type) && (brgemm_row_major == layout);
