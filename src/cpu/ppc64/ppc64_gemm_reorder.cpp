...
void kernel(InputType *inp, OutputType *out, int N, const float Scale,
    ...
    while (N >= 16) {
        ...
        FloatVector0 = vec_mul(FloatVector0, ScaleVector);
        FloatVector1 = vec_mul(FloatVector1, ScaleVector);
        FloatVector2 = vec_mul(FloatVector2, ScaleVector);
        FloatVector3 = vec_mul(FloatVector3, ScaleVector);
    ...
    while (N >= 4) {
        ...
        FloatVector = vec_mul(FloatVector, ScaleVector);
...
status_t ppc64_matrixA_reorder_t::execute_body(const exec_ctx_t &ctx) const {
    ...
    const auto out_strides = output_d.blocking_desc().strides;

    const auto out_dims = output_d.dims();
    const auto M = dims[0];
    const auto K = dims[1];

    const auto lda = K;
    const auto L1_cache_size = platform::get_per_core_cache_size(1);
    dim_t element_size = sizeof(float); // Size of each element
    dim_t max_elements_per_block = L1_cache_size / element_size;
