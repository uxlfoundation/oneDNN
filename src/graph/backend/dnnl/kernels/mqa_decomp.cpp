...
status_t mqa_decomp_kernel_t<quantized, dt>::execute_impl(
    ...
    const auto loop = [=](int tid, int nthr, dim_t bo, dim_t bi) {
        ...
        dnnl_primitive_execute_without_tp_hook(mqa_cfg_.sub_mm1_prim, strm, res->sub_mm1_args[tid]);

        dnnl_primitive_execute_without_tp_hook(mqa_cfg_.sub_softmax_prim, strm, res->sub_softmax_args[tid]);

        mqa_cfg_.sub_reorder2.execute(strm, res->sub_reorder2_args[tid]);

        dnnl_primitive_execute_without_tp_hook(mqa_cfg_.sub_mm2_prim, strm, res->sub_mm2_args[tid]);
