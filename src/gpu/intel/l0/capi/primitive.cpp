...
dnnl_status_t dnnl_l0_interop_primitive_execute(
    ...
    auto *l0_stream_impl
            = utils::downcast<gpu::intel::l0::stream_impl_t *>(stream->impl());
    stream->before_exec_hook();

    if (deps != nullptr) {
        std::vector<gpu::intel::l0::event_wrapper_t> events;
        for (size_t i = 0; i < ndeps; i++) {
            events.emplace_back(gpu::intel::l0::event_wrapper_t(deps[i]));
        }
    ...
    if (return_event != nullptr) {
        if (l0_stream_impl->flags() & stream_flags::in_order) {
            *return_event = nullptr;
        } else {
            *return_event = l0_stream_impl->get_output_event().get();
        }
    }
