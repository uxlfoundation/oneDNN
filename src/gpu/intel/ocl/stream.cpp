...
status_t stream_t::get_async_exec_times(
        double *start_ms, double *end_ms, double *duration_ms) {

    if (!async_tracked_event_) {
        ...
        *duration_ms = get_msec() - *start_ms;
        ...
        // starting time is recorded depending on whether profiler
        // is enabled
        if (async_timing_data_->profiler_enabled) {
            *start_ms = async_timing_data_->start_ms;
        } else {
            async_timing_data_->start_ms = *start_ms;
        }

        *end_ms = async_timing_data_->end_ms;
        *duration_ms = async_timing_data_->end_ms - *start_ms;
