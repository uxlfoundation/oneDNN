...
cell_execution_sig((simple_common_t<aprop>::cell_execution_gru)) {
    ...
    dim_t cell_wei_iter_offset2 = 2 * offsets.weights_iter[3]
            * types::data_type_size(conf.wei_iter_type);
    dim_t cell_scratch_offset2 = 2 * conf.dhc * conf.scratch_gates_elsz;
    ...
    if (aprop == prop_kind::forward) {
        ...
        dim_t cell_scratch_diff_off2
                = 2 * conf.dhc * conf.scratch_diff_gates_elsz;
        sub_buffer_t scratch_diff_ht = *scratch.diff_ht();

        // 1. calculate dG2, dG1, and part of dht-1
        CHECK((this->*elemwise_gru)(ctx, dir, lay, iter, conf.dhc, conf.mb,
...
template cell_execution_sig(simple_fwd_t::cell_execution_gru);
template cell_execution_sig(simple_bwd_t::cell_execution_gru);
