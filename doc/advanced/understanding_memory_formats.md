...
@anchor dev_guide_mem_formats_grouped
...
**Memory Layout:** Grouped format uses two buffers:
...
**Offsets:** Each element in offset buffer specifies the cumulative count, so that
we're able to reference the correct memory location for the start of the group.
For groups with sizes `[M_0, M_1, M_2, ..., M_{n-1}]`, the offsets array is
`[M_0, M_0 + M_1, M_0 + M_1 + M_2, ..., sum(M)]` with length equal to `num_groups`.
Empty groups (size `0`) are valid and common in MoE when no tokens were routed to an expert,
in this case the corresponding offsets (start and end of the group) will be equal.

**Creating Grouped Memory:**
...
- Physical storage: concatenated data for all groups in buffer 0

~~~cpp
const int num_groups = 8;
const int total_M = 5000;  // Sum of all group sizes
const int K = 512;

// Create grouped memory descriptor
// Dimension 0 (total_M) varies per group
...
// Example: offsets = {800, 1400, 2100, 2600, 3250, 3700, 4250, 5000}
...
@note The variable dimension in the descriptor represents the **total size** summed across
all groups. Individual group sizes are encoded in the offsets buffer and resolved
at execution time.
