...
// that is nearly trivial. Each instruction flows to the next, with three
// exceptions:
...
// not. There are no basic blocks. Every instruction is a node.
//
// Backward Liveness Analysis
//
// Liveness is computed backwards and iterated. Needed here depends on uses
// later, so we go through the list back to front. Each instruction keeps alive
...
// counter values. We re-iterate over the instruction list until a pass changes
...
// Compute liveness for each instruction `i`.
//
// A value is `live` at instruction `i` if some future instruction may still
// read it before it is overwritten. In other words, the value must be kept
// available because it might be needed later.
//
// Two values that are live at the same instruction cannot share a register.
//
// Backward data-flow to a fixed point. For each instruction `i`:
//
//   1. Computing `live_in` at instruction `i`:
...
//
//   2. Computing `live_out` at instruction `i`:
...
// changes (the fixed point). Successors are `i+1` for a plain instruction, the
...
void compute_liveness(
    ...
    const int n_instrs = ir.n_instrs();
    const int n_vregs = ir.n_vregs();

    // Phase 1: build per-instruction def/use sets and the control-flow graph.
    //
    // def_at[i][v] is 1 if instruction `i` defines (writes) vreg `v`.
    // use_at[i][v] is 1 if instruction `i` uses (reads) vreg `v`.
    // successors[i] lists all instructions that may execute immediately after i.
    std::vector<std::vector<int8_t>> def_at(
            n_instrs, std::vector<int8_t>(n_vregs, 0));
    std::vector<std::vector<int8_t>> use_at(
            n_instrs, std::vector<int8_t>(n_vregs, 0));

    std::vector<std::vector<int>> successors(n_instrs);
    std::vector<int> def_vregs, use_vregs;

    // Map label IDs to instruction indices so jmp/jz can resolve their target
    // locations when building the control-flow graph.
    std::vector<int> label_to_instr(ir.n_labels, -1);
    for (int i = 0; i < n_instrs; i++)
        if (ir.instrs[i].op == op_kind_t::label)
            label_to_instr[(int)ir.instrs[i].imm] = i;

    for (int i = 0; i < n_instrs; i++) {
        const instr_t &instr = ir.instrs[i];
        ir.def_use(instr, def_vregs, use_vregs);
        ...
        if (instr.op == op_kind_t::loop_end) {
            if (i + 1 < n_instrs) successors[i].push_back(i + 1);
            successors[i].push_back(instr.match + 1);
        } else if (instr.op == op_kind_t::jmp) {
            successors[i].push_back(label_to_instr[(int)instr.imm]);
        } else if (instr.op == op_kind_t::jz) {
            if (i + 1 < n_instrs) successors[i].push_back(i + 1);
            successors[i].push_back(label_to_instr[(int)instr.imm]);
        } else if (i + 1 < n_instrs) {
    ...
    // pass).
    live_in.assign(n_instrs, std::vector<int8_t>(n_vregs, 0));
...
void alloc_file(const ir_t &ir, int file_idx,
    ...
    for (int v = 0; v < n_vregs; v++) {
        if (kind_to_file[(int)ir.vreg_info[v].kind] == file_idx && end[v] >= 0)
...
//
// The pipeline:
//   1. Compute liveness (live_in for each instruction).
//   2. Convert liveness into a single interval per virtual register:
//        start[v] = first instruction where `v` is defined, used, or live_in
//        end[v]   = last such instruction
...
reg_alloc_result_t allocate_registers(
    ...
    // extend_interval(v) grows v's interval to cover instruction i. A vreg
    ...
    // live.
    //
    // extend_interval(v) expands the interval to include instruction `i`
    ...
    for (int i = 0; i < n_instrs; i++) {
        ...
        ir.def_use(ir.instrs[i], def_vregs, use_vregs);
        for (int v : def_vregs)
            extend_interval(v);
        for (int v : use_vregs)
            extend_interval(v);
