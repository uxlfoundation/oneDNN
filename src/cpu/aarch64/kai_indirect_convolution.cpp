...
status_t kai_indirect_convolution_fwd_t::setup_kernel_arrays(
    ...
    // KAI associates each spatial weight position with the input offset at
    // the same position. Reverse those offsets for a deconvolution expressed
    // as a forward convolution. Negative dilation plus the shifted origin is
    // equivalent to indexing (K - 1 - k) for every spatial dimension.
