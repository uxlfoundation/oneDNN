...
private:
    void apply_postops(
            const Xbyak::Reg64 &reg_dst, const dim_t idx, const dim_t offset);
    void generate() override;
    void append_zp_src_comp(dim_t offset, dim_t idx, bool apply_mask);
    void load_as_f32(const Xbyak::Zmm &dst, const Xbyak::Opmask &mask,
            const Xbyak::Address &src_addr, const data_type_t &src_dt);

    dim_t vreg_dst_idx(const dim_t idx) const noexcept;
    Xbyak::Zmm get_vreg_dst(dim_t idx) const;
    Xbyak::Zmm get_vreg_bias(dim_t idx) const;
    Xbyak::Zmm get_vreg_prev_dst(dim_t idx) const;
    Xbyak::Zmm get_vreg_zp_comp_src(dim_t idx) const;
    Xbyak::Zmm get_masked_vreg_dst(dim_t idx, bool apply_mask) const;
    ...
    dim_t number_of_reserved_zmm_regs_;
    ...
    static constexpr dim_t def_unroll_ = 4;
    dim_t zmm_step_;
    const dim_t bias_step_factor_;
    const dim_t sum_step_factor_;
    const dim_t max_unroll_;
...
Xbyak::Zmm jit_pp_ker_t::reserve_zmm() {
    return Xbyak::Zmm(
            static_cast<int>(number_of_reserved_zmm_regs_++));
}

dim_t jit_pp_ker_t::vreg_dst_idx(const dim_t idx) const noexcept {
    return (number_of_reserved_zmm_regs_ + idx * zmm_step_);
}

Xbyak::Zmm jit_pp_ker_t::get_vreg_dst(dim_t idx) const {
    return Xbyak::Zmm(static_cast<int>(vreg_dst_idx(idx)));
}

Xbyak::Zmm jit_pp_ker_t::get_vreg_bias(dim_t idx) const {
    return Xbyak::Zmm(
            static_cast<int>(vreg_dst_idx(idx) + bias_step_factor_));
}

Xbyak::Zmm jit_pp_ker_t::get_vreg_prev_dst(dim_t idx) const {
    return Xbyak::Zmm(
            static_cast<int>(vreg_dst_idx(idx) + sum_step_factor_));
}

Xbyak::Zmm jit_pp_ker_t::get_masked_vreg_dst(
        dim_t idx, bool apply_mask) const {
...
void jit_pp_ker_t::append_zp_src_comp(
        dim_t offset, dim_t idx, bool apply_mask) {
...
#undef PARAM_OFF
    ...
    const auto compute = [&](dim_t offset, dim_t idx, bool apply_mask) {
    ...
    Label main_loop_end;
    {
        ...
        L(main_loop);
        {
            ...
            if (vlen_tail) {
                const dim_t tail_mask = (dim_t(1) << vlen_tail) - 1;
