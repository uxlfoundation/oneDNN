...
Creates a new partition with a given operator and engine kind. The API is used to create a partition from an operation directly without creating the graph and calling ``get_partitions()``. The output partition contains only one operation.
...
Returns the unique ID of the partition. Partition ID is generated by the library internally. The ID can be used for debugging purpose or verbose.
