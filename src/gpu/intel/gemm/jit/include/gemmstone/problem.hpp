...
// @@@@@ used ; or maybe bool has_ao_hostscalar()
    ...
    bool earlyDequantizeA() const {
        ...
        VDEBUGINFO(4, primitive, quantization, "earlyDequantizeA(): aScale2D() = %d Ta_scale.isSubsetOf(Ta) || Ta.isFP() = %d", aScale2D(), Ta_scale.isSubsetOf(Ta) || Ta.isFP());

        bool res = false;

        bool newquant =  true;  //gpu_utils::dev_getenv("NEW", true);
        VDEBUGINFO(4, primitive, quantization, "earlyDequantizeA(): newquant = %d", newquant);
        if (newquant) {
            res = ((aOffset == ABOffset::Calc && earlyDequantizableOffset(Ta_ext, Tao, Ta))
            || (aScale2D() && (Ta_scale.isSubsetOf(Ta) || Ta.isFP()))) && !aOffsetHostScalar();

        } else {
            res = (aOffset == ABOffset::Calc && earlyDequantizableOffset(Ta_ext, Tao, Ta)) || (aScale2D() && (Ta_scale.isSubsetOf(Ta) || Ta.isFP()));
        }
    ...
    bool earlyDequantizeB() const {
        return (bOffset == ABOffset::Calc && earlyDequantizableOffset(Tb_ext, Tbo, Tb)) || (bScale2D() && (Tb_scale.isSubsetOf(Tb) || Tb.isFP()));
    }
#else
    bool earlyDequantizeB() const {
        bool res = ((bOffset == ABOffset::Calc && earlyDequantizableOffset(Tb_ext, Tbo, Tb)) || (bScale2D() && (Tb_scale.isSubsetOf(Tb) || Tb.isFP())))
        && !bOffsetHostScalar();
        VDEBUGINFO(4, primitive, quantization, "earlyDequantizeB() returned %d", res);
        return res;
    }
