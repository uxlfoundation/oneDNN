...
struct micro_bwd_t : public primitive_t {
    ...
    struct pd_t : public sdpa_bwd_pd_t {
        ...
        status_t init(impl::engine_t *engine) {
            ...
            for (int i = 0; i < desc()->qry_md()->ndims; i++)
                VCHECK_SDPA_COND(desc()->diff_qry_md()->dims[i]
                                == desc()->qry_md()->dims[i],
                        "diff_qry dim[%d](%ld) must match qry dim[%d](%ld)", i,
                        (long)desc()->diff_qry_md()->dims[i], i,
                        (long)desc()->qry_md()->dims[i]);
            for (int i = 0; i < desc()->key_md()->ndims; i++)
                VCHECK_SDPA_COND(desc()->diff_key_md()->dims[i]
                                == desc()->key_md()->dims[i],
                        "diff_key dim[%d](%ld) must match key dim[%d](%ld)", i,
                        (long)desc()->diff_key_md()->dims[i], i,
                        (long)desc()->key_md()->dims[i]);
            for (int i = 0; i < desc()->val_md()->ndims; i++)
                VCHECK_SDPA_COND(desc()->diff_val_md()->dims[i]
                                == desc()->val_md()->dims[i],
                        "diff_val dim[%d](%ld) must match val dim[%d](%ld)", i,
                        (long)desc()->diff_val_md()->dims[i], i,
                        (long)desc()->val_md()->dims[i]);
            // dO.dims() == O.dims()
