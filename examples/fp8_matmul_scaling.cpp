...
/// @example fp8_matmul_scaling.cpp
/// > Annotated version: @ref fp8_matmul_scaling_cpp
///
/// @page fp8_matmul_scaling_cpp Matrix Multiplication with FP8 Quantization
...
/// - Matrix multiplication with FP8 inputs and FP32 output.
...
///
/// @include fp8_matmul_scaling.cpp
...
/// @param fp8_val Input FP8 value
/// @returns Approximate float value
float decode_f8_e4m3(uint8_t fp8_val) {
    if (fp8_val == 0) return 0.0f;

    // Extract bit components: F8_E4M3 format is S EEEE MMM (bit 7 to 0)
    const uint8_t sign = (fp8_val >> 7) & 0x1; // Bit 7: sign
    const uint8_t exp = (fp8_val >> 3) & 0xF; // Bits 6-3: 4-bit exponent
    const uint8_t mant = fp8_val & 0x7; // Bits 2-0: 3-bit mantissa
...
/// @param fp8_val Input FP8 value
/// @returns Approximate float value
float decode_f8_e5m2(uint8_t fp8_val) {
    if (fp8_val == 0) return 0.0f;

    // Extract bit components: F8_E5M2 format is S EEEEE MM (bit 7 to 0)
    const uint8_t sign = (fp8_val >> 7) & 0x1; // Bit 7: sign
    const uint8_t exp = (fp8_val >> 2) & 0x1F; // Bits 6-2: 5-bit exponent
    const uint8_t mant = fp8_val & 0x3; // Bits 1-0: 2-bit mantissa
...
/// @param dt Data type
/// @returns Name string
std::string get_fp8_type_name(memory::data_type dt) {
    switch (dt) {
...
/// Helper function to get FP8 maximum value.
///
/// @note Values are based on the OCP FP8 specification and hardcoded here
///       for simplicity.
/// @param dt Data type
...
float return_max_value(memory::data_type dt) {
    switch (dt) {
        case memory::data_type::f8_e5m2:
            ...
            // Per OCP FP8 spec: infinity = 11111.00, NaN = 11111.{01, 10, 11}
        ...
        case memory::data_type::f8_e4m3:
            ...
            // Per OCP FP8 spec: no infinity, NaN = 1111.111
...
/// Computes scaling factors for F32 to FP8 quantization.
///
...
float compute_naive_quantization(const float *data, size_t size,
    ...
    // Get theoretical maximum value for the target FP8 format
    float fp8_max = return_max_value(dst_type);

    // Only apply scaling if values exceed the FP8 range
    float scale;
    if (max_abs <= fp8_max) {
        scale = 1.0f;
        std::cout << "  " << label << " fits in " << get_fp8_type_name(dst_type)
                  << " (max=" << max_abs << ", fp8_max=" << fp8_max << ")"
                  << std::endl;
    } else {
        scale = max_abs / fp8_max;
        std::cout << "  " << label << " max (" << max_abs << ") > "
                  << get_fp8_type_name(dst_type) << " max (" << fp8_max
...
/// FP8 Matmul with Quantization Flow Example.
///
/// @brief Demonstrates matrix multiplication using FP8 data types with scaling.
/// @param engine_kind Execution engine kind (CPU or GPU)
/// @param fp8_type FP8 data type (f8_e5m2 or f8_e4m3)
void perform_fp8_matmul_with_quantization(engine::kind engine_kind,
        memory::data_type fp8_type = memory::data_type::f8_e5m2) {
    if (fp8_type != memory::data_type::f8_e5m2
            && fp8_type != memory::data_type::f8_e4m3) {
        throw std::invalid_argument("Unsupported data type");
    }
    ...
    std::cout << get_fp8_type_name(fp8_type)
    ...
    // Create FP8 memory descriptors for quantized data
    auto src_fp8_md = memory::desc({M, K}, fp8_type, memory::format_tag::ab);
    auto weights_fp8_md
            = memory::desc({K, N}, fp8_type, memory::format_tag::ab);

    auto src_fp8_mem = memory(src_fp8_md, eng);
    auto weights_fp8_mem = memory(weights_fp8_md, eng);

    // Step 1: Compute scaling factors for quantization
    std::cout << "\nStep 1: Computing scaling factors for F32 to "
              << get_fp8_type_name(fp8_type) << " quantization" << std::endl;

    float src_scale = compute_naive_quantization(
            src_f32.data(), src_f32.size(), fp8_type, "Source");
    float weights_scale = compute_naive_quantization(
            weights_f32.data(), weights_f32.size(), fp8_type, "Weights");

    // Step 2: Quantize F32 to FP8 format with scaling
    std::cout << "\nStep 2: Quantizing F32 data to "
              << get_fp8_type_name(fp8_type) << " format with scaling"
              << std::endl;
    ...
    // Check if FP8 reorders are supported on this platform
    try {
        reorder::primitive_desc(eng, src_md, eng, src_fp8_md, src_attr);
        reorder::primitive_desc(
                eng, weights_md, eng, weights_fp8_md, weights_attr);
    } catch (error &e) {
        if (e.status == dnnl_unimplemented)
            throw example_allows_unimplemented {
                    "No FP8 reorder implementation is available for this "
                    "platform.\n"
    ...
    reorder_src.execute(s,
            {{DNNL_ARG_SRC, src_mem}, {DNNL_ARG_DST, src_fp8_mem},
                    {DNNL_ARG_ATTR_SCALES | DNNL_ARG_DST, src_scale_mem}});
    reorder_weights.execute(s,
            {{DNNL_ARG_SRC, weights_mem}, {DNNL_ARG_DST, weights_fp8_mem},
    ...
    // Read a few FP8 values to demonstrate quantization
    std::vector<uint8_t> weights_fp8_data(K * N);
    read_from_dnnl_memory(weights_fp8_data.data(), weights_fp8_mem);

    auto decode_fp8 = (fp8_type == memory::data_type::f8_e4m3) ? decode_f8_e4m3
                                                               : decode_f8_e5m2;
    std::cout << "    Sample: F32=" << weights_f32[0]
              << " -> FP8=" << (int)weights_fp8_data[0]
              << " -> decoded=" << decode_fp8(weights_fp8_data[0])
              << " (FP8 as float)"
              << " -> final=" << decode_fp8(weights_fp8_data[0]) * weights_scale
    ...
    // Step 3: Matrix multiplication with FP8
    std::cout << "\nStep 3: Performing matrix multiplication with "
              << get_fp8_type_name(fp8_type) << " inputs" << std::endl;
    ...
    // Check if FP8 matmul is supported on this platform
    try {
        matmul::primitive_desc(
                eng, src_fp8_md, weights_fp8_md, dst_md, matmul_attr);
    } catch (error &e) {
        if (e.status == dnnl_unimplemented)
            throw example_allows_unimplemented {
                    "No FP8 matmul implementation is available for this "
                    "platform.\n"
                    "Please refer to the developer guide for details."};
    ...
    matmul_prim.execute(s,
            {{DNNL_ARG_SRC, src_fp8_mem}, {DNNL_ARG_WEIGHTS, weights_fp8_mem},
    ...
    // Calculate relative error between FP8 and F32 results
...
void run_fp8_tutorials(engine::kind engine_kind) {
    // Sample 1: F8_E5M2
    std::cout << "Sample 1: F8_E5M2 Format" << std::endl;
    std::cout << "==========================" << std::endl;
    perform_fp8_matmul_with_quantization(
...
int main(int argc, char **argv) {
    ...
    return handle_example_errors(run_fp8_tutorials, engine_kind);
}
