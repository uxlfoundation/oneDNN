...
private:
    static inline Product detectHWInfoImpl(cl_device_id device, cl_context context = nullptr);
...
{
    return getCore(detectHWInfoImpl(device).family);
}

template <HW hw>
HW OpenCLCodeGenerator<hw>::detectHW(cl_context context, cl_device_id device)
{
    return getCore(detectHWInfoImpl(device, context).family);
}

template <HW hw>
Product OpenCLCodeGenerator<hw>::detectHWInfo(cl_device_id device)
{
    return detectHWInfoImpl(device);
}

template <HW hw>
Product OpenCLCodeGenerator<hw>::detectHWInfo(cl_context context, cl_device_id device)
{
    return detectHWInfoImpl(device, context);
}


template <HW hw>
Product OpenCLCodeGenerator<hw>::detectHWInfoImpl(cl_device_id device, cl_context context)
{
    Product product = {};
