...
private:
    ...
    const reg64_savable_t reg_regular_batch {regscratchpad_, rbp, r18};

    const reg64_savable_t reg_bias {regscratchpad_, rbx, r24};
    const reg64_savable_t reg_scales {regscratchpad_, rbx};
    ...
    const reg64_savable_t reg_aux_scales {regscratchpad_, r10};
    const reg64_savable_t reg_dst_scales {regscratchpad_, r10};
...
void jit_brgemm_kernel_t<Wmm>::copy_post_ops_stack_values_to_aux(
    ...
    if (!is_reg_tail) {
        ...
        if (brg.with_scales) {
            reg_scales.restore();
            reg_scales.saveTo(reg_aux_scales);
        }
...
void jit_brgemm_kernel_t<Wmm>::read_params() {
    ...
    if (brg.type == brgemm_addr) {
        ...
        mov(reg_regular_batch, ptr[param1 + GET_OFF(batch)]);
        reg_regular_batch.save();
    }
...
void jit_brgemm_kernel_t<Wmm>::set_A_B_matrices() {
    if (brg.type == brgemm_addr) {
        ...
        add(reg_aux_A,
                ptr[reg_regular_batch + GET_OFF_BATCH_ELEMENT(offset.A)]);
        add(reg_aux_B,
                ptr[reg_regular_batch + GET_OFF_BATCH_ELEMENT(offset.B)]);
        add(reg_regular_batch, sizeof(brgemm_batch_element_t));
                ...
                if (vpad_exist) {
                    reg64_t reg_batch = (brg.type == brgemm_addr)
                            ...
                            : ((brg.type == brgemm_offs) ? reg_addr_batch
                                                         : reg_regular_batch);
                    if (brg.type == brgemm_strd) reg_regular_batch.restore();
