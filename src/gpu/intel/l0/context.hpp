...
struct event_t : public xpu::event_t {
    ...
    event_t(const std::vector<event_wrapper_t> &event) : events_(event) {}
    event_t(std::vector<event_wrapper_t> &&event) : events_(std::move(event)) {}
    event_t(event_wrapper_t &&event) { events_.emplace_back(std::move(event)); }
    ...
    const event_wrapper_t &operator[](size_t i) const { return events_[i]; }
    event_wrapper_t &operator[](size_t i) { return events_[i]; }
    ...
    std::vector<event_wrapper_t> events_;
};
