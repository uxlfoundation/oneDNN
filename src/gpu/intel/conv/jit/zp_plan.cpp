...
public:
    ...
    stmt_t create_stmt(const expr_t &wei_buf, const expr_t &dpas_buf,
        ...
        b_layout_.for_each_tile(tile, [&](const icoord_t &start) {
            auto off = offset_in_bytes<int>(b_layout_, start);
...
private:
    ...
    int get_comp_off(const icoord_t &start) const {
        int off = offset_in_bytes<int>(comp_layout_, start);
    ...
    int get_wei_off(const icoord_t &_start, int ck) const {
        auto start = _start;
        start[ck_idx_] = ck;
        int off = offset_in_bytes<int>(wei_layout_, start);
    ...
    int get_zp_off(const icoord_t &_start, int ck) const {
        ...
        int off = offset_in_bytes<int>(zp_layout_, start);
        return off;
    }
...
private:
    ...
    int get_mask_off(const icoord_t &start) const {
        int off = offset_in_bytes<int>(mask_layout_, start);
        return off;
    }
...
private:
    ...
    int get_c_off(const icoord_t &start, int kw) const {
        int off = offset_in_bytes<int>(c_layout_, start);
        return off;
    }
