...
public:
    ...
    void check_bounds(int off, int elems, ngen::DataType type,
            bool *is_dense = nullptr) const {
        gpu_assert(off >= 0 && elems >= 0);
        if (is_dense) *is_dense = true;
        if (elems == 0) return;
        ...
        if (is_dense) {
            ...
            for (int i = beg_off + 1; i <= end_off; ++i) {
                if (get_grf_base(i) != base0 + i) *is_dense = false;
            }
        }
    }
    ...
    bool is_dense(int bytes) const {
        bool ret = false;
        check_bounds(0, bytes, ngen::DataType::ub, /*is_dense=*/&ret);
        return ret;
    }
