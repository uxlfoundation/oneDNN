...
status_t acl_pooling_fwd_t::pd_t::init_scratchpad(
    ...
    if (!aux_mem_req.empty()) {
        ...
        if (req.size > 0) {
            ...
            scratchpad.book(memory_tracking::names::key_pool_int, req.size, 1,
                    align, align);
        }
    }

    const memory_desc_wrapper dst_d(&dst_md_);
    scratchpad.book(memory_tracking::names::key_pool_int, dst_d.nelems(),
            sizeof(float));
...
status_t acl_pooling_fwd_t::execute_forward(const exec_ctx_t &ctx) const {
    ...
    void *scratchpad_base = ctx.get_scratchpad_grantor().get<void>(
            memory_tracking::names::key_pool_int);
