...
#undef CASE
    ...
    if (kind_idx < primitive_kind::internal_only_start) {
        assert((size_t)kind_idx < sizeof(prim_kind_itt_domain)
                        / sizeof(prim_kind_itt_domain[0]));
        return prim_kind_itt_domain[kind_idx];
    }

    return prim_kind_itt_domain[0];
}
...
void primitive_task_start(primitive_kind_t kind, const char *task_name) {
    ...
    __itt_string_handle *task_label
            = __itt_string_handle_create(dnnl_prim_kind2str(kind));
    __itt_domain *task_domain = itt_domain(kind);
    __itt_task_begin(task_domain, __itt_null, __itt_null, task_label);
    __itt_string_handle *pd_info_fmt = __itt_string_handle_create("[%s]");
    __itt_formatted_metadata_add(task_domain, pd_info_fmt, task_name);

    thread_primitive_kind = kind;
    thread_primitive_name = task_label;
}
