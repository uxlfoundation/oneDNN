...
| input     |                          |                                |                                |
| attribute | normalized_shape         |                                | begin_norm_axis                |
...
- **src** (required): Input tensor of shape `[N, ...]` where normalization is
  performed over dimensions `[begin_norm_axis, ..., rank-1]`.
...
- **begin_norm_axis** (optional, default=-1): The first normalization dimension.
  If rank(src) is r, the allowed range of begin_norm_axis is [-r, r). Negative
  value means counting dimensions from the back. For example,
  `begin_norm_axis=-1` normalizes over the last dimension only.
...
rms_norm_op.set_attr<int64_t>(graph::op_attr::begin_norm_axis, -1);
...
- **begin_norm_axis** (optional, default=-1): Same as forward pass. The first
