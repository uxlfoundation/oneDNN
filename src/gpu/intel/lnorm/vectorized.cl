...
__kernel void vectorized_lnorm_fwd(__global DATA_T *src, __global float *mean,
...
#if USE_SCALE
        sm = block_load(
                sm, scale + c * SUB_GROUP_SIZE * VECT_DT_N + c_block_off);
        sm *= rsqrt_variance;
#else
        sm = rsqrt_variance;
#endif
...
#if USE_SHIFT
        sv = block_load(
                sv, shift + c * SUB_GROUP_SIZE * VECT_DT_N + c_block_off);
#else
        sv = 0.0f;
#endif
        x[NDIMS - 1] = c * SUB_GROUP_SIZE * VECT_DT_N + c_block_off;
    ...
    for (int c = 0; c < C_BLOCK; c += SUB_GROUP_SIZE * VECT_DT_N) {
        VECT_FLOAT_T sm;
#if USE_SCALE
        sm = block_load(sm, scale + c + c_block_off);
        sm *= r_sqrt_variance;
#else
        sm = r_sqrt_variance;
#endif
        VECT_FLOAT_T sv;
#if USE_SHIFT
        sv = block_load(sv, shift + c + c_block_off);
#else
        sv = 0.0f;
#endif
        x[NDIMS - 1] = c + c_block_off;
...
__kernel void vectorized_lnorm_bwd_scaleshift(__global DATA_T *src,
    ...
    if (USE_SCALE) block_write(diff_scale + result_offset, &diff_gamma_vect);
    if (USE_SHIFT) block_write(diff_shift + result_offset, &diff_beta_vect);
}
...
__kernel void vectorized_lnorm_bwd_scaleshift_final(
    ...
    if (diff_scale && lid == 0) write(diff_scale + c, diff_gamma);
    if (diff_shift && lid == 0) write(diff_shift + c, diff_beta);
}
