...
status_t init_ba_to_user_reorder(const engine_t *engine,
        const memory_desc_t *user_md, memory_desc_t &ba_md,
        std::shared_ptr<primitive_desc_t> &reorder_pd,
        memory_tracking::registrar_t &scratchpad,
        memory_tracking::key_t tmp_key, memory_tracking::key_t nested_key) {
    ba_md = *user_md;
    CHECK(memory_desc_init_by_tag(ba_md, format_tag::ba));
    CHECK(reorder_primitive_desc_create(reorder_pd, engine, &ba_md, user_md));

    const memory_desc_wrapper ba_d(&ba_md);
    scratchpad.book(tmp_key, ba_d.size(), 1, 64, 64);
    scratchpad.book(nested_key, reorder_pd->scratchpad_registry());
...
status_t kai_matmul_t::pd_t::init(const engine_t *engine) {
    ...
    // is ~(M*K + K*N).
    _swap_transAB_with_dst_transpose = src_md()->ndims == 2 && sdt == wdt
            && utils::one_of(
                    sdt, data_type::f32, data_type::bf16, data_type::f16)
            && utils::one_of(
                    ddt, data_type::f32, data_type::bf16, data_type::f16)
            && kai_gemm_type_ == kai_gemm_type::noquant && !_fixed_format
            && !with_bias() && attr_.post_ops_.len() == 0
            && helper.transA() == 'T' && helper.transB() == 'T'
            && helper.transC() == 'N'
            && static_cast<double>(M()) * N()
                    <= static_cast<double>(K()) * (M() + N());

    if (_swap_transAB_with_dst_transpose) {
        const dims_t swapped_src_dims = {N(), K()};
        const dims_t swapped_weights_dims = {K(), M()};
        const dims_t swapped_dst_dims = {N(), M()};
        CHECK(memory_desc_init_by_tag(
                swapped_src_md_, 2, swapped_src_dims, wdt, format_tag::ab));
        CHECK(memory_desc_init_by_tag(swapped_weights_md_, 2,
                swapped_weights_dims, sdt, format_tag::ab));
        CHECK(memory_desc_init_by_tag(
                swapped_dst_md_, 2, swapped_dst_dims, ddt, format_tag::ab));
    }

    _reorder_src_ba_to_ab
            = helper.transA() == 'T' && !_swap_transAB_with_dst_transpose;
    ...
    const post_ops_fusion_t post_ops_fusion
            = create_post_ops_fusion(attr_.post_ops_,
                    !with_bias() && !_reorder_dst_ab_to_ba
                            && !_swap_transAB_with_dst_transpose);
    ...
    const auto kernel_m = _swap_transAB_with_dst_transpose ? N() : M();
    const auto kernel_n = _swap_transAB_with_dst_transpose ? M() : N();
    const auto make_args = [&](bool fixed_format) {
        return std::make_shared<kai::ops::GemmArgs>(get_cpu_info(), kernel_m,
                kernel_n, K(), sections, _ag_nbatches, _ag_nmulti, indirect,
                post_ops_fusion.activation, num_threads, fixed_format,
                fast_mode, post_ops_fusion.accumulate, _cfg.get());
    };

    _args = make_args(_fixed_format);
    ...
    kernel = make_kernel();
    if (!kernel && _fixed_format && weights_md_was_any) {
        _fixed_format = false;
        _args = make_args(false);
        kernel = make_kernel();
    }
    VDISPATCH_MATMUL(kernel, VERBOSE_UNSUPPORTED_DT_CFG);

    // We need information from the concrete kernel config, but we do not overwrite _cfg
    ...
    // TODO: this is complicated and can result in two reorders. We should simplify this by either:
    // - making all KleidiAI kernels support transposed weights, or
    // - use fixed format kernels and do a single reorder from transposed to blocked fixed format
    //   (we can't do this yet because there are still some gaps in ff performance, even though it
    //    gives great speedups in some cases, there are ~10-20% slowdowns in other cases)
    const bool weights_are_transposed = !_fixed_format && helper.transB() == 'T'
            && !_swap_transAB_with_dst_transpose;
    _pack_weights = !_fixed_format && kernel->B_is_pretransposed();

    // KleidiAI's a64_sgemm_8x12 f16 ba -> f32 packed path uses a generic
    // transform. ACL matmul uses optimized f16 ba -> f16 ab + f16 ab -> f32
    // packed transforms which are faster in some cases conservatively switch to
    // ACL approach to avoid regressions
    const bool uses_sgemm_for_f16
            = kernel_cfg.filter == "a64_sgemm_8x12" && wdt == data_type::f16;
    ...
    if (_fixed_format) {
        ...
        VDISPATCH_MATMUL(kai_utils::is_fixed_format(kernel_cfg.weight_format),
    ...
    if (_swap_transAB_with_dst_transpose)
        VDISPATCH_MATMUL_SC(
                init_ba_to_user_reorder(engine, dst_md(), dst_ba_md_,
                        dst_ba_to_ab_reorder_pd_, scratchpad,
                        memory_tracking::names::key_matmul_dst_trans,
                        reorder_nested_key(nested_reorder_t::dst)),
                VERBOSE_PRIMITIVE_CREATION_FAIL, "dst transpose reorder");
...
status_t kai_matmul_t::init(engine_t *engine) {
    ...
    if (pd()->dst_ba_to_ab_reorder_pd_)
        CHECK(pd()->dst_ba_to_ab_reorder_pd_->create_primitive(
                dst_ba_to_ab_reorder_, engine));
...
status_t kai_matmul_t::create_resource(
    ...
    if (src_ba_to_ab_reorder_)
        CHECK(src_ba_to_ab_reorder_->create_resource(engine, mapper));
    if (weights_ba_to_ab_reorder_)
        CHECK(weights_ba_to_ab_reorder_->create_resource(engine, mapper));
    if (dst_ab_to_ba_reorder_)
        CHECK(dst_ab_to_ba_reorder_->create_resource(engine, mapper));
    if (dst_ba_to_ab_reorder_)
        CHECK(dst_ba_to_ab_reorder_->create_resource(engine, mapper));
...
status_t kai_matmul_t::execute(const exec_ctx_t &ctx) const {
    ...
    _kernel->set_nthreads(thread_partition.nthr);
    ...
    const bool dst_needs_reorder = pd()->_reorder_dst_ab_to_ba
            || pd()->_swap_transAB_with_dst_transpose;
    auto dst_base = dst_needs_reorder
    ...
    if (pd()->_swap_transAB_with_dst_transpose) {
        // The user weights buffer is row-major B^T (N x K), and the user source
        // buffer is row-major A^T (K x M), so KAI can compute B^T * A^T.
        kernel_src_base = raw_wei;
        kernel_raw_wei = src_base;
        kernel_wei_base = const_cast<void *>(kernel_raw_wei);
        if (pd()->_pack_weights) {
            kernel_wei_base = scratchpad.get<void>(
                    memory_tracking::names::key_matmul_wei_trans);
        }

        kernel_src_md = &pd()->swapped_src_md_;
        kernel_wei_md = &pd()->swapped_weights_md_;
        kernel_dst_md = &pd()->swapped_dst_md_;
    }
    ...
    if (pd()->_pack_weights) {
        parallel_pretranspose_B_array(*_kernel, kernel_wei_base, kernel_raw_wei,
                ld_wei, multi_stride_b, pd()->_pack_transposed_weights,
                thread_partition.team_nthr);
    }
    ...
    _kernel->set_arrays_generic(kernel_src_base, ld_src, batch_stride_a,
    ...
    if (pd()->_swap_transAB_with_dst_transpose) {
        CHECK(execute_reorder(ctx, dst_ba_to_ab_reorder_, &pd()->dst_ba_md_,
