...
struct jit_gemm_pd_t : public gpu_gemm_pd_t {
    ...
    dim_t stride_scale(int idx, const memory_desc_t &md) const;
    ...
    dim_t eff_a_scale_stride(int idx) const;
    dim_t eff_b_scale_stride(int idx) const;
    void quant_entry_init(const quant_entry_t &entry, const memory_desc_t &md,
            memory_desc_t &quant_md, int k_idx);
