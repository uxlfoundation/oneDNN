...
status_t brgemm_matmul_t<isa>::pd_t::init(engine_t *engine) {
    ...
    auto check_attr_scales = [&]() -> bool {
        ...
        // Per-batch scales (batch bits set in the mask) are supported via a
        // per-batch pointer offset. Grouping *over* the batch dimension is not.
        if (batch() > 1) {
            if (!asc.has_default_values(DNNL_ARG_SRC)
                    && asc.get_group(DNNL_ARG_SRC, 2) > 1)
                ok = false;
            if (!asc.has_default_values(DNNL_ARG_WEIGHTS)
                    && asc.get_group(DNNL_ARG_WEIGHTS, 2) > 1)
                ok = false;
        }
        ...
        if (!asc.has_default_values(DNNL_ARG_WEIGHTS)) {
            if (!asc.get(DNNL_ARG_WEIGHTS).has_default_groups()) {
                ...
                // 'per_ocic' (K/N) mask is supported, optionally with batch
                // bits set for per-batch ('per_tensor') scales. In numbers,
                // for 3D both '6' (per_ocic) and '7' (per_tensor) are allowed.
    ...
    auto check_attr_zero_points
        ...
        if (!zp.has_default_values(DNNL_ARG_SRC)) {
            ...
            if (allow_grouped_src_zp) {
                ...
                // Per-batch src ZP are supported via a per-batch pointer
                // offset; grouping over the batch dimension is not.
                const bool grouped_over_batch
                        = zp.get_group(DNNL_ARG_SRC, 2) > 1;
                const bool mask_ok = (mask & ~(mk_mask | batch_mask)) == 0;
                return !(grouped_over_batch && batch() > 1) && mask_ok;
            } else {
                return mask == 0;
            }
        }
        if (!zp.has_default_values(DNNL_ARG_WEIGHTS)) {
            ...
            if (allow_multiple_wei_zp) {
                ...
                const int batch_mask = full_tensor_mask() & ~kn_mask;
                // Per-batch weights ZP are supported via a per-batch pointer
                // offset; grouping over the batch dimension is not.
                const bool grouped_over_batch
                        = zp.get_group(DNNL_ARG_WEIGHTS, 2) > 1;
                const bool mask_ok = (mask & ~(kn_mask | batch_mask)) == 0;
                return !(grouped_over_batch && batch() > 1) && mask_ok;
            } else {
                return mask == 0;
