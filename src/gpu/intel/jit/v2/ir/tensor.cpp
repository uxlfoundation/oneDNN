...
static void advance(pvar_icoord_t &idx,
...
dim_t layout_t::offset_in_bytes(const pvar_coord_t &_coord) const {
    gpu_assert(has_const_sizes() && has_const_strides());
    dim_t ret = to_cpp<dim_t>(base_);
    auto coord = _coord.to_int();
    for (int i = 0; i < nblocks(); i++) {
        auto &b = blocks_[i];
...
bool try_div_mod(const expr_t &a, int b, const var_range_info_t &range_info, expr_t &div, expr_t &mod) {
if (is_const(a)) {
    ...
    for (auto &b : blocks()) {
        ...
        for (int i = 0; i < linear.nargs(); i++) {
            ...
            if (b.has_const_size() && !is_outer) {
                ...
                expr_t div;
                expr_t mod;
                if (try_div_mod(idxs[dim], b.int_size(),
...
pvar_coord_t layout_t::to_coord(
...
pvar_coord_t layout_iterator_t::coord() const {
    pvar_coord_t ret;
    pvar_tile_t sizes;
    for (int i = 0; i < parent_->nblocks(); i++) {
        auto &b = parent_->blocks()[i];
...
expr_t dim_mask_desc_t::to_expr(
        const pvar_coord_t &coord, bool with_const) const {
...
dim_mask_desc_t dim_mask_desc_t::map(const pvar_coord_t &coord) const {
    auto ret = *this;
...
mask_desc_t mask_desc_t::map(const pvar_coord_t &coord) const {
    auto ret = *this;
...
view_t view_t::split(const dim_mapper_t &dim_mapper,
        const layout_t &base_layout, const pvar_coord_t &_coord,
    ...
    pvar_coord_t split_coord = coord;
