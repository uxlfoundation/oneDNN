    ...
    jit_brgemm_amx_uker_base_t(const brgemm_desc_t &abrg)
        ...
        if (brg.with_eltwise || brg.with_binary || brg.with_sum) {
            ...
            const binary_injector::rhs_arg_static_params_t rhs_sp {
                    static_cast<dim_t>(Xbyak::Zmm(1).getIdx()), this->r14,
...
dim_t jit_brgemm_amx_uker_base_t::get_C_tensor(
    ...
    // m/n are AMX tile-block indices, bounded by the descriptor's
    // bd_block2/ld_block2 (<= AMX_TILES_NUM); brg.get_C_tensor() takes int.
    const int im = static_cast<int>(m);
    const int in = static_cast<int>(n);
    return brg.get_C_tensor(im, in, bi.bdi->is_tail(im), bi.ldi->is_tail(in));
}
...
void jit_brgemm_amx_uker_base_t::maybe_tileloadd_nt(
    ...
    // xdb is an AMX tile-block index, bounded by bd_block2/ld_block2;
    // brg.get_A_tensor()/get_B_tensor() take int.
    const int ixdb = static_cast<int>(xdb);
    auto t1 = Tmm(xbyak_register_index(is_A
                    ? brg.get_A_tensor(ixdb, bi.bdi->is_tail(ixdb))
                    : brg.get_B_tensor(ixdb, bi.ldi->is_tail(ixdb))));
...
void jit_brgemm_amx_uker_base_t::maybe_fused_copy_A_nt_load(
    ...
    // bdb is an AMX tile-block index, bounded by bd_block2;
    // brg.get_A_tensor() takes int.
    const int ibdb = static_cast<int>(bdb);
    auto t1 = Tmm(xbyak_register_index(
            brg.get_A_tensor(ibdb, bi.bdi->is_tail(ibdb))));
...
void jit_brgemm_amx_uker_base_t::tdpbxxd(brgemm_iteration_t &bi, dim_t bdb_idx,
    ...
    // bdb_idx/ldb_idx are AMX tile-block indices, bounded by
    // bd_block2/ld_block2; brg.get_A_tensor()/get_B_tensor() take int.
    const int ibdb_idx = static_cast<int>(bdb_idx);
    const int ildb_idx = static_cast<int>(ldb_idx);
    const Tmm &x1
            = Tmm(xbyak_register_index(get_C_tensor(bi, bdb_idx, ldb_idx)));
    const Tmm &x2 = Tmm(xbyak_register_index(
            brg.get_A_tensor(ibdb_idx, bi.bdi->is_tail(ibdb_idx))));
    const Tmm &x3 = Tmm(xbyak_register_index(
            brg.get_B_tensor(ildb_idx, bi.ldi->is_tail(ildb_idx))));
