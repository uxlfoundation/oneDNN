...
*******************************************************************************/
#ifndef GPU_INTEL_BNORM_XE_H
#define GPU_INTEL_BNORM_XE_H
...
#if HAS_IC_TAIL
#define MAYBE_LAST_IC_BLOCK_LOAD_1x16(var, ptr, idx) \
    do { \
        if (is_last_ic_block) \
            var = simd_id < 8 ? ptr[(idx) + simd_id] : 0.0f; \
        else \
            var = block_load(var, &ptr[(idx)]); \
    } while (0)
#else
#define MAYBE_LAST_IC_BLOCK_LOAD_1x16(var, ptr, idx) \
    var = block_load(var, &ptr[(idx)])
...
#if IS_FWD
#if USE_STATS_ONE_PASS
void xe_mean_var_calc_fused_reduction(volatile __global atomic_float *mean,
...
#else // regular alg
void xe_calc_fused_reduction(volatile __global atomic_float *dst,
...
#if IS_BWD
void xe_calc_fused_reduction(volatile __global atomic_float *diff_scale,
