...
status_t pd_t::init(const impl::engine_t *engine, compute::gpu_arch_t arch) {
    arch_ = arch;
...
static status_t init_post_ops(
        kernel_config_t &cfg, const pd_t *pd, const impl::engine_t *engine) {
    ...
    if (seen_sum) {
        const auto idx = po.find(primitive_kind::sum);
        const auto &sum = po.entry_[idx].sum;
        if (cfg.beta * sum.zero_point != 0) {
            post_ops_t::entry_t::eltwise_t linear {
                    eltwise_linear, 1.0f, 1.0f, -cfg.beta * sum.zero_point};
            post_ops_t::entry_t linear_entry;
            linear_entry.kind = primitive_kind::eltwise;
            linear_entry.eltwise = linear;
            po.entry_.insert(po.entry_.begin() + idx, linear_entry);
            cfg.binary_srcs.insert(cfg.binary_srcs.begin() + idx,
    ...
    if (bias_via_binary) {
        ...
        // A binary bias blocks folding a host-scalar dst scale into alpha.
        cfg.c_host_scale_to_alpha = false;
    }
    ...
    bool try_c_scale
            = !c_scales.is_host_scalar() || !cfg.c_host_scale_to_alpha;
...
status_t zp_ok(const pd_t *pd, const impl::engine_t *engine) {
    using namespace data_type;
    const auto d = pd->desc();
    ...
    if (!a_zps.has_default_values()) {
        ...
        // Disable this path.
        VDISPATCH_GEMM_F(IMPLICATION(has_prB, b_zps.has_default_values()),
...
status_t gs_ok(const pd_t *pd, const impl::engine_t *engine) {
...
status_t scales_ok(const pd_t *pd, const impl::engine_t *engine) {
    const auto &scales = pd->attr()->scales_;
...
status_t finalize_problem(kernel_config_t &cfg, const intel::engine_t *engine) {
    ...
    if (problem.needsAGroupSums()) {
        VDISPATCH_GEMM_IC(problem.hasGroupSumsA, VERBOSE_UNSUPPORTED_ZP_CFG);
    ...
    if (problem.needsBGroupSums()) {
        VDISPATCH_GEMM_IC(problem.hasGroupSumsB, VERBOSE_UNSUPPORTED_ZP_CFG);
