...
void CopyPlan::planInt4Downconversion(CopyInstruction &i)
{
    ...
    if(ddst.stride >= sStride && sStride > 1 && ssrc.type == DataType::ub){
        int dst_mask = 0x0;//ffffffff;
        switch (ddst.stride) {
            case 2:
                //(ddst.offset % 2) ? dst_mask = 0xf0f0f0f0 : 
                dst_mask = 0x0f0f0f0f << ((ddst.offset % 2) * 4);
                break;
            case 4:
                dst_mask = 0x000f000f << ((ddst.offset % 4) * 4);
                break;
            case 8:
                dst_mask = 0x0000000f << ((ddst.offset % 8) * 4);
                break;
            default: stub();
        }
