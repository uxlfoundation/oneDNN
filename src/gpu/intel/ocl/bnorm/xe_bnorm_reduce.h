...
*******************************************************************************/
#ifndef GPU_INTEL_OCL_BNORM_XE_BNORM_REDUCE_H
#define GPU_INTEL_OCL_BNORM_XE_BNORM_REDUCE_H

#include "gpu/intel/ocl/bnorm/xe_bnorm.h"
...
__kernel void xe_fused_reduce_init(
#if IS_FWD
...
#if IS_FWD
NAMED_KERNEL_ATTR(AUX)
__kernel void xe_fused_reduce_final(
...
__kernel void xe_reduce_mean_var(__global ACCUM_DATA_T *reduce_temp,
        __global float *mean, __global float *variance) {
...
void xe_reduce_common(__global float *reduce_temp, __local float *local_sum,
        __global float *dst) {
...
NAMED_KERNEL_ATTR(REDUCE)
__kernel void xe_reduce_mean(
        __global float *reduce_temp, __global float *mean) {
    __local float local_sum[SG_SIZE * REDUCE_IC_SUB_GROUPS];
    xe_reduce_common(reduce_temp, local_sum, mean);
}

NAMED_KERNEL_ATTR(REDUCE)
__kernel void xe_reduce_variance(
...
#if IS_BWD
NAMED_KERNEL_ATTR(REDUCE)
__kernel void xe_reduce_stats(__global float *temp_reduce,
        __global float *diff_scale, __global float *diff_shift,
