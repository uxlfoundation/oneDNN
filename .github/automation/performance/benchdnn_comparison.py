...
def compare_two_benchdnn(file1, file2, tolerance=0.05):
    ...
    for prb in r1_exec:
        ...
        # - Execution time shows statistically significant regression (t-test p ≤ 0.05 and ≥ 10% slowdown)
        # - Creation time shows ≥ 10% slowdown in median value
        exec_regressed = res.pvalue <= 0.05 and ((r2_med_exec - r1_med_exec) / r1_med_exec >= 0.1)
        ctime_regressed = ((r2_med_ctime - r1_med_ctime) / r1_med_ctime >= 0.1)
