...
uchar __attribute__((overloadable)) get_half_byte(const __global uchar *x, off_t y) {
    uchar ret = 0;
    if (y % 2) {
...
char __attribute__((overloadable)) get_half_byte(const __global char *x, off_t y) {
    if (y % 2) {
        return (x[y / 2] & 0xf0) >> 4;
    } else {
        return x[y / 2] & 0x0f;
