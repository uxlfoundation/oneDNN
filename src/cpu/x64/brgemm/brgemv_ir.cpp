...
void emit_bs_body(ir::ir_t &ir, const brgemv_ir_conf_t &cfg,
    ...
    // Reduce full K blocks, then handle the tail (if any).
    // We handle `k_blocks` in three cases:
    //   k_blocks == 0: emit_loop_imm runs body once, no advance
    //   k_blocks == 1: emit single block, advance manually if tail exists
    //   k_blocks >= 2: emit_loop_imm loop, advance handled per iteration
    // In all cases, `advance` is kept explicit to ensure correct tail
    // positioning.
    if (cfg.k_blocks >= 2) {
...
bool brgemv_ir_supported(const brgemm_desc_t &brg) {
    ...
    if (!utils::everyone_is(f32, brg.dt_a, brg.dt_b, brg.dt_c, brg.dt_d))
        return false;
    ...
    const dim_t m = brg.bcast_dim, k = brg.reduce_dim;
    ...
    if (!fits(dt_sz_a * (dim_t)(m_block - 1) * brg.LDA)) return false;
    if (!fits(dt_sz_y * (dim_t)(m_block - 1) * brg.LDC)) return false;
    if (!fits(dt_sz_a * (dim_t)m_block * brg.LDA)) return false;
    if (!fits(dt_sz_y * (dim_t)m_block * brg.LDC)) return false;

    return true;
}

brgemm_kernel_t *new_brgemv_ir_kernel(const brgemm_desc_t &brg) {
