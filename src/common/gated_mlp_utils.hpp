...
static inline gated_mlp_desc_t create_gated_mlp_desc(
    ...
    if (gate_attr) {
        gated_mlp_desc.wts_gate_scales      = gate_attr->scales_; // SRC_0
        gated_mlp_desc.wts_gate_zero_points = gate_attr->zero_points_;
    }
    if (up_attr) {
        gated_mlp_desc.wts_up_scales      = up_attr->scales_; // SRC_1
        gated_mlp_desc.wts_up_zero_points = up_attr->zero_points_;
    }
    if (down_attr) {
        gated_mlp_desc.wts_down_scales      = down_attr->scales_; // SRC_2
...
static inline status_t create_gated_mlp_pd(
    ...
    dnnl_dim_t mb = src_md->dims[0];
    dnnl_dim_t ic = src_md->dims[1];
    dnnl_dim_t oc = W_gate_md->dims[1];
