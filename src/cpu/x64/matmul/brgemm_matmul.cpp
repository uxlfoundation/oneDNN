...
status_t brgemm_matmul_t<isa>::pd_t::init(engine_t *engine) {
    ...
    for (int prefetching = 0; prefetching < 2; prefetching++) {
        ...
        // brgemm expects a 2D dst zp mask; remap `dst_qmask_N()` to 1 << 1
        const primitive_attr_t *brg_attr = attr();
        primitive_attr_t local_attr;
        if (!attr()->zero_points_.has_default_values(DNNL_ARG_DST)
                && attr()->zero_points_.get_mask(DNNL_ARG_DST) == dst_qmask_N()
                && dst_qmask_N() != (1 << 1)) {
            CHECK(local_attr.copy_from(*attr()));
            CHECK(local_attr.zero_points_.set(DNNL_ARG_DST, 1 << 1));
            brg_attr = &local_attr;
        }
        CHECK(brgemm_desc_set_postops(
                &brg, brg_attr, &dst_md_, LDD, bgmmc_.bia_dt));
