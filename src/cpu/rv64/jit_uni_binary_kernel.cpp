...
void jit_uni_binary_kernel_t::init_post_ops_injector() {
    ...
    // with (post_ops_ok(mayiuse(zvfh) ? zvfh : v)) so its is_data_supported
    // contract admits exactly the rhs dtypes the pd accepted (an f16 rhs is
    // legal only on zvfh). RVV codegen is identical either way.
    if (mayiuse(zvfh))
