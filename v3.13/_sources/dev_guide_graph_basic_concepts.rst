...
``Compiled partition`` (:ref:`dnnl::graph::compiled_partition <doxid-classdnnl_1_1graph_1_1compiled__partition>`) represents the generated code specialized for a target hardware and tensor metadata passed through compilation API. To execute a compiled partition (:ref:`dnnl::graph::compiled_partition::execute <doxid-classdnnl_1_1graph_1_1compiled__partition_1aa93ee468a74520638dc87212415b1ea6>`), both input and output tensors, and a stream (:ref:`dnnl::stream <doxid-structdnnl_1_1stream>`) are required to pass. Input and output tensors must bind data buffers to the input and output logical tensors respectively.
