...
int check_s8s8_reorder(const prb_t &prb, rnn_data_kind_t kind,
    ...
    const auto mem_fp_h = mem_fp.get_host_f32_handle();
        ...
        auto quantize_chunk
                = [&](const float *scales, int nscales, int idx_chunk) {
                      ...
                      for (int64_t idx = idx_start; idx < idx_end; ++idx) {
                          ...
                          mem_fp_h[idx] = MAX2(MIN2(val, max), min);
                      }
                  };
        switch (kind) {
            case WEIGHTS_LAYER:
...
int fill_weights(const prb_t &prb, rnn_data_kind_t kind, dnn_mem_t &mem_dt,
    ...
    auto mem_fp_h = mem_fp.get_host_f32_handle();
    auto mem_pure_fp_h = mem_pure_fp.get_host_f32_handle();
    ...
    benchdnn_parallel_nd(
            L, D, G, O, [&](int64_t l, int64_t d, int64_t g, int64_t o) {
                ...
                mem_pure_fp_h[off] = val;
                if (prb.is_int8()) val *= scales[off % n_scales];
                mem_fp_h[off] = round_to_nearest_representable(dt, val);
            });
...
int fill_bias(const prb_t &prb, rnn_data_kind_t kind, dnn_mem_t &mem_dt,
    ...
    benchdnn_parallel_nd(n_chunks, [&](int64_t idx_chunk) {
        ...
        for (int64_t idx = idx_start; idx < idx_end; ++idx) {
            ...
            mem_fp_h[idx]
                    = round_to_nearest_representable(prb.cfg[kind].dt, val);
        }
    });
