...
void bench_sdpa(engine::kind ekind, logical_tensor::data_type dt,
    ...
    auto softmax_b = op(id++, op::kind::SoftMax, "softmax");
    softmax_b.set_attr<int64_t>(op::attr::axis, -1);
    softmax_b.set_attr<std::string>(op::attr::mode, "inf_as_zero");
    softmax_b.add_inputs({masked_score_b});
    softmax_b.add_outputs({probs_b});
    ...
    scale_div_b2.add_inputs({dmasked_score, scale_b2});
    ...
    auto ts_scale_b2 = tensor(scale_b2, eng);
    ...
    write_to_dnnl_tensor(scale_data.data(), ts_scale_b2);

    // Warmup run.
    // Execute the compiled partition of sdpa.
    cp_b.execute(strm,
            {ts_query, ts_key, ts_scale, ts_mask, ts_value, ts_scale_b2,
                    ts_output, ts_doutput},
