...
size_t engine_factory_t::count() const {
    ...
    func_zeDriverGet(&driver_count, nullptr);

    std::vector<ze_driver_handle_t> drivers(driver_count);
    func_zeDriverGet(&driver_count, drivers.data());

    uint32_t device_count = 0;
    func_zeDeviceGet(drivers[0], &device_count, nullptr);
...
status_t engine_factory_t::engine_create(
    ...
    uint32_t device_count = 0;
    CHECK(func_zeDeviceGet(driver, &device_count, nullptr));
    ...
    CHECK(func_zeDeviceGet(driver, &device_count, devices.data()));
    ...
    CHECK(func_zeContextCreate(driver, &context_desc, &context));
