...
TEST(IRBuilderTests, BroadcastDefUse) {
    ...
    const vreg_t b = ir.new_vec(data_type::f32);
    ir.vload_bcast(b, ptr, (dim_t)sizeof(float), data_type::f32);

    ASSERT_EQ(ir.n_ops(), 2);
    EXPECT_EQ(ir.ops()[1].kind, op_kind_t::vload_bcast);
...
TEST(IntegrationTests, BroadcastScalesWholeVector) {
    ...
    ir.vload_bcast(scale, scale_ptr, sizeof(float), data_type::f32);
    ir.vmul(input, scale);
