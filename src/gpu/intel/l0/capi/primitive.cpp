...
dnnl_status_t dnnl_l0_interop_primitive_execute(
    ...
    const bool ok = !dnnl::impl::utils::any_null(primitive_iface, stream)
            && primitive_iface->engine() == stream->engine()
            && primitive_iface->engine()->runtime_kind()
                    == dnnl::impl::runtime_kind::l0
            && IMPLICATION(nargs > 0, args != nullptr)
            && IMPLICATION(ndeps > 0, deps != nullptr);
    if (!ok) return dnnl::impl::status::invalid_arguments;

    auto *l0_stream_impl = dnnl::impl::utils::downcast<
            dnnl::impl::gpu::intel::l0::stream_impl_t *>(stream->impl());
    ...
    if (deps != nullptr) {
        ...
        l0_stream_impl->ctx().set_deps(events);
    }

    // run primitive
    dnnl::impl::exec_args_t exec_args;
    CHECK(cvt_primitive_args(
            primitive_iface->pd()->impl().get(), nargs, args, exec_args));

    dnnl::impl::exec_ctx_t ctx(stream, std::move(exec_args));
    CHECK(primitive_execute(primitive_iface, ctx));

    // return output event
    if (return_event != nullptr) {
        if (l0_stream_impl->flags() & dnnl::impl::stream_flags::in_order) {
            *return_event = nullptr;
        } else {
            *return_event = l0_stream_impl->get_output_event();
        }
    }
