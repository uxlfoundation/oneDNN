...
The Unary category includes operations such as: Abs, Clamp, Elu, Exp, GELU,
HardSigmoid, HardSwish, LeakyReLU, Log, Mish, Sigmoid, SoftPlus, ReLU, Round,
Sqrt, Square, Tanh.
...
1. **Unary Operation**: Performs the corresponding unary operation for the
   `src` tensor. See the [Abs](@ref dev_guide_op_abs),
     ...
     [Square](@ref dev_guide_op_square) and [Tanh](@ref dev_guide_op_tanh)
