...
status_t init_brgemm_matmul_conf(cpu_isa_t isa, brgemm_matmul_conf_t &bgmmc,
    ...
    // For per_n zp, force N_blk=1 for accurate per-column compensation
    ...
    // For per_n zp compensation calculation requires N_blk=1 which only BRGEMM
    // supports
...
void init_scratchpad(memory_tracking::registrar_t &scratchpad,
    ...
    if (bgmmc.wei_zp_type == brgemm_broadcast_t::per_n)
        scratchpad.book(key_brgemm_primitive_zp_b_per_oc, bgmmc.N,
                types::data_type_size(s32));
