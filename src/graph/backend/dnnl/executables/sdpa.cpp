...
sdpa_bwd_executable_t::sdpa_bwd_executable_t(std::shared_ptr<op_t> &op,
    ...
    status_t s = create_sdpa_pd(hint_fwd_pd, p_engine.get(), md_q.get(),
            ...
            impl::prop_kind::forward_training);
    if (s != dnnl::impl::status::success) {
        is_initialized_ = false;
        return;
    }
...
void sdpa_bwd_executable_t::execute(const stream &stream,
    ...
    memory_arg_t mem_arg_k = {args.at(DNNL_ARG_KEYS).get(), true};
    memory_arg_t mem_arg_v = {args.at(DNNL_ARG_VALUES).get(), true};
    memory_arg_t mem_arg_dst = {args.at(DNNL_ARG_DST).get(), true};
    memory_arg_t mem_arg_diff_dst = {args.at(DNNL_ARG_DIFF_DST).get(), true};
    memory_arg_t mem_arg_workspace = {args.at(DNNL_ARG_WORKSPACE).get(), true};
    memory_arg_t mem_arg_scale
            = {with_scale_ ? args.at(DNNL_ARG_SCALE).get() : nullptr, true};
    memory_arg_t mem_arg_mask = {
            with_explicit_mask_ ? args.at(DNNL_ARG_ATTN_MASK).get() : nullptr,
            true};
    memory_arg_t mem_arg_diff_q = {args.at(DNNL_ARG_DIFF_QUERIES).get(), false};
    memory_arg_t mem_arg_diff_k = {args.at(DNNL_ARG_DIFF_KEYS).get(), false};
    memory_arg_t mem_arg_diff_v = {args.at(DNNL_ARG_DIFF_VALUES).get(), false};
    memory_arg_t mem_arg_scratchpad
    ...
    exec_args[DNNL_ARG_VALUES] = mem_arg_v;
    exec_args[DNNL_ARG_DST] = mem_arg_dst;
    exec_args[DNNL_ARG_DIFF_DST] = mem_arg_diff_dst;
    exec_args[DNNL_ARG_WORKSPACE] = mem_arg_workspace;
    exec_args[DNNL_ARG_SCALE] = mem_arg_scale;
    exec_args[DNNL_ARG_ATTN_MASK] = mem_arg_mask;
    exec_args[DNNL_ARG_DIFF_QUERIES] = mem_arg_diff_q;
    exec_args[DNNL_ARG_DIFF_KEYS] = mem_arg_diff_k;
    exec_args[DNNL_ARG_DIFF_VALUES] = mem_arg_diff_v;
    exec_args[DNNL_ARG_SCRATCHPAD] = mem_arg_scratchpad;
