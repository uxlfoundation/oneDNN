...
struct GEMMProblem : public CommonProblem {
    ...
    int ascPtrDims = -1, bscPtrDims = -1;           // A/B scale dimensionality (-1: none; 0: scalar; 1: vector, 2: matrix)
    ...
    bool aScale2D() const { return (ascPtrDims == 2); }
    bool bScale2D() const { return (bscPtrDims == 2); }

    bool quantized2DA() const { return (aoPtrDims == 2) || (ascPtrDims == 2); }
    bool quantized2DB() const { return (boPtrDims == 2) || (bscPtrDims == 2); }
    ...
    bool earlyDequantizeB() const {
        return (bOffset == ABOffset::Calc && earlyDequantizableOffset(Tb_ext, Tbo, Tb)
            ...
            || ((bScale2D() == 2) && (Tb_scale.isSubsetOf(Tb) || downconvertBScales()));
    }
    ...
    void serialize(SerializationStream &s) const
    {
        ...
        s.append(ascPtrDims, bscPtrDims);
