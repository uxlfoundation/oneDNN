...
struct kernel_config_t {
    ...
    // Attr-level: a dst scale was requested, whatever lowering does with it
    // (binary post-op, alpha, or MX). Set in init_attrs.
    bool with_c_scales = false;
    ...
    // Position in the attr chain, not the lowered one: a sum zero point
    // injects a linear eltwise ahead of the sum without invalidating this.
...
struct pd_t : public gemm::pd_t {
    ...
    // init_post_ops additionally clears it for a binary bias.
