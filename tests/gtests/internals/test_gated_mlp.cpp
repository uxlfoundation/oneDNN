...
void bench_gated_mlp_internal(std::vector<T> &res, double &avg_time,
    ...
    const memory::dims FC_retn_sz_t = {p.ic, p.mb};
    auto FC_retn_md_t
    ...
    switch (p.qtype) {
        ...
        case quantize_type::per_tensor: {
            int mask = (p.qtype == quantize_type::per_token_with_groups)
                    ...
                    : 0;
            //wts_gate scale+zp
            attr.set_scales(DNNL_ARG_WEIGHTS_GATE, mask,
                    {1, p.gateup_group_size},
                    m_W_gate_scales_md.get_data_type());
            attr.set_zero_points(DNNL_ARG_WEIGHTS_GATE, mask,
                    {1, p.gateup_group_size}, m_W_gate_zp_md.get_data_type());
            //wts_up scale+zp
            attr.set_scales(DNNL_ARG_WEIGHTS_UP, mask, {1, p.gateup_group_size},
                    m_W_up_scales_md.get_data_type());
            attr.set_zero_points(DNNL_ARG_WEIGHTS_UP, mask,
    ...
    auto gmlp_pd = [&]() {
        ...
        //activation = dnnl_alg_kind_t::dnnl_eltwise_exp; //shouldfail
        if (p.qtype == quantize_type::no_quantization) {
            return gmlp::primitive_desc(eng, O_proj_md, W_gate_md, W_up_md,
                    W_down_md, FC_retn_md_t, activation, attr);
        } else {
            return gmlp::primitive_desc(eng, O_proj_md, m_W_gate_quant_md,
                    m_W_up_quant_md, W_down_md, FC_retn_md_t, activation, attr);
        }
    }();
...
// clang-format off
INSTANTIATE_TEST_SUITE_P(VEC, mlp_test, testing::Values(
