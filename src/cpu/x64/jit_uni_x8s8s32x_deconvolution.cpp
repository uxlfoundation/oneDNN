...
std::function<Vmm()> jit_uni_x8s8s32x_deconv_fwd_kernel_vmm_t<isa,
    ...
    const int start_vmm_idx
            = vmm_inp(ur_w - 1, jcp_.nb_oc_blocking).getIdx();
...
void jit_uni_x8s8s32x_deconv_fwd_kernel_vmm_t<isa,
    ...
    if (jcp_.ndims > 3) {
        ...
        add(reg_zp_src_pad_comp, static_cast<int>(kh_offset));
        ...
        for (dim_t icb1 = 0; icb1 < n_ic_blocks; icb1++) {
            if (h_padded) {
                ...
                for (dim_t jj = _start; jj < _end; jj += ur_w_stride) {
                    ...
                    const auto vmm_src = vmm_inp(
                            jj, jcp_.nb_oc_blocking);
            ...
            for (dim_t ocb = 0; ocb < jcp_.nb_oc_blocking; ocb++) {
                ...
                if (_end - _start > 0) {
                    if (jcp_.is_depthwise) {
                        uni_vpmovsxbd(vmm_wei_,
                                ptr[aux_reg_filt_
                                        + aux_filt_off]);
                    } else
                        uni_vmovups(vmm_wei_,
                                ptr[aux_reg_filt_
                                        + aux_filt_off]);
                }
                ...
                for (dim_t jj = _start; jj < _end; jj += ur_w_stride) {
                    ...
                    if (jcp_.signed_input || !inside_padded_area) {
                        const Vmm inp
                                = vmm_inp(h_padded ? 0 : jj, jcp_.nb_oc_blocking);
...
void jit_uni_x8s8s32x_deconv_fwd_kernel_vmm_t<isa, Vmm>::kh_loop(dim_t ur_w,
        ...
        if (signed_input_or_src_zp) {
            ...
            L(back_overflow_label);
            {
                ...
                L(d_h_overflow_label);
                {
                    ...
                    add(aux_reg_filt_, static_cast<int>(shift_filt_kh));
                    dec(reg_kh_);
                    jnz(d_h_overflow_label);
                }

                add(aux_reg_filt_d_, static_cast<int>(shift_filt_kd));
                dec(reg_ki_);
    ...
    L(kh_loop_label);
    {
        ...
        sub(aux_reg_src_, static_cast<int>(shift_src_ih));
        add(aux_reg_filt_, static_cast<int>(shift_filt_kh));
        dec(reg_kh_);
    ...
    if (jcp_.ndims == 5) {
        sub(aux_reg_src_d_, static_cast<int>(shift_src_id));
        add(aux_reg_filt_d_, static_cast<int>(shift_filt_kd));
        dec(reg_ki_);
...
template <cpu_isa_t isa, typename Vmm>
void jit_uni_x8s8s32x_deconv_fwd_kernel_vmm_t<isa, Vmm>::apply_postops(dim_t ur_w,
...
void jit_uni_x8s8s32x_deconv_fwd_kernel_vmm_t<isa, Vmm>::store_output(
    ...
    if (jcp_.with_eltwise || jcp_.with_binary || jcp_.with_sum)
        apply_postops(
                ur_w, last_oc_block, p_sum_scale, p_sum_zp);
...
void jit_uni_x8s8s32x_deconv_fwd_kernel_vmm_t<isa, Vmm>::generate() {
    ...
    if (jcp_.ur_w == jcp_.ow) {
        ...
        add(reg_src_, static_cast<int>(src_shift));
        add(reg_dst_, static_cast<int>(dst_shift));
