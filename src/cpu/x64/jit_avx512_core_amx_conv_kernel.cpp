    ...
    for (int oc = 0; oc < nb_oc_block; oc++) {
        ...
        for (dim_t ur = 0; ur < ur_w; ur++) {
            ...
            vmovups(EVEX_compress_addr(
                            reg_zp_pbuff, static_cast<int>(output_offset)),
                    m_zmm_dst);
        }
    }
}
...
void jit_avx512_core_amx_compute_zp_pbuff_t::kh_loop(dim_t ur_w, dim_t pad_l,
    ...
    auto compute_kh_loop = [&](size_t param_overflow) {
        ...
        L(overflow_label);
        {
            ...
            add(aux_reg_filt, static_cast<int>(shift_wei_h_step));
            dec(reg_overflow);
...
void jit_avx512_core_amx_compute_zp_pbuff_t::kd_loop(dim_t ur_w, dim_t pad_l,
    ...
    auto compute_kd_loop = [&](size_t param_overflow) {
        ...
        L(overflow_label);
        {
            ...
            add(aux_reg_filt_d, static_cast<int>(shift_wei_h_step * jcp.kh));
            dec(reg_ki);
            jne(overflow_label, T_NEAR);
        }
        L(no_overflow_label);
...
void jit_avx512_core_amx_compute_zp_pbuff_t::icb_loop(
    ...
    if (do_icb_loop) {
        ...
        add(reg_filt, static_cast<int>(sizeof(char) * shift_wei_icb_step));
...
void jit_avx512_core_amx_compute_zp_pbuff_t::unroll_width(
    ...
    while (cur_l_pad_output > 0) {
        ...
        add(reg_zp_pbuff, static_cast<int>(ur_w_shift(ur_w)));
...
void jit_avx512_core_amx_copy_to_wbuffer_t::generate() {
    ...
    for (dim_t g = 0; g < jcp.ngroups; g++) {
        for (dim_t ocb = 0; ocb < jcp.nb_oc; ocb++) {
            ...
            add(reg_src, static_cast<int>(ocb_src_step));
            add(reg_dst, static_cast<int>(ocb_dst_step));
        }
    }
...
void jit_avx512_core_amx_copy_to_pbuffer_t::copy_row_reduced_lowering() {
    ...
    { // Handle Left Overflow
        ...
        L(label_lov); // handle left or right overflow
        {
            ...
            L(label_lov_inner);
            {
                ...
                add(reg_aux_out_ptr, static_cast<int>(out_h_step));
                dec(reg_cnt);
                jnz(label_lov_inner, T_NEAR);
            }
            add(reg_out_ptr, static_cast<int>(out_w_step));
            dec(reg_lov);
    ...
    L(label_tov); // handle top overflow
    {
        ...
        {
            zero_it(reg_aux_out_ptr);
            add(reg_aux_out_ptr, static_cast<int>(out_w_step));
            dec(reg_cnt);
            jnz(label_tov_inner, T_NEAR);
        }
        add(reg_out_ptr, static_cast<int>(out_h_step));
        dec(reg_tov);
        jnz(label_tov, T_NEAR);
    }
    test(reg_khp, reg_khp);
    ...
    L(label_khp); // handle kh padding (not fully unrolled)
    {
        ...
        L(label_khp_inner);
        {
            ...
            add(reg_aux_inp_ptr, static_cast<int>(inp_w_step));
            add(reg_aux_out_ptr, static_cast<int>(out_w_step));
            dec(reg_cnt);
            jnz(label_khp_inner, T_NEAR);
        }
        add(reg_inp_ptr, static_cast<int>(inp_h_step));
        add(reg_out_ptr, static_cast<int>(out_h_step));
        dec(reg_khp);
        jnz(label_khp, T_NEAR);
    }
    ...
    if (is_f32 || is_xf16) {
        ...
        sub(reg_out_ptr, static_cast<int>(out_w_step));
        if (is_f32)
...
void jit_avx512_core_amx_copy_to_pbuffer_t::generate() {
    ...
    if (jcp.is_nspc && jcp.ic_without_padding % jcp.ic_block_int) {
        int tail_size = static_cast<int>(
                jcp.ic_without_padding % jcp.ic_block_int);
    ...
    for (int icb = 0; icb < jcp.nb_ic_int; icb++) {
        ...
        L(kh_tover_label);
        {
            ...
            add(reg_aux_out_ptr, static_cast<int>(out_h_offset));
        ...
        L(kh_label);
        {
            ...
            add(reg_aux_inp_ptr, static_cast<int>(inp_h_offset));
            add(reg_aux_out_ptr, static_cast<int>(out_h_offset));
        ...
        if (is_3d) {
            ...
            pop(reg_aux_out_ptr);
            pop(reg_aux_inp_ptr);
            add(reg_aux_inp_ptr, static_cast<int>(inp_d_offset));
            add(reg_aux_out_ptr, static_cast<int>(out_d_offset));
            dec(reg_kdc);
            jnz(kd_label, T_NEAR);
            L(no_kd_label);
        }
        ...
        add(reg_inp_ptr, static_cast<int>(inp_cb_offset));
        add(reg_out_ptr, static_cast<int>(out_cb_offset));
    }
...
void jit_avx512_core_amx_fwd_kernel_t::store_output_vector_bf16(
    ...
    auto addr = EVEX_compress_addr(reg_out_ptr, static_cast<int>(off));
    ...
    if (jcp.with_bias) {
        ...
        auto bias_addr
                = EVEX_compress_addr(reg_bias, static_cast<int>(bias_offset));
...
void jit_avx512_core_amx_fwd_kernel_t::store_output_vector_int8(
    ...
    if (jcp.src_zero_point) {
        ...
        vpmulld(m_zmm_zp, zmm_src_zp,
                EVEX_compress_addr(
                        reg_zp_compensation, static_cast<int>(zp_offset)));
    ...
    if (jcp.is_relo) {
        ...
        for (dim_t ireduce = 0; ireduce < nreduce; ireduce += stride) {
            ...
            if (ireduce + stride < nreduce) {
                add(reg_inp_ptr, static_cast<int>(stride * jcp.typesize_in));
                add(reg_wei_ptr,
                        static_cast<int>(
        ...
        add(reg_inp_ptr, static_cast<int>(get_inp_shift()));
...
void jit_avx512_core_amx_fwd_kernel_t::tile_configure(char *tcfg_buff) {
    ...
    const int a_col = static_cast<int>(jcp.is_relo ? jcp.ic_block_int
                                                   : jcp.ic_block_int_np
                                                           * jcp.kw_per_tile);
...
status_t jit_avx512_core_amx_fwd_kernel_t::init_conf(jit_conv_conf_t &jcp,
    ...
    // k-remainders
    jcp.kw_per_tile = is_small_ic && !jcp.is_relo
                            && jcp.dilate_w == 0
                            && jcp.stride_w
                                    <= jcp.kw // TODO: relax this restriction
...
void jit_avx512_core_amx_bwd_data_copy_kernel_t::copy_row(
    ...
    { // Handle Top Overflow
        ...
        L(label_tov_loop);
        {
            ...
            add(reg_ptr_aux_out, static_cast<int>(out_h_step));
            dec(reg_tov);
    ...
    L(label_khp_loop);
    {
        ...
        L(label_kwp); // Handle Center Loop
        {
            ...
            L(label_kwp_loop);
            {
                ...
                add(reg_ptr_aux_out, static_cast<int>(out_w_step));
                add(reg_ptr_aux_inp_w, static_cast<int>(inp_w_step));
                dec(reg_cnt_tmp);
        ...
        add(reg_ptr_aux_inp_h, static_cast<int>(inp_h_step));
        dec(reg_cnt_khp);
...
void jit_avx512_core_amx_bwd_data_copy_kernel_t::kd_loop(bool is_masked) {
    ...
    if (is_3d) {
        ...
        pop(reg_ptr_aux_out);
        sub(reg_ptr_aux_inp_h, static_cast<int>(inp_d_offset));
        add(reg_ptr_aux_out, static_cast<int>(out_d_offset));
...
void jit_avx512_core_amx_bwd_data_kernel_t::store_output_vector_xf16(
    ...
    auto addr = EVEX_compress_addr(
            reg_out_ptr, static_cast<int>(get_out_row_offset(h, icb, w)));
...
void jit_avx512_core_amx_bwd_data_kernel_t::store_output(
    ...
    if (do_store) add(reg_out_ptr, static_cast<int>(get_out_shift(width)));
}
...
void jit_avx512_core_amx_bwd_data_kernel_t::interleave_store(dim_t width) {
    for (int c = 0;
        ...
        if (row_count_
            ...
            add(reg_out_ptr, static_cast<int>(get_out_shift(prv_width_)));
...
void jit_avx512_core_amx_bwd_data_kernel_t::skipped_interleave_store() {
    ...
    add(reg_out_ptr, static_cast<int>(get_out_shift(prv_width_save_)));
}
...
void jit_avx512_core_amx_bwd_data_kernel_t::compute_ocb_loop(
    ...
    for (dim_t ocb = 0; ocb < jcp.nb_oc_int; ocb++) {
        ...
        add(reg_inp_ptr, static_cast<int>(get_inp_ocb_step()));
        add(reg_wei_ptr, static_cast<int>(get_wei_ocb_step()));
    }
    sub(reg_inp_ptr, static_cast<int>(get_inp_ocb_step() * jcp.nb_oc_int));
    sub(reg_wei_ptr, static_cast<int>(get_wei_ocb_step() * jcp.nb_oc_int));
}
...
void jit_avx512_core_amx_bwd_data_kernel_t::compute_kd_loop(
    ...
    if (jcp.ndims == 5) {
        ...
        add(reg_inp_ptr, static_cast<int>(get_inp_d_step()));
        add(reg_wei_ptr, static_cast<int>(get_wei_d_step()));
...
status_t jit_avx512_core_amx_bwd_weights_kernel_t::init_conf(
    ...
    switch (jcp.harness) {
        case harness_2d_reduction:
            jcp.nthr_mb_work = jcp.mb * jcp.oh;
            break;
        case harness_3d_reduction:
            jcp.nthr_mb_work = jcp.mb * jcp.od;
            break;
        case harness_compute_full_spatial:
        case harness_mb_reduction:
            jcp.nthr_mb_work = jcp.mb;
            break;
        default:
...
void jit_avx512_core_amx_bwd_weights_kernel_t::balance(const jit_conv_conf_t &j,
    ...
    const int nthr_mb_max = static_cast<int>(
            nstl::min<dim_t>(nthr, j.nthr_mb_work));
    ...
    if (nthr_mb_ > nthr / 2 && nthr_mb_ < nthr)
        nthr_mb_ = static_cast<int>(
                nstl::min<dim_t>(j.nthr_mb_work, nthr));
