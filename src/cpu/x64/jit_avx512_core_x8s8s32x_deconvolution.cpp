...
jit_avx512_core_x8s8s32x_deconv_fwd_kernel_t<Vmm>::
    ...
    if (jcp.with_eltwise || jcp.with_binary || jcp.with_sum) {
        ...
        const binary_injector::rhs_arg_static_params_t rhs_sp {
                static_cast<dim_t>(Xbyak::Xmm(31).getIdx()), this->r14,
...
void jit_avx512_core_x8s8s32x_deconv_fwd_kernel_t<Vmm>::compute_ker(int ur_w,
    ...
    auto kernel_offset = [this, ch_block_all](
                                dim_t ocb, dim_t icb, dim_t ki) {
        return jcp.typesize_in
...
jit_avx512_core_x8s8s32x_deconv_fwd_kernel_t<Vmm>::get_ur_w_blks_params() {
    ...
    for (dim_t blk_idx = 0; blk_idx < n_ur_blocks; blk_idx++) {
        ...
        const dim_t last_blk_src_idx = nstl::min<dim_t>(
                jcp.iw - 1,
                (last_dst_blk_elem + jcp.l_pad) / jcp.stride_w);
...
void jit_avx512_core_x8s8s32x_deconv_fwd_kernel_t<Vmm>::generate() {
    ...
    if (jcp.ngroups % jcp.ch_block != 0 || jcp.oc_without_padding != jcp.oc) {
        const dim_t tail_size = jcp.is_depthwise
                ? jcp.ngroups % jcp.ch_block
                : jcp.oc_without_padding % jcp.oc_block;
        const dim_t mask = (1 << tail_size) - 1;
    ...
    for (dim_t i = 0; i < num_pre_blks; i++) {
        ...
        icb_loop(jcp.ur_w, static_cast<int>(blk_l_overflow),
                static_cast<int>(blk_r_overflow),
