...
status_t xe_hp_systolic_t::pd_t::init(impl::engine_t *engine) {
    ...
    // Seed packed ld* strides for the exec path; must precede jit::pd_t::init.
    a_packed_stride_
    ...
    // Select here to snapshot driver_info and gate dispatch. The per-k-block
    // variants built at primitive-init share one strategy, so this selection
    // reproduces their driver_info.
    {
        jit::gen_xe_systolic_desc_t kd;
        VDISPATCH_GEMM_SC(kd.select_kernel(*dev_info_, cfg_.problem,
                                  with_batch(), packed_c(), 1.0f, cfg_.beta,
                                  cfg_.m, cfg_.n, cfg_.k, cfg_.batch(), unroll_m(),
                                  unroll_n(), alt()),
...
status_t xe_hp_systolic_t::init(impl::engine_t *engine) {
    ...
    for (bool copy_b : {false, true}) {
        for (bool clear_sum : {false, true}) {
            ...
            bool trans = !copy_b ? pd()->cfg().trans_a() : pd()->cfg().trans_b();
    ...
    if (get_verbose(verbose_t::debuginfo) >= 2) {
        verbose_printf("info,gpu,gemm,kernel:%dx%d,%dx%dx%d\n",
                pd()->unroll_m(), pd()->unroll_n(), pd()->driver_info().wg[LoopM],
                pd()->driver_info().wg[LoopN], pd()->driver_info().wg[LoopK]);
    }
...
status_t xe_hp_systolic_t::init_compute(impl::engine_t *engine) {
    ...
    auto select = [&](kd_t &kd, const GEMMProblem &p, float beta) {
        return kd.select_kernel(dev_info, p, pd()->with_batch(),
                pd()->packed_c(), 1.0f, beta, cfg.m, cfg.n, cfg.k,
                cfg.batch(), pd()->unroll_m(), pd()->unroll_n(), pd()->alt());
    };
