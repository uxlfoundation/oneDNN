...
static status_t init_exec_args(jit::exec_args_t &exec_args,
    ...
    if (cfg.with_c_zero_points()) {
        exec_args.off_co0 = types::bytes_to_elements(cfg.c_type(), co->offset());
    ...
    if (pd->a_host_scale() || pd->b_host_scale() || pd->c_host_scale_to_alpha()) {
        exec_args.alpha = 1.0f;
        float scale_val = 0;
        if (pd->a_host_scale()) {
            CHECK(maybe_get_host_scalar_value(
                    GEMM_CTX_ARG_STORAGE(a_scales), scale_val));
            exec_args.alpha *= scale_val;
        }
        if (pd->b_host_scale()) {
            CHECK(maybe_get_host_scalar_value(
                    GEMM_CTX_ARG_STORAGE(b_scales), scale_val));
            exec_args.alpha *= scale_val;
        }
        // Limited support of host scalar dst scales.
        if (pd->c_host_scale_to_alpha()) {
            CHECK(maybe_get_host_scalar_value(
...
status_t gen_t::execute(const exec_ctx_t &ctx) const {
    ...
    CHECK(init_exec_args(exec_args, pd(), ctx));
    const auto &cfg = pd()->cfg();
    ...
    if (k_parallel_fixed) {
        ...
        if (k_parallel_global && !nocopy_info()->fusedBeta() && beta != 1.0f
            ...
            status = launch_nocopy(ctx, compute_stream, zero_pool, exec_args,
                    nullptr, po_count, po_srcs, exec_args.off_a0,
                    exec_args.off_b0, exec_args.off_c0, off_aq0, off_bq0,
                    exec_args.off_co0, po_offsets0, m, n, 0, 1, 1.0f, beta,
                    false, true);
