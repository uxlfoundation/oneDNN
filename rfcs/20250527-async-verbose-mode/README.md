...
The goal of this proposal is to introduce an *asynchronous verbose mode* that enables non-blocking, accurate tracking of kernel executions with minimal synchronization latencies. The new mechanism will be implemented as an experimental functionality using the following apporach: 
...
- Fall back to the default synchronous mode if the profiling data cannot be retrieved asynchronously.

## Proposal
This proposal implements the above mentioned approach for an asynchronous, event-driven verbose profiling. DEscribed below are the main elements of this design: the data structures to hold timing information, the process for registering trackers and callbacks and the behavior of the asynhronous callback function.
...
```cpp
    ...
    dnnl::impl::status_t check_async_exec_times();
```
...
The implementation will be added as an experimental functionality enabled during build-time with `DDNNL_EXPERIMENTAL_ASYNC_VERBOSE`:
```bash
cmake .. -DDNNL_EXPERIMENTAL=ON -DDNNL_EXPERIMENTAL_ASYNC_VERBOSE=ON -DDNNL_EXPERIMENTAL_PROFILING=ON -DDNNL_GPU_RUNTIME=OCL 
```

A PoC implementation is available for the OpenCL GPU backend, which demonstrates the use of `clSetEventCallback` for non-blocking event-driven timing and tracking.

**Reference**: [[link](https://github.com/uxlfoundation/oneDNN/pull/3055)]

## Scope for Future Coverage
- **SYCL and Other Backends**: Provide analogous implementations for SYCL and other accelerators.
