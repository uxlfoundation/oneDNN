...
cl_int maybe_print_debug_info(
    ...
    if (log_length > 1 && (is_err || is_warn)) {
        ...
        err = xpu::ocl::clGetProgramBuildInfo(program, dev,
                CL_PROGRAM_BUILD_LOG, log_length, log_buf.data(), nullptr);
        gpu_assert(err == CL_SUCCESS);
        if (is_err)
            VERROR(common, ocl,
...
status_t engine_t::build_program_from_source(
    ...
    // The `cl_cache` requires using `xpu::ocl::clBuildProgram`. Unfortunately, unlike
    // `clCompileProgram` `xpu::ocl::clBuildProgram` doesn't take headers. Because of
