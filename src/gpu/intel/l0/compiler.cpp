...
status_t ocloc_get_extensions(std::vector<std::string> &extensions) {
    ...
    CHECK(ocloc_invoke(args.size(), args.data(), 0, nullptr, 0, nullptr, 0,
            nullptr, nullptr, nullptr, &num_outputs, &data_outputs,
            &len_outputs, &name_outputs));

    extensions.resize(num_outputs);
    for (uint32_t i = 0; i < num_outputs; i++) {
        if (!strcmp(name_outputs[i], "stdout.log")) {
            if (len_outputs[i] > 0) {
                extensions[i] = std::string(
...
bool ocloc_mayiuse_microkernels(const std::string &kernel_code) {
    ...
    if (ocloc_invoke(args.size(), args.data(), 1, data_sources, len_sources,
                name_sources, 0, nullptr, nullptr, nullptr, &num_outputs,
...
status_t ocloc_build_kernels(const std::string &kernel_code,
    ...
    status_t ret = ocloc_invoke(args.size(), args.data(), 1, data_sources,
            len_sources, name_sources, 0, nullptr, nullptr, nullptr,
    ...
    for (uint32_t i = 0; i < num_outputs; i++) {
        if (!strcmp(name_outputs[i], "main.bin")) {
            if (len_outputs[i] > 0) {
                ...
                binary.insert(binary.begin(), data_outputs[i],
                        data_outputs[i] + len_outputs[i]);
                break;
            }
        }
    }
