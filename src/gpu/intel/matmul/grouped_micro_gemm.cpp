...
status_t grouped_micro_gemm_t::execute(const exec_ctx_t &ctx) const {
    ...
    float po_alpha = 0.0f;
    float po_scale = 1.0f;
    const memory_storage_t *binary_scale = &memory_storage_t::empty_storage();
    const memory_storage_t *binary_offset = &memory_storage_t::empty_storage();
    if (pd()->with_post_op_) {
        for (int i = 0; i < pd()->attr()->post_ops_.len(); ++i) {
            ...
            if (e.is_eltwise()) {
                ...
                binary_scale = &CTX_IN_STORAGE(po_arg, 0);
                const memory_desc_wrapper bin_desc(e.binary.src1_desc);
                if (bin_desc.is_grouped_desc()) {
                    binary_offset = &CTX_IN_STORAGE(po_arg, 1);
                }
            }
        }
    }
    ...
    arg_list.append(*binary_scale);
    arg_list.append(*binary_offset);
