...
inline int threads_for_kernel_execute(double work, int max_threads) {
    constexpr double min_kernel_work_per_thread = 16 * 1024;
...
inline void parallel_pretranspose_B_array(kai::ops::IGemmCommon &kernel,
    ...
    // Match ACL's OpenMP scheduler behavior: the amount of work may be smaller
    // than the configured team, but the team size stays stable across pack/GEMM.
...
inline std::unique_ptr<kai::ops::IGemmCommon> create_kai_gemm(
        const kai::ops::GemmArgs &base_args, data_type_t src_dt,
        data_type_t weights_dt, data_type_t dst_dt, int max_threads = 0) {
...
struct thread_partition_t {
    ...
    // nthr is the active KAI worker count; team_nthr is the OpenMP team size
    // requested from oneDNN. Keep team_nthr at max-or-1 to avoid libgomp
    // overhead from spawning different team sizes across primitives.
    int nthr = 1;
    int team_nthr = 1;
...
inline thread_partition_t make_thread_partition(
    ...
    partition.team_nthr = partition.nthr > 1 ? requested_nthr : 1;
...
inline void parallel_execute(kai::ops::IGemmCommon &kernel,
    ...
    parallel(partition.team_nthr, [&](int ithr, int) {
        if (ithr >= partition.nthr) return;
        ...
        if (partition.nthr_dim1 > 1) {
            balance2D(partition.nthr, ithr, window_size.get_size(0), dim0_start,
                    dim0_end, window_size.get_size(1), dim1_start, dim1_end,
                    partition.nthr_dim1);
            thread_dim0 = ithr % partition.nthr_dim0;
            thread_dim1 = ithr / partition.nthr_dim0;
        } else {
            balance211(window_size.get_size(0), partition.nthr, ithr,
                    dim0_start, dim0_end);
        }
