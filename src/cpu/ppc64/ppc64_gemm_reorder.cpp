...
status_t ppc64_matrixA_reorder_t::pd_t::init(
    ...
    const bool is_row_major = (
            (in_strides[0] == out_strides[0]) && (in_strides[1] == out_strides[1])
            && (out_strides[1] ==  1)) ? true: false;
...
void kernel(InputType *inp, OutputType *out, int N, const float SrcScale,
        const float DstScale, const int SrcZeroPoint,
        const int DstZeroPoint, const float beta){
