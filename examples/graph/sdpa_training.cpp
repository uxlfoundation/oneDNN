...
void bench_sdpa(engine::kind ekind, logical_tensor::data_type dt,
    ...
    auto bmm_dvalue = op(id++, op::kind::MatMul, "bmm_dv");
    bmm_dvalue.set_attr<bool>(op::attr::transpose_a, true);
    bmm_dvalue.add_inputs({probs_b, doutput});
    bmm_dvalue.add_outputs({dvalue});
    ...
    auto bmm_dquery = op(id++, op::kind::MatMul, "bmm_dq");
    bmm_dquery.add_inputs({dscaled_score, key_b});
    bmm_dquery.add_outputs({dquery});

    // compute dkey = dscaled_score^T * query
    auto dkey = logical_tensor(id++, dt, k_sz, layout_type::strided);
    auto bmm_dkey = op(id++, op::kind::MatMul, "bmm_dk");
    bmm_dkey.set_attr<bool>(op::attr::transpose_a, true);
    bmm_dkey.add_inputs({dscaled_score, query_b});
    bmm_dkey.add_outputs({dkey});
