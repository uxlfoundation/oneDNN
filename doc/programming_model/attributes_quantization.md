...
- during primitive descriptor creation, if one or multiple
  inputs/outputs use low precision datatype (e.g. `s8` or `fp8_e4m3`),
  then the primitive will behave as a quantized operation.
- still during primitive descriptor creation, the dimensionality
  (using masks), group size and data type of the scaling factors and
  zero-point should be provided using primitive attributes.
...
The only supported formula for static quantization currently supported
by oneDNN is with scales applied after zero-point as follow:
...
In this model, oneDNN assumes that quantization parameters are
inputs provided by the user the library does not compute those scaling
...
oneDNN is with scales computed following the [Open Compute Project
Microscaling specification version
...
*scaling factor*:
- in e8m0 format,
- computed for each group of size 32 (see set_scales group),
- and computed as the largest power-of-two less than or equal to the
  maximum abolute value of the group divided by the largest
  power-of-two representable in the \f$x_{quant}\f$ data type
  (e.g. \f$E8M0(amax(x_quant[:])) / E8M0(MAX_X_QUANT_DT) \f$).
...
happens according to [rounding mode attribute](@ref
dev_guide_attributes_rounding_mode).
