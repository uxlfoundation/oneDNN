...
status_t ref_t::execute_ref(const exec_ctx_t &ctx) const {
    ...
    const auto &ag = CTX_IN_STORAGE(DNNL_ARG_ATTR_PLACEHOLDER | DNNL_ARG_SRC);
    ...
    const auto &attr_gs = pd()->attr()->placeholder_;
    int src_gs_mask = attr_gs.get_mask(DNNL_ARG_SRC);
    const auto src_gs_group_k = attr_gs.get_group(DNNL_ARG_SRC, 1);
    const auto src_gs_ngroups_k = K / src_gs_group_k;
    // Identify src_gs dimensions as user may not pass them.
    dims_t src_gs_dims {};
    dims_t src_gs_strides {};
    utils::copy_dims_with_mask(
            src_gs_dims, a_d.dims(), a_d.ndims(), src_gs_mask);
    src_gs_dims[a_d.ndims() - 1] /= src_gs_group_k;

    dim_t last_gs_dim = 0;
    dim_t last_gs_stride = 0;
    for (int d = a_d.ndims() - 1; d >= 0; d--) {
        if (src_gs_dims[d] == 0) continue;
        src_gs_strides[d]
                = last_gs_stride == 0 ? 1 : last_gs_dim * last_gs_stride;
        last_gs_stride = src_gs_strides[d];
        last_gs_dim = src_gs_dims[d];
    }
    const dim_t src_gs_stride_k = src_gs_strides[a_d.ndims() - 1];
    const dim_t src_gs_stride_m = src_gs_strides[a_d.ndims() - 2];
    const dim_t src_gs_stride_b0
            = a_d.ndims() > 2 ? src_gs_strides[a_d.ndims() - 3] : 0;
    const dim_t src_gs_stride_b1
            = a_d.ndims() > 3 ? src_gs_strides[a_d.ndims() - 4] : 0;
    ...
    const auto gs_ngroups_k = src_gs_ngroups_k;
    ...
    arg_list.set(arg_idx++, ag);
    arg_list.set(arg_idx++, src_gs_stride_k);
    arg_list.set(arg_idx++, src_gs_stride_m);
    arg_list.set(arg_idx++, src_gs_stride_b0);
    arg_list.set(arg_idx++, src_gs_stride_b1);
    arg_list.set(arg_idx++, src_gs_group_k);
