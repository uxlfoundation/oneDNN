...
public:
    ...
    void generate_prologue() {
        ...
        for (int i = 0; i < 3; i++) {
            ra_.claim(BaseGeneratorT::getLocalID(i));
            ra_.claim(BaseGeneratorT::getLocalSize(i));
        }

        for (int i = 0; i < kernel_iface_.nargs(); i++) {
            ra_.claim(BaseGeneratorT::getArgument(kernel_iface_.arg_name(i)));
        }
    ...
    void bind_external_vars(
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (local_id) {
                expr_binding.bind(
                        local_id, BaseGeneratorT::getLocalID(i).uw(0));
            }
            auto local_size
                    = alloc_mgr.find_let(ir_builder_t::local_size(i), true);
            if (local_size) {
                expr_binding.bind(
                        local_size, BaseGeneratorT::getLocalSize(i).uw(0));
            }
        }
        ...
        for (int i = 0; i < kernel_iface_.nargs(); i++) {
            ...
            expr_binding.bind(arg_var, BaseGeneratorT::getArgument(name));
        }
    ...
    void generate_epilogue() {
        BaseGeneratorT::epilogue(r0_info);
        pad_kernel();
    }
