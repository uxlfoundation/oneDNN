...
enum cpu_isa_t : unsigned {
    ...
    avx10_2_256 = avx10_2 | xmm_bit | ymm_bit,
...
struct avx10_t {
    typedef Wmm Vmm;
    static constexpr int vlen_shift = vreg_traits<Wmm>::vlen_shift;
    static constexpr int n_vregs = 32;
    static constexpr int vlen = vreg_traits<Wmm>::vlen;
};

template <>
struct cpu_isa_traits<avx10_2_512> : public avx10_t<Xbyak::Zmm> {
...
template <>
struct cpu_isa_traits<avx10_2_256> : public avx10_t<Xbyak::Ymm> {
    static constexpr dnnl_cpu_isa_t user_option_val = dnnl_cpu_isa_avx10_2_256;
    static constexpr const char *user_option_env = "avx10_2_256";
};

template <>
struct cpu_isa_traits<avx10_2_512_amx_2> : public avx10_t<Xbyak::Zmm> {
    static constexpr dnnl_cpu_isa_t user_option_val
...
static inline bool mayiuse(const cpu_isa_t cpu_isa, bool soft = false) {
    ...
    switch (cpu_isa) {
        ...
        case avx10_2_256:
            REG_AMX_ISA(return cpu().getAVX10version() >= 2
                    && cpu().has(Cpu::tAVX) && mayiuse(avx2_vnni_2, soft)
                    && cpu().has(Cpu::tAPX_F));
...
static inline bool isa_has_sat_cvt(cpu_isa_t isa, data_type_t dt) {
    return utils::one_of(dt, data_type::u8, data_type::s8)
            && is_superset(isa, avx10_2_512); // Todo: allow avx10_2_256
}
...
#define JIT_IMPL_NAME_HELPER(prefix, isa, suffix_if_any) \
    ...
    (isa) == avx10_2_256 ? prefix STRINGIFY(avx10_2_256) : \
