...
void set_isa_impl(brgemm_desc_t *brg) {
    ...
    if (brg->is_tf32) {
        ...
        if (brg->dt_a == data_type::f32 && brg->dt_b == data_type::bf16) {
            ...
            brg->isa_impl = utils::map(true, isa_undef,
                    is_isa_ok(avx10_512_amx10), avx10_512_amx10,
        ...
        brg->isa_impl = utils::map(true, isa_undef,
                ...
                is_isa_ok(avx512_core_amx_fp16), avx512_core_amx_fp16,
                is_isa_ok(avx512_core_amx), avx512_core_amx,
                is_isa_ok(avx10_2_512), avx10_2_512, is_isa_ok(avx10_512_amx10),
                avx10_512_amx10, is_isa_ok(avx512_core_fp16), avx512_core_fp16,
...
static void find_bdb_bd_mask(
        const brgemm_desc_t *brg, int bd_block, int &bdb, int &bdb_tail) {
...
status_t brgemm_blocking_tmm(brgemm_desc_t *brg) {
    ...
    auto recalc_blocking_ext
            = [&](int new_bd_block, int new_ld_block, int new_bd_block2,
                  ...
                  recalc_blocking(brg, new_bd_block, new_ld_block,
                          new_bd_block2, new_ld_block2);
...
status_t brgemm_blocking_amx10(brgemm_desc_t *brg) {
    ...
    for (auto bd_block2 = 1; bd_block2 <= nstl::min(ntiles, brg->bdb);
            bd_block2++) {
        const auto ld_block2
                = nstl::min(nstl::max(1, brg->ldb), ntiles / bd_block2);

        // Calculate the number of loads for one iteration by reduce_dim
        const auto loads_number
                = static_cast<dim_t>(brg->ldb) * div_up(brg->bdb, bd_block2)
                + brg->bdb * div_up(brg->ldb, ld_block2);
    ...
    // TODO: maybe calculate rd_block more precisely
    ...
    // otherwise load several blocks of B and keep them in registers
...
status_t brgemm_blocking(brgemm_desc_t *brg) {
    ...
    if (brg->is_amx10())
        CHECK(brgemm_blocking_amx10(brg));
    else if (brg->is_tmm)
