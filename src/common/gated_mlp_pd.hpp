...
protected:
    ...
    bool set_default_formats() {
        ...
        for (auto idx : all_idxs())
            ok = ok && set_default_format(arg_md(idx));
        auto status = attr_.post_ops_.set_default_formats(arg_md(DNNL_ARG_DST));
...
private:
    ...
    static std::vector<int> all_idxs() {
        return {DNNL_ARG_SRC, DNNL_ARG_WEIGHTS_GATE, DNNL_ARG_WEIGHTS_UP,
                DNNL_ARG_WEIGHTS_DOWN, DNNL_ARG_DST};
    }
