...
void Generator<hw>::gemmOffsetBatchABC(const GEMMProblem &problem, const GEMMStrategy &strategy, GEMMState &state)
{
    ...
    if (problem.batch == BatchMode::Strided) {
        ...
        if(problem.postOps.cMXScale && state.offsetCs.isInvalid()){
            state.offsetCs = state.ra.alloc_sub(state.offsetCs.getType());
...
#undef CREATE_LAYOUT
    ...
    if (problem.postOps.cMXScale){
        ...
        state.C_scaleLayout = RegisterLayout::tryCreate(hw, Type::f8_e8m0,(unrollM / problem.cqGroupM), (unrollN / problem.cqGroupN),  Cs, Cs_strategy, false, false, true);
    }
    ...
    if (problem.postOps.cMXScale) {
        ...
        setupQAddr(Type::u8, state.C_scaleAddrs, state.C_scaleLayout, state.inputs.cScalePtr,
               i0qs, j0q, state.inputs.ldcScale, state.offsetCs);
    }
