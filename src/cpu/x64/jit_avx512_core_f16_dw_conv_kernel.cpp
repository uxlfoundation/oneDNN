...
void jit_avx512_dw_conv_fwd_kernel_f16::load_src(
    ...
    const int vlen = cpu_isa_traits<avx512_core>::vlen / sizeof(float);
    const int c_tail = jcp.oc % jcp.ch_block;

    for (int ch = 0; ch < ur_ch_blocks; ch++) {
        const bool is_tail_load = check_if_tail_load(
                is_ch_tail, c_tail, ch, ur_ch_blocks, vlen, 0);
        if ((ch + 1 == ur_ch_blocks) && is_ch_tail && c_tail <= 0) continue;
        for (int ow = 0; ow < ur_w; ow++) {
            Zmm vmm_acc = get_acc_reg(ch * ur_w + ow);
            const Zmm vmm_acc_msk
                    = is_tail_load ? vmm_acc | k_oc_tail_mask | T_z : vmm_acc;

            if (this->jcp.with_bias) {
                const int b_off = ch * ch_blk;
                uni_vmovups(vmm_acc_msk, ptr[reg_bias + b_off * sizeof(float)]);
            } else {
            ...
            if (this->jcp.with_sum) {
                ...
                add_from_mem(vmm_acc_msk, vmm_tmp, reg_output,
                        o_off * jcp.typesize_in);
            }
        }
    }
}
        ...
        for (int ch = 0; ch < ur_ch_blocks; ch++) {
            ...
            if (jcp.is_resrc_depthwise) {
                ...
                for (int ii = ii_start; ii <= ii_end; ii++) {
                    Zmm vmm_src = get_src_reg(ii);
                    const Zmm vmm_src_msk = is_tail_load
                            ? vmm_src | k_oc_tail_mask | T_z
                            : vmm_src;
                    const int inp_off = get_input_offset(ii, ch, 0);
                    load(vmm_src_msk, aux_reg_input, inp_off);
                }
            }
            for (int kw = 0; kw < jcp.kw; kw++) {
                const int ker_off = ch * jcp.kh * jcp.kw * ch_blk + kw * ch_blk;

                Zmm vmm_ker = get_ker_reg(0);
                const Zmm vmm_ker_msk = is_tail_load
                        ? vmm_ker | k_oc_tail_mask | T_z
                        : vmm_ker;
                load(vmm_ker_msk, aux_reg_kernel, ker_off * jcp.typesize_in);
                ...
                for (int ow = ow_start; ow < ow_end; ow++) {
                    ...
                    Zmm vmm_src = jcp.is_resrc_depthwise ? get_src_reg(ii)
                                                         : get_src_reg(0);
                    const Zmm vmm_src_msk = is_tail_load
                            ? vmm_src | k_oc_tail_mask | T_z
                            : vmm_src;
                    ...
                    Zmm vmm_acc = get_acc_reg(ch * ur_w + ow);
                    uni_vfmadd231ps(vmm_acc, vmm_src, vmm_ker_msk);
                }
            }
        }
...
void jit_avx512_dw_conv_fwd_kernel_f16::load(
        const Zmm &vmm, const Xbyak::Reg64 &reg, int64_t offset) {
    uni_vcvtph2psx(vmm, ptr[reg + offset]);
}

void jit_avx512_dw_conv_fwd_kernel_f16::add_from_mem(const Zmm &vmm_acc,
        Zmm &vmm_tmp, const Xbyak::Reg64 &reg, int64_t offset) {
    uni_vcvtph2psx(vmm_tmp, ptr[reg + offset]);
    uni_vaddps(vmm_acc, vmm_acc, vmm_tmp);
}

void jit_avx512_dw_conv_fwd_kernel_f16::store(
        const Zmm &vmm, const Xbyak::Address &dst_addr) {
    const auto &cvt_lower_vmm =
            typename vreg_traits<Zmm>::Vmm_lower_t(vmm.getIdx());
    uni_vcvtps2phx(cvt_lower_vmm, vmm);
    uni_vmovdqu16(dst_addr, cvt_lower_vmm);
}
...
void jit_avx512_dw_conv_fwd_kernel_f16::store_dst(
    ...
    for (int ch = 0; ch < ur_ch_blocks; ch++) {
        ...
        for (int ow = 0; ow < ur_w; ow++) {
            const int o_off = ch * ocb_stride + ow * ow_stride;
            Zmm vmm_dst = get_acc_reg(ch * ur_w + ow);
            const auto dst_addr = is_tail_load
                    ? ptr[reg_output + o_off * jcp.typesize_out]
                            | k_oc_tail_mask
                    : ptr[reg_output + o_off * jcp.typesize_out];
            if (jcp.dst_dt == data_type::f16)
                store(vmm_dst, dst_addr);
            else
                vmovups(dst_addr, vmm_dst);
        }
    }
}
