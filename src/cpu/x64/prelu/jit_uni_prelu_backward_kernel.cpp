...
Xbyak::Address jit_prelu_backward_kernel_t::data_ptr(int arg_num, size_t offt) {
    const auto get_addr
        ...
        return ptr[reg_base + reg_offset_ * static_cast<int>(dt_size)
                + static_cast<int>(offt) * static_cast<int>(dt_size)];
    };
...
void jit_uni_prelu_backward_kernel_t<Xbyak::Zmm>::compute_dst(
    ...
    auto get_next_opmask = [opmask_counter]() mutable {
        ...
        const auto opmask = Xbyak::Opmask(static_cast<int>(opmask_counter++));
