...
using pooling_key_t = decltype(memory_tracking::names::key_pool_reduction);

const std::vector<pooling_key_t> pooling_keys
...
status_t acl_pooling_fwd_t::pd_t::init_scratchpad(
    ...
    if (aux_mem_req.empty()) return status::success;

    const size_t n = std::min(aux_mem_req.size(), pooling_keys.size());

    for (size_t id = 0; id < n; ++id) {
        const auto &req = aux_mem_req[id];
        if (req.size == 0) continue;

        size_t align = req.alignment;
        if (align == 0) align = 64;
        if (align & (align - 1)) { align = utils::rnd_up_pow2(align); }

        scratchpad.book(pooling_keys[id], req.size, 1, align, align);
    }
...
status_t acl_pooling_fwd_t::execute_forward(const exec_ctx_t &ctx) const {
    ...
    void *scratchpad_base
            = ctx.get_scratchpad_grantor().get<void>(pooling_keys[0]);
