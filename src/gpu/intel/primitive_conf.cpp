...
bool post_ops_with_binary_ok(const primitive_attr_t *attr,
    ...
    for (int po_idx = 0; po_idx < p.len(); ++po_idx) {
        ...
        if (is_prelu(po_idx)) {
            auto mask = p.entry_[po_idx].prelu.mask;
            if (mask > prelu_mask_supported) is_po_ok = false;
            bool has_runtime_dims = false;
            int num_size_one_dims = 0;
            for (int dim_idx = 0; dim_idx < dst_md.ndims; dim_idx++) {
                if (((mask >> dim_idx) & 1)
                        && dst_md.dims[dim_idx] == DNNL_RUNTIME_DIM_VAL)
                    has_runtime_dims = true;
                else if (((mask >> dim_idx) & 1) == 0
                        || dst_md.dims[dim_idx] == 1) {
                    num_size_one_dims++;
                }
            }
