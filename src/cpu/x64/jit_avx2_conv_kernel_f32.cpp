        ...
        auto compute = [&](int cur_ic_blk) {
            for (int ifm2 = 0; ifm2 < cur_ic_blk; ifm2++) {
                for (int jj = jj_start; jj < jj_end; jj++) {
                    size_t inp_off = get_input_offset(ifm2,
                            static_cast<int>(filter_w_to_input(ki, jj, pad_l)));
...
void jit_avx2_conv_fwd_kernel_f32_t::oh_step_nopad(
    ...
    L(kw_loop);
    {
        ...
        for (int ifm2 = 0; ifm2 < ic_blk; ifm2++) {
            for (int jj = jj_start; jj < jj_end; jj++) {
                size_t inp_off = get_input_offset(ifm2,
                        static_cast<int>(filter_w_to_input(0, jj, pad_l)));
                vbroadcastss(Ymm(oc_blocks * ur_w + jj),
        ...
        safe_add(aux_reg_input,
                get_input_offset(0, static_cast<int>(filter_w_to_input(1))),
                reg_long_offt);
...
void jit_avx2_conv_fwd_kernel_f32_t::width_blk_step(
        ...
        if ((jcp.ic % jcp.ic_block == 0) && jcp.kw >= 5 && pad_l == 0
            ...
            add(aux_reg_input,
                    static_cast<uint32_t>(
                            get_input_offset(
                                    0, static_cast<int>(filter_h_to_input(1)))
                            - get_input_offset(0,
                                    static_cast<int>(filter_w_to_input(kw)))));
        } else {
            oh_step_unroll_kw(ur_w, pad_l, pad_r, oc_blocks);
            safe_add(
                    aux_reg_kernel, get_kernel_offset(0, kw, 0), reg_long_offt);
            safe_add(aux_reg_input,
                    get_input_offset(0, static_cast<int>(filter_h_to_input(1))),
                    reg_long_offt);
        }
    ...
    if (jcp.ndims == 5) {
        safe_add(aux_reg_inp_d,
                get_input_offset(0, static_cast<int>(filter_d_to_input(1))),
                reg_long_offt);
...
inline void jit_avx2_conv_fwd_kernel_f32_t::solve_common(int oc_blocks) {
    ...
    if (l_pad > 0) {
        ...
        add(reg_input,
                static_cast<uint32_t>(get_input_offset(0,
                        static_cast<int>(filter_w_to_input(0, ur_w, l_pad)))));
        add(reg_output, static_cast<uint32_t>(get_output_offset(0, ur_w)));
    }
    ...
    if (n_oi > 0) {
        ...
        add(reg_input,
                static_cast<uint32_t>(get_input_offset(
                        0, static_cast<int>(filter_w_to_input(0, ur_w)))));
        add(reg_output, static_cast<uint32_t>(get_output_offset(0, ur_w)));
...
void jit_avx2_conv_bwd_data_kernel_f32_t::compute_loop(
    ...
    L(kh_loop);
    {
                ...
                for (int ofm2 = 0; ofm2 < cur_oc_blk; ofm2++) {
                    for (int jj = jj_start; jj < jj_end; jj += stride_w) {
                        int aux_output_offset
                                = static_cast<int>(get_ddst_offset(0,
                                        static_cast<int>(filter_w_to_ddst(
                                                ki, jj, jcp.l_pad)),
                                        ofm2));
        ...
        sub(aux_reg_ddst,
                static_cast<uint32_t>(
                        get_ddst_offset(0, (jcp.dilate_h + 1) * ow, 0)));
    ...
    if (jcp.ndims == 5) {
        sub(aux_reg_dst_d,
                static_cast<uint32_t>(get_ddst_offset(
    ...
    if (one_of(jcp.ndims, 3, 4)) {
        int ddst_oc_shift = static_cast<int>(get_ddst_offset(1, 0, 0));
        int kernel_oc_shift = static_cast<int>(get_kernel_offset(1, 0, 0, 0));
...
void jit_avx2_conv_bwd_data_kernel_f32_t::generate() {
    ...
    int ddst_shift = static_cast<int>(get_ddst_offset(
            0, static_cast<int>(filter_w_to_ddst(0, jcp.ur_w)), 0));
    int dsrc_shift = static_cast<int>(get_dsrc_offset(0, jcp.ur_w));
...
jit_avx2_conv_bwd_weights_kernel_f32_t::od_step_comeback_pointers() {
    ...
    {
        sub(aux_reg_input,
                static_cast<uint32_t>(get_input_offset(0, jcp.iw * jcp.ih)));
        sub(aux_reg_kernel,
                static_cast<uint32_t>(get_kernel_offset(jcp.kw * jcp.kh, 0)));
        dec(kj);
        cmp(kj, 0);
...
jit_avx2_conv_bwd_weights_kernel_f32_t::oh_step_comeback_pointers() {
    ...
    L(kh_comeback_loop);
    {
        sub(reg_input, static_cast<uint32_t>(get_input_offset(0, jcp.iw)));
        sub(reg_kernel, static_cast<uint32_t>(get_kernel_offset(jcp.kw, 0)));
        dec(kj);
        cmp(kj, 0);
...
inline void jit_avx2_conv_bwd_weights_kernel_f32_t::compute_oh_step_unroll_ow(
    ...
    L(kh_loop);
    {
        ...
        L(ic_block_loop);
        {
            ...
            add(reg_kernel,
                    static_cast<uint32_t>(get_kernel_offset(0, ic_block_step)));
            add(b_ic, ic_block_step);
            cmp(b_ic, ic_block);
            jl(ic_block_loop, T_NEAR);
        }
        add(reg_input,
                static_cast<uint32_t>(get_input_offset(0, jcp.iw)
                        - get_input_offset(ic_block, 0)));
        add(reg_kernel,
                static_cast<uint32_t>(get_kernel_offset((jcp.kw - 1), 0)));
        dec(kj);
        cmp(kj, 0);
        jg(kh_loop, T_NEAR);
    }
    ...
    L(kh_loop_ic_tail);
    {
        ...
        if (ic_block_step_tail) {
            ...
            add(reg_input,
                    static_cast<uint32_t>(
                            get_input_offset(ic_block_step_tail, 0)));
            add(reg_kernel,
                    static_cast<uint32_t>(
                            get_kernel_offset(0, ic_block_step_tail)));
        }
        ...
        add(reg_kernel,
                static_cast<uint32_t>(get_kernel_offset(0, ic_block - ic_tail)
    ...
    if (jcp.ndims == 5) {
        add(aux_reg_input,
                static_cast<uint32_t>(get_input_offset(0, jcp.ih * jcp.iw)));
        add(aux_reg_kernel,
                static_cast<uint32_t>(get_kernel_offset(jcp.kh * jcp.kw, 0)));
        dec(ki);
        cmp(ki, 0);
        jg(kd_loop, T_NEAR);
    }
...
inline void jit_avx2_conv_bwd_weights_kernel_f32_t::compute_oh_step_common(
    ...
    int input_comeback = static_cast<int>(
            get_input_offset(0, ur_w_trips * ur_w * stride_w - jcp.l_pad));
    int output_comeback
            = static_cast<int>(get_output_offset(0, ur_w_trips * ur_w));
    ...
    L(kh_loop);
    {
        ...
        L(ic_block_loop);
        {
            if (jcp.l_pad != 0) {
                ...
                add(reg_output,
                        static_cast<uint32_t>(get_output_offset(0, ur_w)));
            }
...
inline void jit_avx2_conv_bwd_weights_kernel_f32_t::compute_oh_loop_common() {
    ...
    if (t_pad > 0) {
        ...
        add(reg_kernel,
                static_cast<uint32_t>(get_kernel_offset(t_pad * jcp.kw, 0)));

        L(oh_tpad_loop);
        {
            compute_oh_step_disp();
            add(reg_output,
                    static_cast<uint32_t>(get_output_offset(0, jcp.ow)));
            sub(reg_kernel,
                    static_cast<uint32_t>(
    ...
    L(oh_loop);
    {
        compute_oh_step_disp();
        add(reg_input,
                static_cast<uint32_t>(get_input_offset(0, stride_h * jcp.iw)));
        add(reg_output, static_cast<uint32_t>(get_output_offset(0, jcp.ow)));
    ...
    if (b_pad > 0) {
        ...
        L(oh_bpad_loop);
        {
            ...
            add(reg_input,
                    static_cast<uint32_t>(
                            get_input_offset(0, stride_h * jcp.iw)));
            add(reg_output,
                    static_cast<uint32_t>(get_output_offset(0, jcp.ow)));
