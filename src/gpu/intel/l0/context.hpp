...
struct event_t : public xpu::event_t {
    ...
    event_t(const std::vector<ze_event_handle_t> &event) : events(event) {}
    ...
    const ze_event_handle_t &operator[](size_t i) const { return events[i]; }
    ze_event_handle_t &operator[](size_t i) { return events[i]; }
