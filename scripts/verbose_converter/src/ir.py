...
class MemoryDescriptor(Mapping):
    ...
    def _format(self, tag: str, convert) -> str:
        ...
        if self.is_grouped:
            fields = [header, self.properties, self.format_kind, "grouped"]
            # Old-format grouped MDs omit var_dim_idx/group_count.
            if self.variable_dim_idx is not None:
                fields += [str(self.variable_dim_idx), str(self.group_count)]
            fields += ["", convert(self.flags)]
            return ":".join(fields)
        return ":".join(
            [
                header,
