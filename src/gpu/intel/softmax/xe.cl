...
xe_softmax_fwd(__global SRC_DATA_T *src, __global DST_DATA_T *dst,
    ...
    for (int i = 0; i < num_buf; i++) {
        ...
        dim_t data_off = (dim_t)mb * CHANNELS_PADDED * SOFTMAX_AXIS_SIZE
    ...
    dim_t data_off = (dim_t)mb * CHANNELS * SOFTMAX_AXIS_SIZE + channel_offset
            + channel_in_block;
    ...
    const dim_t data_off
            = (dim_t)(get_global_id(0) / GROUP_SIZE) * SOFTMAX_AXIS_SIZE;
...
xe_softmax_bwd(__global DST_DATA_T *dst, __global SRC_DATA_T *diff_src,
    ...
    dim_t data_off = (dim_t)BATCH * IC * SOFTMAX_AXIS_SIZE * ic_blk
            + (dim_t)batch * IC * SOFTMAX_AXIS_SIZE + ic_buff * sub_grp_id
            + ic_in_blk;
#else
    dim_t data_off = (dim_t)batch * IC * SOFTMAX_AXIS_SIZE
            + ic_buff * sub_grp_id + ic;
#endif
