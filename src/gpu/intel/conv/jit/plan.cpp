...
private:
    ...
    plan_status_t try_init_plan() {
        ...
        auto k_blk_dpas_aligned = plan_.fma.m_blk == 1
                || // plan_.fma.k_blk == k_tile;

                (plan_.fma.k_blk * plan_.fma.a_layout.type().size()
                                % plan_.grf_size()
                        == 0);
 ...
 auto is_dpas = utils::one_of(
         plan_.fma.fma_kind, fma_kind_t::dpas, fma_kind_t::dpasw);
 auto k_blk_dpas_aligned = plan_.fma.m_blk  == 1 ||( plan_.fma.k_blk * plan_.fma.a_layout.type().size()
                         % plan_.grf_size()
                 == 0);
 if (cfg_.hw() >= ngen::HW::XE3P_35_10 && is_dpas
         && plan_.fma.m_blk % 2 != 0 && !k_blk_dpas_aligned)
