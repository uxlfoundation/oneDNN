...
void jit_uni_binary_injector_t<isa, Vmm>::compute_vector_range(
    ...
    for (const auto vmm_idx : vmm_idxs) {
        ...
        if (needs_ternary_input) {
            ...
            load_rhs(rhs2_arg_data_type, tmp_vmm, rhs2_arg_addr, tail_load_mode,
                    with_tail);
            ///////////////////////////////////////////////////////////////////
            // if (with_tail){
            ...
            //     host_->vpmovsxbd(tmp_vmm | tail_opmask | host_->T_z, rhs2_arg_addr);
            // }
            // else {
            //     // host_->vpmovsxbd(tmp_vmm, rhs2_arg_addr);
            //     // host_->movaps(tmp_vmm, rhs2_arg_addr);
            //     host_->uni_vpmovsxbd(tmp_vmm, rhs2_arg_addr);
            // }
            // load_rhs_no_tail(rhs2_arg_data_type, tmp_vmm, rhs2_arg_addr);
            ///////////////////////////////////////////////////////////////////

            if (types::is_integral_dt(rhs2_arg_data_type)) cvt_to_f32(tmp_vmm);
        }
        ...
        if (vmm_preservation_needed || needs_ternary_input) {
            ...
            if (needs_ternary_input)
                inject_binary_with_ternary_op(post_op, dst_vmm, rhs1_arg_addr,
                        tmp_vmm, with_tail, tail_load_mode);
            else
...
Xbyak::Address jit_uni_binary_injector_t<isa, Vmm>::prepare_rhs_arg_addr(
    ...
    switch (rhs_broadcasting_strategy) {
        ...
        case broadcasting_strategy_t::no_broadcast: {
            ...
            // overwriting the address for the binary input as they are loaded
            // consecutively
...
void jit_uni_binary_injector_t<isa, Vmm>::inject_binary_with_ternary_op(
    ...
    if (alg == alg_kind::binary_select) {
        ...
        // src1 tensor ANDed with the complimented sleect input.
        ...
        if (is_superset(isa, avx512_core)) {
            ...
            host_->vcmpps(tmp_vmm, tmp_vmm, dst, jit_generator::_cmp_nle_us);
            pop_vmm(host_, dst);
