    ...
    const auto call_brgemm
        ...
        if (do_postops) {
            ...
            brgemm_kernel_execute_postops(brg_ker,
                    static_cast<int>(n_ic_blocks), brg_batch, (void *)ptr_C,
                    (void *)ptr_D, post_ops_data, scratch);
        } else {
            void *scratch = is_amx ? static_cast<void *>(wsp_tile)
                                   : static_cast<void *>(s8s8_comp_ptr);
            brgemm_kernel_execute(brg_ker, static_cast<int>(n_ic_blocks),
                    brg_batch, (void *)ptr_C, scratch);
        }
    };
