...
// Quant-mask bits for the ungrouped allow-lists below: bit i set => a distinct
// value per index along logical dim i (counted from the outermost), broadcast
// where clear; mask 0 is per-tensor. The gemm dim a bit denotes depends on the
// operand and rank (A=wei[..,K,N], B=src[..,M,K], C=dst[..,M,N]), so these are
// named by dim index, not by M/N/K. Grouped/2D masks use valid_2d_mask().
...
status_t init_kernel_config(
    ...
    auto leading_dim = [](const memory_desc_t &md) -> dnnl_dim_t {
        return md.format_desc.blocking.inner_nblks > 0 ? 0
    ...
    cfg.fpmath_modes = (attr->mayiconvert(f32, tf32) ? gen_nocopy_desc_t::mode_tf32 : 0)
            | (attr->mayiconvert(f32, bf16) ? gen_nocopy_desc_t::mode_bf16x1 : 0)
            | (attr->mayiconvert(f32, f16) ? gen_nocopy_desc_t::mode_f16x1 : 0)
            | (attr->mayiconvert(f32, f32) ? gen_nocopy_desc_t::mode_strict : 0);
    cfg.deterministic = pd->attr()->deterministic_;

    const bool any_host_scale = pd->a_host_scale() || pd->b_host_scale()
            || pd->c_host_scale_to_alpha();
...
static status_t init_attrs(
    ...
    // Host-scalar c-zp folds to -1, matching aoPtrDims/boPtrDims.
...
status_t zp_ok(const pd_t *pd, impl::engine_t *engine) {
    ...
    const auto entry_2d = [](const quant_entry_t &e, const memory_desc_t &base,
                                  int k_idx) {
    ...
    if (!a_zps.has_default_values()) {
        ...
        if (!a_zps.has_default_groups()) {
            ...
            VDISPATCH_GEMM_F(utils::one_of(a_zps.get_mask(), 0, mask_dim1,
                                     mask_dim2),
                    "%s: unsupported A mask", VERBOSE_UNSUPPORTED_ZP_CFG);
    ...
    if (!b_zps.has_default_values()) {
        ...
        if (!b_zps.has_default_groups()) {
            ...
            VDISPATCH_GEMM_F(utils::one_of(b_zps.get_group(0), dim_t(1), d->n()),
...
status_t gs_ok(const pd_t *pd, impl::engine_t *engine) {
    ...
    VDISPATCH_GEMM_F(IMPLICATION(with_a_group_sums_,
                             attr_gs.get_data_type(DNNL_ARG_A) == data_type::s32),
            VERBOSE_UNSUPPORTED_DT_CFG);
    VDISPATCH_GEMM_F(IMPLICATION(with_b_group_sums_,
...
status_t scales_ok(const pd_t *pd, impl::engine_t *engine) {
    ...
    for (auto s : {DNNL_ARG_A, DNNL_ARG_B}) {
        ...
        bool supportedMask = utils::one_of(mask, 0, mask_dim0,
                                     mask_dim1, mask_dim2)
...
status_t finalize_problem(
        kernel_config_t &cfg, const intel::engine_t *engine) {
    ...
    // Strict acc mode default. Must be f32, not Tc_ext: it gates atomic-C
    // updates (useAutoAtomic), and an integer dst would yield s32 and
    // re-enable atomics.
    ...
    if (!problem.forceGroupSumsB) problem.Bg.layout = layout_T;
    ...
    // C must be passthrough once finalized.
...
void apply_swap_ab(kernel_config_t &cfg, bool swap) {
    ...
    if (swap) {
        ...
        // Tag/Tbg are derived post-swap in finalize_problem; no swap needed
        // here.
    ...
    // Cross-check m/n were swapped along with the transpose()-swapped group
    // dims.
    gpu_assert(IMPLICATION(
