...
void brgemm_matmul_t<isa>::copy_b_chunk_in_buffer(
    ...
    // easy to call a full `K_blk` piece, or a portion of it, and apply single
    // scale group over. When doing a portion of B, it requires an additional
...
struct brgemm_matmul_t<isa>::brg_matmul_exec_ctx_t {
    ...
    // `k` is only used when apply wei_decompression with a single scale over
    // a sub-piece of a `K_blk`.
    char *get_buf_B_ptr(
...
private:
    ...
    // TODO: implement kernels to avoid the need in this pointer and the
    // overhead around filling that bigger buffer for the scenario mentioned
    // above.
    void *wei_scales_tr_;
