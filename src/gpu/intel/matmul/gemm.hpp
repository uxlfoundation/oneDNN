    ...
    struct pd_t : public matmul::pd_t {
        ...
        status_t init(impl::engine_t *engine) {
            ...
            auto maybe_reshape = [&]() -> status_t {
                // reduce output is not threaded through the reshape logic.
            ...
            // Translate the matmul K-reduction output into the jit gemm
            // `sum_ab` feature. The wrapper maps matmul src -> gemm a_desc, so
            // reducing the src over K is `sum_a_row` (which reduces a_desc).
            if (with_reduce()) {
