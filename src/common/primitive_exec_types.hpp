...
struct exec_ctx_t {
    ...
    // A main version when only a stream and args are available.
    exec_ctx_t(stream_t *stream, exec_args_t &&args = {});

    // A version used by `nested_scratchpad_t` objects. Underlying
    // implementation copies a master context members except args and grantor.
...
private:
    ...
    // grantor with a `exec_ctx_t` lifetime. The requirement of identical
    // lifetime comes from asynchronous threadpool runtime.
