...
dnnl_status_t init_pd(init_pd_args_t<prb_t> &init_pd_args) {
    ...
    overload_quant_mask(prb->attr.placeholder.get(DNNL_ARG_SRC).policy,
            DNNL_ARG_ATTR_PLACEHOLDER | DNNL_ARG_SRC);
...
int init_ref_memory_args(dnn_mem_map_t &ref_mem_map, dnn_mem_map_t &mem_map,
    ...
    for (auto &entry : mem_map) {
        ...
        switch (exec_arg) {
            ...
            case DNNL_ARG_ATTR_PLACEHOLDER:
    ...
    if (ref_mem_map.count(DNNL_ARG_ATTR_PLACEHOLDER | DNNL_ARG_SRC)) {
        auto &mem = mem_map.at(DNNL_ARG_ATTR_PLACEHOLDER | DNNL_ARG_SRC);
        const auto &ref_mem
                = ref_mem_map.at(DNNL_ARG_ATTR_PLACEHOLDER | DNNL_ARG_SRC);
        const auto &ref_mem_src = ref_mem_map.at(DNNL_ARG_SRC);
        const auto src_placeholder_gs
                = prb->attr.placeholder.get(DNNL_ARG_SRC).groups[1];
        ...
        for (int64_t i = 0; i < ref_mem_src.nelems() / src_placeholder_gs;
                i++) {
            float val = 0;
            for (int64_t k = 0; k < src_placeholder_gs; k++) {
                const auto offset = i * src_placeholder_gs + k;
