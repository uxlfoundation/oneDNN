...
struct jit_softmax_base_t : public jit_generator {
    ...
    std::unique_ptr<jit_uni_eltwise_injector_f32> exp_injector_;
    std::unique_ptr<jit_uni_eltwise_injector_f32> log_injector_;
...
#undef PARAM_LOAD
    ...
    void generate() override {
        if (pd_->is_fwd() || is_logsoftmax_)
            exp_injector_.reset(new jit_uni_eltwise_injector_f32(this,
                    alg_kind::eltwise_exp, 0.0f, 0.0f, 1.0f, true,
                    reg_exp_injector_table, injector_mask, injector_tmp));
        if (pd_->is_fwd() && is_logsoftmax_) {
            log_injector_.reset(new jit_uni_eltwise_injector_f32(this,
