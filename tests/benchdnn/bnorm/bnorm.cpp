...
int prepare_fwd(const prb_t *prb, dnn_mem_map_t &mem_map,
    ...
    if (file_fill_map.at(DNNL_ARG_VARIANCE) != OK)
...
int prepare_bwd(const prb_t *prb, dnn_mem_map_t &mem_map,
        dnn_mem_map_t &ref_mem_map,
        const std::unordered_map<int, int> &file_fill_map) {
    if (file_fill_map.at(DNNL_ARG_DIFF_DST) == OK) return OK;
...
int init_ref_memory_args(dnn_mem_map_t &ref_mem_map, dnn_mem_map_t &mem_map,
    ...
    std::unordered_map<int, int> file_fill_map;
    for (auto &entry : mem_map) {
        ...
        file_fill_map[exec_arg]
                = fill_from_file(exec_arg, mem, ref_mem_map[exec_arg]);
    }

    if (is_fwd_prim) {
        SAFE(prepare_fwd(prb, mem_map, ref_mem_map, file_fill_map, res), WARN);
    } else {
        SAFE(prepare_bwd(prb, mem_map, ref_mem_map, file_fill_map), WARN);
    }
