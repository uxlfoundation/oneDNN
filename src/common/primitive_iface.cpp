...
status_t primitive_create(primitive_iface_t **primitive_iface,
    ...
    if (enable_itt) {
        ...
        __itt_id task_id = __itt_id_make(
                const_cast<char *>(pd_info), static_cast<uint64_t>(get_msec()));
...
status_t primitive_execute(
    ...
    const bool enable_itt = itt::get_itt(itt::__itt_task_level_low);
    if (enable_itt) {
        // ITT task IDs are uniquely assigned using pd->info() and timestamps
        // - the timestamps helps distinguish between different instances of the same configuration
        __itt_id task_id = __itt_id_make(const_cast<char *>(pd->info()),
                static_cast<uint64_t>(get_msec()));
