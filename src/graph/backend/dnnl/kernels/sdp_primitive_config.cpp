...
status_t sdp_primitive_config_t::initial_check(
    ...
    for (const auto &cur_op : sg->get_ops()) {
        ...
        if (post_op && mm1_post_op_kind.count(post_op->get_kind())) {
            ...
            // Running score value, used to tell a select's score branch
            // from its scalar fill.
            ...
            if (post_op) {
                ...
                // CPU primitive applies `cond ? fill : score` (or its inverse)
                // to the scores ahead of the softmax. The GPU ukernel has no
                // such path; select between mm1 and scale is rejected above.
                ...
                if (post_op && post_op->get_kind() == graph::op_kind::Select) {
                    ...
                    const bool then_is_score = post_op->get_input_value(1).get()
                            == score_val.get();
