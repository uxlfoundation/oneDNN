...
status_t brgemm_desc_set_attr(
    ...
    // OTF already copies A for the BRGEMM. No need for partial copy.
    if (brgattr.hint_use_otf
            && (brgattr.extendable_k || brgattr.wary_A_k_tail_read)) {
        return status::unimplemented;
    }

    if (!IMPLICATION(
                brgattr.hint_use_otf, is_superset(brg->isa_impl, amx_tile))) {
        return status::unimplemented;
    }
    ...
    if (brgattr.hint_use_otf) brg->otf = true;
