...
#define TAIL_PARAM(x) ptr[abi_param1 + offsetof(tail_call_param_t, x)]
    ...
    void process_unroll_generic(
        ...
        for (dim_t off = 0; off < len; off += blk) {
            const int reg_unroll = static_cast<int>(
                    nstl::min<dim_t>(off + blk, len) - off);
    ...
    void compute_ker(
            const int ndims, const dim_t len_unroll, const bool tail_processing) {
    ...
    void finalize_tail_loop(
            dim_t i_step, dim_t o_step, dim_t s_step, dim_t c_step,
        ...
        if (prb_.nodes[curr_node_id].is_zero_pad_needed) {
            ...
            const dim_t bytes_to_zeroing = num_of_zero_padded_values * otype_sz_;
...
static void prb_block_for_cache(tr::prb_t &prb) {
    ...
    const dim_t L1_cache_sz = static_cast<dim_t>(
            static_cast<dim_t>(size_t(3)
                    * platform::get_per_core_cache_size(1) / 4));
    ...
    if (prb.ndims >= 2 && prb.nodes[0].os == 1 && prb.nodes[1].is == 1) {
        ...
        if (split_needed) {
            ...
            const int min_idx
                    = static_cast<int>(std::distance(dim_beg_it, min_n_node_it));
...
static void prb_thread_kernel_balance(
    ...
    if (want_borrow_ker_from_drv) {
        ...
        dim_t size_want_borrow
                = utils::div_up(static_cast<dim_t>(tr::ker_prb_size_min),
                        size_ker_cur);
...
void jit_uni_reorder_t::omp_driver_2d(int ithr, int nthr, int off,
    ...
    const tr::prb_t &prb = pd()->prb_;
    const tr::node_t *ns = prb.nodes + off;
    for_nd(ithr, nthr, static_cast<dim_t>(ns[1].n),
            static_cast<dim_t>(ns[0].n), [&](dim_t d1, dim_t d0) {
        tr::call_param_t base_params;
        base_params.in = in
...
void jit_uni_reorder_t::omp_driver_3d(int ithr, int nthr, int off,
    ...
    const tr::prb_t &prb = pd()->prb_;
    const tr::node_t *ns = prb.nodes + off;
    for_nd(ithr, nthr, static_cast<dim_t>(ns[2].n),
...
void jit_uni_reorder_t::omp_driver_4d(int ithr, int nthr, int off,
    ...
    const tr::prb_t &prb = pd()->prb_;
    const tr::node_t *ns = prb.nodes + off;
    for_nd(ithr, nthr, static_cast<dim_t>(ns[3].n),
            static_cast<dim_t>(ns[2].n), static_cast<dim_t>(ns[1].n),
            static_cast<dim_t>(ns[0].n),
