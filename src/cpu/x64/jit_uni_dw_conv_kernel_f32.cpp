...
void jit_uni_dw_conv_fwd_kernel_f32_t<isa>::apply_filter_unrolled(
    ...
    L(kh_label);
    {
        ...
        add(aux_reg_kernel, static_cast<int>(jcp.kw * ch_blk * sizeof(float)));
        if (jcp.is_fused_conv) {
...
void jit_uni_dw_conv_fwd_kernel_f32_t<isa>::compute_loop(
    ...
    if (ch_loop) {
        ...
        const int ch_step = static_cast<int>(
                jcp.nb_ch_blocking * jcp.ch_block);
        ...
        if ((jcp.oc / jcp.ch_block) >= jcp.nb_ch_blocking) {
            ...
            L(ch_loop_label);
            {
                ...
                add(reg_kernel, static_cast<int>(wei_ch_stride));
                add(reg_input, static_cast<int>(inp_ch_stride));
                add(reg_output, static_cast<int>(out_ch_stride));
                if (jcp.with_bias) add(reg_bias, static_cast<int>(bias_stride));
...
void jit_uni_dw_conv_fwd_kernel_f32_t<isa>::ow_loop(int ur_ch_blocks) {
    ...
    if (ow == ur_w) {
        ...
        if (n_oi == 0) {
            ...
            add(reg_input, static_cast<int>(inp_shift_pad));
            add(reg_output, static_cast<int>(out_shift));
            if (ur_w_tail != 0) {
            ...
            if ((l_pad <= 0 && n_oi > 0) || (l_pad > 0 && n_oi > 1)) {
                ...
                L(ow_loop_label);
                {
                    ...
                    add(reg_input, static_cast<int>(inp_shift));
                    add(reg_output, static_cast<int>(out_shift));
...
inline void jit_uni_dw_conv_bwd_data_kernel_f32_t<isa>::apply_filter(
    ...
    L(kh_label);
    {
        ...
        L(kw_label);
        {
            ...
            sub(aux1_reg_ddst, static_cast<int>(sp_step * sizeof(float)));
        ...
        sub(aux_reg_ddst, static_cast<int>(ow * sp_step * sizeof(float)));
    ...
    if (write_ch_loop) {
        ...
        if (nb_oc >= jcp.nb_ch_blocking) {
            ...
            L(ch_loop_label);
            {
                ...
                add(reg_kernel, static_cast<int>(wei_ch_stride));
                add(reg_dsrc, static_cast<int>(data_ch_stride));
                add(reg_ddst, static_cast<int>(data_ch_stride));
...
inline void jit_uni_dw_conv_bwd_data_kernel_f32_t<isa>::unroll_width_body(
    ...
    auto unroll_width_loop = [&](int unroll_w) {
        ...
        L(unroll_w_label);
        {
            ...
            add(reg_dsrc, static_cast<int>(unroll_w * jcp.stride_w * ch_step));
            add(reg_ddst, static_cast<int>(unroll_w * ch_step));
...
void jit_uni_dw_conv_bwd_data_kernel_f32_t<isa>::generate() {
    ...
    if (is_dsrc_layout_nxc()) {
        if (isa == avx512_core && (jcp.ch_tail > 0)) {
            ...
            cmp(reg_ch_blocks, static_cast<int>(channel_step));
