...
void bench_sdpa(engine::kind ekind, logical_tensor::data_type dt,
    ...
    // Stacked qkv tensor shape: [mb, seq_len, 3, head_num, head_size]. This
    // follows the definition of StackedQueryKeyValueTensor in DirectML. The
    // shape of each becomes [mb, seq_len, 1, head_num, head_size]. The strides
    // of each: [seq_len x head_num x head_dim x 3, head_dim, head_num x head_dim x 3, 1].
    // The handle of each: Query: handle, Key:
    // handle + head_num x head_size x sizeof(dt), Value: handle + 2 x head_num x head_size x
    // sizeof(dt).
    const dims stacked_qkv_sz = {p.mb, p.seq_len, 3, p.head_num, p.head_size};
    // Calculate the 4D strides with transposed seq_len and head_num.
    const dims qkv_strides = {p.seq_len * p.head_num * 3 * p.head_size,
            p.head_size, p.head_num * 3 * p.head_size, 1};
    ...
    compiled_partition cp_ukernel = partitions[0].compile(
    ...
    auto ts_key
            = tensor(key, eng, handle + p.head_size * p.head_num * size_of(dt));
    auto ts_value = tensor(
            value, eng, handle + 2 * p.head_size * p.head_num * size_of(dt));
    ...
    auto ukernel_out_ts = ts_output;
    std::vector<float> ukernel_out_data(product(qkv_sz));
    ukernel_out_data = as_vec_type<float>(ukernel_out_ts, product(qkv_sz) / 2);

    dnnl_graph_set_compiled_partition_cache_capacity(0);
    setenv("_ONEDNN_GRAPH_SDPA_FORCE_PRIMITIVE", "1", 1);

    compiled_partition cp_ref = partitions[0].compile(
            {query, key, scale, mask, value}, {output}, eng);
    cp_ref.execute(
            strm, {ts_query, ts_key, ts_scale, ts_mask, ts_value}, {ts_output});
    ...
    // Wait for the computation to finish.
    strm.wait();
    auto ref_out_ts = ts_output;
    std::vector<float> ref_out_data(product(qkv_sz));
    ref_out_data = as_vec_type<float>(ref_out_ts, product(qkv_sz) / 2);
    bool result = allclose(
            ref_out_data, ukernel_out_data, /*rtol=*/4e-2f, /*atol=*/4e-2f);
...
void sdpa_perf(engine::kind ekind, int argc, char **argv) {
    ...
    sdpa_dims_t params = {1, 1024, 12, 64}; // false
    //sdpa_dims_t params = {1, 384, 12, 64}; // true
