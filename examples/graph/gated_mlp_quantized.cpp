...
vector<float> dequantize(const vector<float> &input, memory::desc &desc,
        primitive_attr &attr, int arg, memory::desc &scale_md,
    ...
    switch (p.qtype) {
        case quantize_type::per_token_with_groups: {
            ...
            if (wgu_wt != mdt::f16 && wgu_s_dt != mdt::undef) {
                out.gate_attr_quantized.set_scales(
                        DNNL_ARG_WTS_GATE, gateup_mask, {p.gateup_group_size, 1}, wgu_s_dt);
                out.up_attr_quantized.set_scales(
                        DNNL_ARG_WTS_UP, gateup_mask, {p.gateup_group_size, 1}, wgu_s_dt);
            }
            if (wgu_wt != mdt::f16 && wgu_zp_dt != mdt::undef) {
                out.gate_attr_quantized.set_zero_points(
                        DNNL_ARG_WTS_GATE, gateup_mask, {p.gateup_group_size, 1}, wgu_zp_dt);
                out.up_attr_quantized.set_zero_points(
                        DNNL_ARG_WTS_UP, gateup_mask, {p.gateup_group_size, 1}, wgu_zp_dt);
            }
            if (wd_wt != mdt::f16 && wd_s_dt != mdt::undef)
                out.down_attr_quantized.set_scales(
                        DNNL_ARG_WTS_DOWN, down_mask, {p.down_group_size, 1}, wd_s_dt);
            if (wd_wt != mdt::f16 && wd_zp_dt != mdt::undef)
                out.down_attr_quantized.set_zero_points(
                        DNNL_ARG_WTS_DOWN, down_mask, {p.down_group_size, 1}, wd_zp_dt);
        } break;
        case quantize_type::per_token: {
            ...
            if (wgu_wt != mdt::f16 && wgu_s_dt != mdt::undef) {
                out.gate_attr_quantized.set_scales(
                        DNNL_ARG_WTS_GATE, gateup_mask, {}, wgu_s_dt);
                out.up_attr_quantized.set_scales(
                        DNNL_ARG_WTS_UP, gateup_mask, {}, wgu_s_dt);
            }
            if (wgu_wt != mdt::f16 && wgu_zp_dt != mdt::undef) {
                out.gate_attr_quantized.set_zero_points(
                        DNNL_ARG_WTS_GATE, gateup_mask, {}, wgu_zp_dt);
                out.up_attr_quantized.set_zero_points(
                        DNNL_ARG_WTS_UP, gateup_mask, {}, wgu_zp_dt);
            }
            if (wd_wt != mdt::f16 && wd_s_dt != mdt::undef)
                out.down_attr_quantized.set_scales(
                        DNNL_ARG_WTS_DOWN, down_mask, {}, wd_s_dt);
            if (wd_wt != mdt::f16 && wd_zp_dt != mdt::undef)
                out.down_attr_quantized.set_zero_points(
                        DNNL_ARG_WTS_DOWN, down_mask, {}, wd_zp_dt);
        } break;
        case quantize_type::per_tensor:
            if (wgu_wt != mdt::f16 && wgu_s_dt != mdt::undef) {
                out.gate_attr_quantized.set_scales(
                        DNNL_ARG_WTS_GATE, 0, {}, wgu_s_dt);
                out.up_attr_quantized.set_scales(
                        DNNL_ARG_WTS_UP, 0, {}, wgu_s_dt);
            }
            if (wgu_wt != mdt::f16 && wgu_zp_dt != mdt::undef) {
                out.gate_attr_quantized.set_zero_points(
                        DNNL_ARG_WTS_GATE, 0, {}, wgu_zp_dt);
                out.up_attr_quantized.set_zero_points(
                        DNNL_ARG_WTS_UP, 0, {}, wgu_zp_dt);
            }
            if (wd_wt != mdt::f16 && wd_s_dt != mdt::undef)
                out.down_attr_quantized.set_scales(
                        DNNL_ARG_WTS_DOWN, 0, {}, wd_s_dt);
            if (wd_wt != mdt::f16 && wd_zp_dt != mdt::undef)
                out.down_attr_quantized.set_zero_points(
                        DNNL_ARG_WTS_DOWN, 0, {}, wd_zp_dt);
            break;
    ...
    if(!p.do_quantize) {
        ...
        if (wgu_wt == mdt::s4 || wgu_wt == mdt::s8) {
            ...
            w_gate_data = dequantize(w_gate_quantized_data, w_gate_md,
                   out.gate_attr_quantized, DNNL_ARG_WTS_GATE, w_gate_scales_md,
                   w_gate_zp_data_signed, w_gate_scales_data, wgu_group_size,
                   p.qtype);
            w_up_data = dequantize(w_up_quantized_data, w_up_md,
                   out.up_attr_quantized, DNNL_ARG_WTS_UP, w_up_scales_md,
                   w_up_zp_data_signed, w_up_scales_data, wgu_group_size,
                   p.qtype);
            w_down_data = dequantize(w_down_quantized_data, w_down_md,
                   out.down_attr_quantized, DNNL_ARG_WTS_DOWN, w_down_scales_md,
                   w_down_zp_data_signed, w_down_scales_data, wgu_group_size,
                   p.qtype);
        } else {
            printf("quant init\n");
            w_gate_data = dequantize(w_gate_quantized_data, w_gate_md,
                   out.gate_attr_quantized, DNNL_ARG_WTS_GATE, w_gate_scales_md,
                   w_gate_zp_data_unsigned, w_gate_scales_data, wgu_group_size,
                   p.qtype);
            w_up_data = dequantize(w_up_quantized_data, w_up_md,
                   out.up_attr_quantized, DNNL_ARG_WTS_UP, w_up_scales_md,
            ...
            w_down_data = dequantize(w_down_quantized_data, w_down_md,
                   out.down_attr_quantized, DNNL_ARG_WTS_DOWN, w_down_scales_md,
                   w_down_zp_data_unsigned, w_down_scales_data, wgu_group_size,
                   p.qtype);
        }
    }
