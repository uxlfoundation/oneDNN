...
void jit_uni_shuffle_kernel_t<avx512_core>::emu_gather_data(
    ...
    const unsigned number_of_xmms = static_cast<unsigned int>(is_tail
                    ? utils::div_up(conf_.simd_tail, xmm_size_elem)
                    : utils::div_up(conf_.simd_w, xmm_size_elem));

    for (unsigned i = 0; i < number_of_xmms; i++) {
        const unsigned number_of_xmm_halfs = static_cast<unsigned int>(
        ...
        for (unsigned j = 0; j < number_of_xmm_halfs; j++) {
            ...
            vextractf32x4(xmm_tmp, Zmm(indices_idx), static_cast<uint8_t>(j + i * 2));
        ...
        vinsertf128(Ymm(data_idx), Ymm(data_idx), xmm_dst, static_cast<uint8_t>(i));
    }
}
...
void jit_uni_shuffle_kernel_t<avx>::emu_gather_data(const Reg64 &reg_src_addr,
    ...
    for (unsigned i = 0; i < number_of_xmms; i++) {
        ...
        for (unsigned j = 0; j < number_of_values_to_load; j++) {
            ...
            add(reg_src_addr, reg_tmp_);
            vpinsrd(xmm_dst, xmm_dst, ptr[reg_src_addr], static_cast<uint8_t>(j));
            mov(reg_src_addr, reg_tmp1_);
        }
...
void jit_uni_shuffle_kernel_t<sse41>::emu_gather_data(const Reg64 &reg_src_addr,
    ...
    const unsigned number_of_values_to_load
            = static_cast<unsigned int>(is_tail ? conf_.simd_tail : xmm_size_elem);
    for (unsigned j = 0; j < number_of_values_to_load; j++) {
...
void jit_uni_shuffle_kernel_t<isa>::shuffle_blocked_format() {
    ...
    const int simd_in_tail_blk
            = static_cast<int>(utils::div_up(conf_.c % conf_.blk_size, conf_.simd_w));
    ...
    auto shuffle = ([&](bool is_blk_tail) {
        ...
        for (int i = 0; i < simd_to_process; ++i) {
            ...
            store_data(vmm_src_.getIdx(), reg_dst_,
                    static_cast<int>(i * conf_.simd_w * conf_.dt_size),
                    simd_tail_condition);
        }
    });
    ...
    cmp(reg_cb_loop_size, static_cast<uint32_t>(conf_.blk_size));
    je(no_tail, T_NEAR);
    ...
    L(cb_loop_begin);
    {
        ...
        L(sp_loop_begin);
        {
            cmp(reg_sp, static_cast<uint32_t>(conf_.sp_split_size));
            jge(sp_loop_end, T_NEAR);

            shuffle(false);

            inc(reg_sp);
            add(reg_src_, static_cast<uint32_t>(conf_.blk_size * conf_.dt_size));
            add(reg_dst_, static_cast<uint32_t>(conf_.blk_size * conf_.dt_size));
        ...
        add(reg_cb, static_cast<uint32_t>(conf_.blk_size));
        add(reg_dst_,
                static_cast<uint32_t>(conf_.blk_size * (conf_.sp - conf_.sp_split_size)
                        * conf_.dt_size));
        add(reg_indices_,
                static_cast<uint32_t>(conf_.blk_size * conf_.el_size_of_indices));
