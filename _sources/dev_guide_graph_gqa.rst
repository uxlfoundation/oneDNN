...
#. For training forward propagation, the ``Stats`` output of the SoftMax operation is needed. See :ref:`SoftMax <doxid-dev_guide_op_softmax>` in Graph API for more details.

#. An optional Dropout operation may be used; it takes the normalized output and randomly drops some elements during training. See :ref:`Dropout <doxid-dev_guide_op_dropout>` operation in Graph API.
...
#. The optional End operation marks the output of SoftMaxBackward as a partition output, representing the gradients with respect to the Mask. Note that the output shape of ``dM`` is (N, H_kv, N_rep, S, S) and the data type is f32. The library does not perform any reduction or typecast on this output (for example, reducing to (1, 1, 1, S, S), casting to f16).
