...
status_t dnnl_ze_interop_memory_get_mem_object(
    ...
    bool args_ok = (memory->engine()->runtime_kind() == runtime_kind::ze);
    if (!args_ok) return status::invalid_arguments;

    void *handle;
    CHECK(memory->get_data_handle(&handle));
...
status_t dnnl_ze_interop_memory_set_mem_object(
        memory_t *memory, void *mem_object) {
    bool args_ok = memory != nullptr
            && memory->engine()->runtime_kind() == runtime_kind::ze;
    if (!args_ok) return status::invalid_arguments;
