...
inline size_t data_type_size(data_type_t data_type) {
    ...
    switch ((int)data_type) {
        ...
        case s2: return sizeof(prec_traits_t<s2>::type);
...
inline data_type_t default_accum_data_type(
    ...
    // true
    if (one_of(src_dt, s8, u8, u4, s4, u2, s2) && (dst_dt != f32 || strict))
        return s32;
    ...
    if (one_of(s8, src_dt, dst_dt) || one_of(u8, src_dt, dst_dt)
            ...
            || one_of(s2, src_dt, dst_dt) || one_of(u2, src_dt, dst_dt))
        return s32;
...
inline data_type_t default_accum_data_type(data_type_t src_dt,
    ...
    if (one_of(prop_kind, forward_training, forward_inference)) {
        if (one_of(src_dt, u8, s8) && one_of(wei_dt, u8, s8, s4, u4, s2, u2))
            return s32;
        if (one_of(f16, src_dt, wei_dt)) return f32;
        // weights decompression
        if (one_of(src_dt, bf16, f32) && one_of(wei_dt, u8, s8, s4, u4, s2, u2))
            return f32;
...
inline bool is_integral_dt(data_type_t dt) {
    using namespace data_type;
    return utils::one_of(dt, s64, s32, s8, u8, u4, s4, s2, u2);
}
