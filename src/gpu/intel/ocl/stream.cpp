...
status_t stream_t::set_verbose_profiler(int &last_entry) {
    ...
    // This function prepares the stream profiler for current primitive
    // execution without affecting any ongoing events.
    last_entry = 0;
...
status_t stream_t::run_verbose_profiler(
    ...
    err = clSetEventCallback(
        ...
        hold->s->get_profiling_data(
                profiling_data_kind::time, &num_entries, timing_data.data());
