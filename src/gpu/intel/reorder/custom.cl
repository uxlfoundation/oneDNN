...
#endif
            write(dst + dst_off, AXPY(_sf, _df, src_scale, dst_scale,
...
\
        unroll_for(unsigned sidx = 0; sidx < block_size; ++sidx) { \
            ...
            write(&dst_tmp, AXPY(_sf, _df, src_scale, \
...
#define BATCH_SIZE SUB_GROUP_SIZE
    ...
    for (int d5i = 0; d5i < d5_block; d5i++) {
        ...
        if (!pad) {
            ...
            dst_scale = dst_scales[SCALE_OFF(DST, d0, d1, d2, d3, d4, d5)];
#endif
            float _sf = into_float(dst_buf[iter]);
            write(&dst_tmp, AXPY(_sf, _df, src_scale, dst_scale,
                    sum_scale, src_zp, dst_zp, sum_zp));
        } else {
            write(&dst_tmp, 0.f);
        }
...
#define STRIDE_D (SRC_D2 * SRC_D3)
    ...
    for (int i = 0; i < d1_block; i++) {
        ...
        _df = into_float(dst_tmp);
#endif
        float _sf = into_float(dst_buf[i]);
        write(&dst_tmp, AXPY(_sf, _df, src_scale, dst_scale,
                sum_scale, src_zp, dst_zp, sum_zp));
        block_write(dst + dst_off, &dst_tmp);
    }
        ...
        for (int d = 0; d < SUB_GROUP_SIZE; d += 8) {
            ...
            for (int n = 0; n < 8; n++) {
                ...
                write(dst + dst_off + sglid + n * SUB_GROUP_SIZE,
                        AXPY(_sf, _df, src_scale, dst_scale,
                                sum_scale, src_zp, dst_zp, sum_zp));
            }
        }
    } else {
...
#endif
        write(dst + dst_off + sgId, AXPY(_sf, _df, src_scale, dst_scale,
                sum_scale, src_zp, dst_zp, sum_zp));
    }
...
#endif
        write(&dst0[_i], AXPY(_sf, _df, src_scale, dst_scale,
                sum_scale, src_zp, dst_zp, sum_zp));
    }
    for (int _i = 0; _i < 8; _i++) {
...
#endif
        write(&dst1[_i], AXPY(_sf, _df, src_scale, dst_scale,
                sum_scale, src_zp, dst_zp, sum_zp));
    }
...
// unroll_16b
    ...
    float _sf = into_float(src_tmp);
    write(&dst_tmp, AXPY(_sf, _df, src_scale, dst_scale, sum_scale,
