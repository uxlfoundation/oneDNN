...
jit_avx2_conv_bwd_weights_kernel_f32_t::od_step_comeback_pointers() {
    ...
    L(kd_comeback_loop);
    {
        ...
        sub(aux_reg_kernel,
                get_kernel_offset(static_cast<int>(jcp.kw * jcp.kh), 0));
        dec(kj);
        cmp(kj, 0);
...
jit_avx2_conv_bwd_weights_kernel_f32_t::oh_step_comeback_pointers() {
    ...
    L(kh_comeback_loop);
    {
        ...
        sub(reg_kernel, get_kernel_offset(static_cast<int>(jcp.kw), 0));
        dec(kj);
        cmp(kj, 0);
...
inline void jit_avx2_conv_bwd_weights_kernel_f32_t::compute_oh_step_unroll_ow(
    ...
    L(kh_loop);
    {
        ...
        add(reg_kernel, get_kernel_offset(static_cast<int>(jcp.kw - 1), 0));
        dec(kj);
        cmp(kj, 0);
        jg(kh_loop, T_NEAR);
    }
    ...
    L(kh_loop_ic_tail);
    {
        ...
        add(reg_kernel,
                get_kernel_offset(0, ic_block - ic_tail)
                        + get_kernel_offset(static_cast<int>(jcp.kw - 1), 0));
        dec(kj);
        cmp(kj, 0);
    ...
    if (jcp.ndims == 5) {
        ...
        add(aux_reg_kernel,
                get_kernel_offset(static_cast<int>(jcp.kh * jcp.kw), 0));
        dec(ki);
        cmp(ki, 0);
        jg(kd_loop, T_NEAR);
    }
...
inline void jit_avx2_conv_bwd_weights_kernel_f32_t::compute_oh_loop_common() {
    ...
    if (t_pad > 0) {
        ...
        add(reg_kernel, get_kernel_offset(static_cast<int>(t_pad * jcp.kw), 0));

        L(oh_tpad_loop);
        {
            compute_oh_step_disp();
            add(reg_output, get_output_offset(0, static_cast<int>(jcp.ow)));
            sub(reg_kernel,
                    get_kernel_offset(static_cast<int>(stride_h * jcp.kw), 0));
        ...
        if (t_pad % stride_h != 0) {
            ...
            add(reg_kernel,
                    get_kernel_offset(static_cast<int>(inp_corr * jcp.kw), 0));
