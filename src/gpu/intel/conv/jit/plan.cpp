...
grf_usage_t plan_t::grf_usage() const {
    ...
    // The SLM k-slicing reduction runs after the mainloop and reuses its dead
    // load registers; count only the excess. The load reuses a single header.
...
private:
    void set_plan() {
        ...
        if (slm) {
            ...
            // Extra gmem buffers are dead by the SLM reduction; size them
            // from the mainloop headroom.
            int free = std::max(0,
                    bound - usage.total()
    ...
    // The SLM k-slicing reduction loads (a part of) the thread group C tile
    // back to GRF. Fewer participating threads -> larger per-thread buffer.
