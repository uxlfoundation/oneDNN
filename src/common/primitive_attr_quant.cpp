...
size_t quant_entry_t::get_hash() const {
    ...
    seed = hash_combine(seed, is_host_value_);
    return seed;
}
...
std::string quant_entry_t::get_verbose() const {
    ...
    s.append(is_host_value_ ? ":host_scalar"
                            : ":device_scalar"); // todo: not sure
    return s;
}
