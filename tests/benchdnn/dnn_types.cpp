...
std::ostream &operator<<(std::ostream &s, const attr_t::dropout_t &drop) {
    ...
    if (drop.tag != tag::any || drop.offset != 0 || drop.use_host_scalars)
        s << ":" << drop.tag;
    if (drop.offset != 0 || drop.use_host_scalars) s << ":" << drop.offset;
...
int attr_args_t::prepare_post_ops_mds(const attr_t &attr, int ndims,
    ...
    // dropout
    if (!attr.dropout.is_def()) {
        auto dropout_mask_desc = attr.dropout.tag == tag::undef
                ? dnn_mem_t::init_md()
