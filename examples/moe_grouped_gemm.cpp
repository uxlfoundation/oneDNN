...
/// @param output_concat Concatenated output buffer [total_tokens x N_dim]
/// @param weight_concat Concatenated weights [num_experts x K_dim x N_dim]
/// @param bias_concat Concatenated bias [num_experts x N_dim]
/// @param offsets Cumulative row offsets [num_experts + 1]
...
/// Uses concatenated memory layout where all experts are contiguous
///
/// This implementation demonstrates the grouped memory descriptor API
...
/// @param input_concat Concatenated input buffer [total_tokens x K_dim]
/// @param weight_concat Concatenated weights [num_experts x K_dim x N_dim]
/// @param bias_concat Concatenated bias buffer [num_experts x N_dim]
/// @param output_concat Concatenated output buffer [total_tokens x N_dim]
/// @param offsets Cumulative row offsets [num_experts + 1]
/// @param num_experts Number of experts
...
void onednn_style_grouped_gemm(const float *input_concat,
    ...
    auto weights_md = memory::desc({num_experts, K_dim, N_dim},
            memory::data_type::f32, memory::format_tag::abc);

    // Bias: [num_experts x N_dim]
