...
status_t micro_fwd_t::pd_t::init_conf_microkernels(impl::engine_t *engine) {
    ...
    problem_kq.A.setAlignment(alignmentForLD(int(ldk)));
    ...
    problem_vs.A.setAlignment(alignmentForLD(int(ldv)));
...
status_t micro_bwd_t::pd_t::init_conf_microkernels(impl::engine_t *engine) {
    ...
    problem_kq.B.setAlignment(alignmentForLD(int(ldq)));
    ...
    problem_vs.A.setAlignment(alignmentForLD(int(lda)));
    problem_vs.B.setAlignment(64); // S is packed in SLM
    if (use_systolic_ukernel()) { problem_vs.B.crosspack = 16; }
    ...
    auto ldv
            = gemm_desc_t::get_ld(*desc()->val_md()) * val_mdw.data_type_size();
    problem_vtdA.A.setAlignment(alignmentForLD(int(ldv)));
    problem_vtdA.B.setAlignment(alignmentForLD(int(lda)));
    ...
    problem_qdSt.B.setAlignment(alignmentForLD(int(ldq)));
    if (use_systolic_ukernel()) {
    ...
    problem_ktq.A.setAlignment(alignmentForLD(int(ldk)));
...
status_t micro_fwd_t::pd_t::init_conf(impl::engine_t *engine) {
    ...
    auto max_nelems = nstl::max(qry_mdw.nelems(),
            nstl::max(key_mdw.nelems(),
                    nstl::max(val_mdw.nelems(), dst_mdw.nelems())));
    conf.use_int32_offset = max_nelems <= std::numeric_limits<int32_t>::max();
...
static void append_qry_offs(
    ...
    arg_list.append(static_cast<T>(offs[3][3])); // QRY_D3
}
...
status_t micro_fwd_t::execute_forward(const exec_ctx_t &ctx) const {
    ...
    if (conf.use_int32_offset) {
        append_key_offs<int>(arg_list, key_off);
        append_qry_offs<int>(arg_list, qry_off);
        append_val_offs<int>(arg_list, val_off);
        append_dst_offs<int>(arg_list, dst_off);
        if (pd()->with_attn_mask()) append_msk_offs<int>(arg_list, msk_off);
    } else {
        ...
        if (pd()->with_attn_mask()) {
            //TODO: int64_t mask offset may still cause register corruption
            append_msk_offs<int64_t>(arg_list, msk_off);
        }
    }
