...
impl::status_t sdp_decomp_config_t::construct_params(
    ...
    for (int i = 0; i < mm1_ori_dnnl_pops.get()->len(); i++) {
        ...
        if (mm1_ori_dnnl_pops.get()->entry_[i].is_eltwise()) {
            auto alg = static_cast<algorithm>(
    ...
    if (has_select) {
        ...
        sub_select_cond_md = memory::desc(
                {select_cond_ltw.vdims()[ndims - 2],
                        select_cond_ltw.vdims()[ndims - 1]},
                static_cast<memory::data_type>(select_cond_ltw.data_type()),
                {select_cond_ltw.vstrides()[ndims - 2],
                        select_cond_ltw.vstrides()[ndims - 1]});
        sub_select_src0_md = memory::desc(
                {select_src0_ltw.vdims()[ndims - 2],
                        select_src0_ltw.vdims()[ndims - 1]},
                static_cast<memory::data_type>(select_src0_ltw.data_type()),
                {select_src0_ltw.vstrides()[ndims - 2],
                        select_src0_ltw.vstrides()[ndims - 1]});
