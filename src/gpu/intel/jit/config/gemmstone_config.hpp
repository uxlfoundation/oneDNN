...
#ifndef GPU_INTEL_JIT_GEMMSTONE_CONFIG_HPP
#define GPU_INTEL_JIT_GEMMSTONE_CONFIG_HPP
...
template <ngen::HW hw>
using PostOpInjector
        = dnnl::impl::gpu::intel::jit::post_op_injector_t<GENERATOR_BASE(hw)>;
...
void injectNonBinaryPostOps(const PostOps::entry_t &entry,
        ...
        int C_grfs[ngen::GRF::maxRegs()], int C_ngrf, bool postOpFwd) {
...
template <ngen::HW hw>
void injectStochasticRound(GENERATOR_BASE(hw) * g, ngen::RegisterAllocator ra,
        int C_grfs[ngen::GRF::maxRegs()], int C_ngrf, bool postOpFwd,
        const ngen::Subregister &seed, ngen::DataType t) {
    namespace jit = dnnl::impl::gpu::intel::jit;
    using Injector = jit::eltwise_injector_f32_t<jit::generator_t<hw>>;
    int euCount = 0; /* only used for a DG2 W/A for conv */
