...
/// > Annotated version: @ref vanilla_rnn
///
...
void vanilla_rnn(engine::kind engine_kind) {
    ...
    auto src_layer_mem = memory(src_layer_md, eng);
    auto bias_mem = memory(bias_md, eng);
    auto dst_layer_mem = memory(dst_layer_md, eng);
    ...
    auto vanilla_rnn_pd = vanilla_rnn_forward::primitive_desc(
            eng, prop_kind::forward_training, dnnl::algorithm::eltwise_tanh,
    ...
    vanilla_rnn_args.insert({DNNL_ARG_WORKSPACE, workspace_memory});

    // Primitive execution: vanilla rnn forward

    try {
        vanilla_rnn_prim.execute(s, vanilla_rnn_args);
    ...
    auto diff_src_layer_mem = memory(diff_src_layer_md, eng);
    auto diff_bias_mem = memory(diff_bias_md, eng);
    auto diff_dst_layer_mem = memory(diff_dst_layer_md, eng);
    ...
    write_to_dnnl_memory(
            diff_weights_layer_data.data(), diff_weights_layer_mem);
    write_to_dnnl_memory(diff_weights_iter_data.data(), diff_weights_iter_mem);
    write_to_dnnl_memory(diff_bias_data.data(), diff_bias_mem);

    // Create backward primitive descriptor.
    auto vanilla_rnn_bwd_desc = vanilla_rnn_backward::primitive_desc(eng,
    ...
    // Create the primitive.
    auto vanilla_rnn_backward_prim = vanilla_rnn_backward(vanilla_rnn_bwd_desc);
    ...
    try {
        vanilla_rnn_backward_prim.execute(s, vanilla_rnn_bwd_args);
    } catch (const dnnl::error &e) {
        std::cerr << "Error: " << e.what() << std::endl;
        return;
    }
