        ...
        while (icbb < jcp.nb_ic) {
            ...
            for (dim_t iwork = start; iwork < end; ++iwork) {
                ...
                for (dim_t icb = icbb; icb < icbb + icb_step; ++icb) {
                    ...
                    const dim_t i_t_overflow = nstl::max<dim_t>(
                            0, jcp.t_pad - ij);
                    ...
                    const dim_t d_t_overflow = nstl::max<dim_t>(
                            0, jcp.f_pad - dj);
                    ...
                    const dim_t ih = nstl::max<dim_t>(ij
                                    - jcp.t_pad
                                    + div_up(i_t_overflow, (jcp.dilate_h + 1))
                                            * (jcp.dilate_h + 1),
                            0);

                    const dim_t id = nstl::max<dim_t>(dj
                                    - jcp.f_pad
            ...
            for (dim_t id = 0; id < jcp.id; ++id) {
                ...
                if (jcp.dilate_d != 0) { // stride == 1
                    ...
                    d_t_overflow = div_up(
                            ...
                            dilate_d);
                    d_b_overflow = div_up(nstl::max<dim_t>(0,
                                           ext_kd - jcp.id + id - jcp.back_pad),
                                    dilate_d);
                    od = id + jcp.f_pad - d_b_overflow * dilate_d;
                } else {
                    d_t_overflow = nstl::max<dim_t>(
                            0, jcp.kd - 1 - id - jcp.f_pad);
                ...
                for (dim_t ih = ih_start; ih < ih_end; ++ih) {
                    ...
                    if (jcp.dilate_h != 0) { // stride == 1
                        ...
                        dim_t i_t_overflow = div_up(nstl::max<dim_t>(
                                               0, ext_kh - 1 - ih - jcp.t_pad),
                                        dilate_h);
