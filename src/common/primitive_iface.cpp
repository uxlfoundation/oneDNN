...
status_t primitive_create(primitive_iface_t **primitive_iface,
    ...
    static const std::string &log_kind = VERBOSE_create;

    if (enable_itt) {
        const std::string &pd_info = primitive_desc_iface->info();
        itt::primitive_task_start(
                primitive_desc_iface->impl()->kind(), pd_info, log_kind);
    }
#endif
...
#if defined(DNNL_ENABLE_ITT_TASKS)
    if (enable_itt) itt::primitive_task_end(log_kind);
#endif
...
#if defined(DNNL_ENABLE_ITT_TASKS)
    const bool enable_itt = itt::get_itt(itt::__itt_task_level_low);
    static const std::string &log_kind = VERBOSE_exec;
    if (enable_itt) {
        const std::string &pd_info = pd->info();
