...
kai_matmul_t::pd_t::create_kai_gemm_dequant(
    ...
    return kai_utils::create_kai_gemm_dequant(*args_, kai_src_dt_,
            kai_weights_dt_, kai_dst_dt_, dequant, max_threads);
}

std::unique_ptr<kai::ops::IGemmCommon> kai_matmul_t::pd_t::create_kai_gemm(
        int max_threads) const {
    return kai_utils::create_kai_gemm(
            *args_, kai_src_dt_, kai_weights_dt_, kai_dst_dt_, max_threads);
}
...
status_t kai_matmul_t::execute(const exec_ctx_t &ctx) const {
    ...
    if (pd()->is_dequant()) {
        ...
        kernel = pd()->create_kai_gemm_dequant(dequant, num_threads);
    } else {
        kernel = pd()->create_kai_gemm(num_threads);
    }
