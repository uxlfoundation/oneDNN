...
status_t jit_uni_dw_conv_bwd_weights_kernel<isa, kernel_dt>::init_conf(
    ...
    VDISPATCH_CONV_IC(
            IMPLICATION(!is_data_layout_nxc, jcp.ngroups % jcp.ch_block == 0),
            VERBOSE_BAD_PARAM, "");
    VDISPATCH_CONV_IC(jcp.dilate_h == 0, VERBOSE_BAD_PARAM, "");
    VDISPATCH_CONV_IC(jcp.dilate_w == 0, VERBOSE_BAD_PARAM, "");
    VDISPATCH_CONV_IC(jcp.kw <= 3, VERBOSE_BAD_PARAM, "");
    VDISPATCH_CONV_IC(
            jcp.stride_w <= jcp.kw, VERBOSE_BAD_PARAM, ""); // no gaps in kernel
