...
Adding Level Zero runtime does not require new build options and only requires a new runtime `L0` in the existing CMake build option `ONEDNN_GPU_RUNTIME`, that is only compatible with `ONEDNN_GPU_VENDOR=INTEL` (default).

### External API/ABI change

Adding Level Zero as a new oneDNN runtime is fairly invisible to the end user with the exception of new Level Zero interop API no existing API/ABI requires modification. 
...
///     otherwise.
dnnl_status_t DNNL_API dnnl_l0_interop_stream_create(dnnl_stream_t *stream, dnnl_engine_t engine, ze_command_list_handle_t list, bool profiling);
```
...
///     - The DNNL_MEMORY_NONE specific value. Instructs the library to create memory object without an underlying buffer.
/// @returns #dnnl_success on success and a status describing the error otherwise.
dnnl_status_t DNNL_API dnnl_l0_interop_memory_create_v2(dnnl_memory_t *memory, const_dnnl_memory_desc_t memory_desc, dnnl_engine_t engine, int nhandles, void **handles);
```
...
/// @returns #dnnl_success on success and a status describing the error otherwise.
dnnl_status_t DNNL_API dnnl_l0_interop_primitive_execute(const_dnnl_primitive_t primitive, dnnl_stream_t stream, int nargs, const dnnl_exec_arg_t *args, int ndeps, const ze_event_handle_t *deps, ze_event_handle_t *return_event);
```
...
This approach is preferable, since it does not add any external dependencies and no runtime overhead. The downside is that only Level Zero runtime release with OpenCL™ C compilation support (a future release) can be supported, other version before that can't be used.

2. OpenCL™ runtime compiler - use `clBuildProgram` to create a native binary.

This approach would require oneDNN to load the OpenCL™ runtime, map Level Zero devices to OpenCL™, and use the OpenCL™ compiler to create the binary. This is the path that oneDNN is currently using for SYCL with the Level Zero backend. It's undesired due to unnecessary OpenCL™ runtime dependency when using SYCL with Level Zero and also unnecessary complication with device mapping.
