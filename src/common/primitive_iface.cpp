...
status_t primitive_execute(
    ...
    if (get_verbose(verbose_t::exec_profile,
        ...
        if (!stream->is_verbose_profiler_enabled()) {
            ...
            // measured exeuction times in an asynchronous, non-blocking manner.
            uint64_t profiler_last_entry = 0;
            CHECK(stream->init_verbose_profiler(profiler_last_entry));

            double start_ms = get_msec();
            status = stream->enqueue_primitive(primitive_iface, ctx);
            CHECK(stream->run_verbose_profiler(
                    pd_info, start_ms, profiler_last_entry));
        }
    } else {
        status = stream->enqueue_primitive(primitive_iface, ctx);
    }
