...
bool is_optimized(const matmul::primitive_desc &pd) {
    return pd && std::strstr(pd.impl_info_str(), "brg_matmul") != nullptr;
}

bool requires_reorder(const memory::desc &user_md, const memory::desc &dense_md,
...
status_t sdp_decomp_bmm1_t::init(const dnnl::engine &engine,
    ...
    const bool query_semantic_reorder = requires_reorder(
            query_user_md, query_dense_md, query_reorder_attr);
    const bool key_semantic_reorder
            = requires_reorder(key_user_md, key_dense_md, key_reorder_attr);
    const auto &direct_query_md
            = query_semantic_reorder ? query_dense_md : query_user_md;
    const auto &direct_key_md
            = key_semantic_reorder ? key_dense_md : key_user_md;
    ...
    query_plan_.needs_reorder
            = query_semantic_reorder || src_desc() != direct_query_md;
    key_plan_.selected_md = weights_desc();
    key_plan_.needs_reorder
            = key_semantic_reorder || weights_desc() != direct_key_md;
    return status::success;
}
...
status_t sdp_decomp_bmm2_t::init(const dnnl::engine &engine,
        ...
        const primitive_attr &matmul_attr) {
    const bool value_semantic_reorder = requires_reorder(
            value_user_md, value_dense_md, value_reorder_attr);
    const bool output_semantic_reorder = requires_reorder(
            output_user_md, output_dense_md, output_reorder_attr);
    const auto &direct_value_md
            = value_semantic_reorder ? value_dense_md : value_user_md;
    const auto &direct_output_md
            = output_semantic_reorder ? output_dense_md : output_user_md;
    ...
    value_plan_.needs_reorder
            = value_semantic_reorder || weights_desc() != direct_value_md;
    output_plan_.selected_md = dst_desc();
    output_plan_.needs_reorder
            = output_semantic_reorder || dst_desc() != direct_output_md;
    return status::success;
}
