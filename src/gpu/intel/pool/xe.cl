...
#if IS_TRAINING
#if DT_F64
                VECT_N(long) CMP0 = isless(D0, S0);
                WS0 = select(WS0, kd * KH * KW + kh * KW + kw,
                        CONCAT2(convert_, VECT_N(int))(CMP0));
                D0 = select(D0, S0, CMP0);
#else
                VECT_N(int) CMP0 = isless(D0, S0);
                WS0 = select(WS0, kd * KH * KW + kh * KW + kw, CMP0);
                D0 = select(D0, S0, CMP0);
#endif

#if DT_F64
                VECT_N(long) CMP1 = isless(D1, S1);
                WS1 = select(WS1, kd * KH * KW + kh * KW + kw,
                        CONCAT2(convert_, VECT_N(int))(CMP1));
                D1 = select(D1, S1, CMP1);
#else
                VECT_N(int) CMP1 = isless(D1, S1);
