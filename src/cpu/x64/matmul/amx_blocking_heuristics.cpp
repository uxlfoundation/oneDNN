...
float matmul_amx_blocking_params_macro_t::calculate_blocking_scores() const {
    ...
    bool strip1_b_in_mlc_h
            = strip1_b_tranform_h && b_transform_fits_in_mlc();
...
bool matmul_amx_blocking_params_macro_t::b_transform_fits_in_mlc() const {
    bool b_transform_in_mlc;
    auto max_l2_usage = l2_matrix_usage(
            k_chunk_size_, n_blk_, k_blk_, true, true); /* B transform size */
    b_transform_in_mlc = max_l2_usage < L2_threshold();
...
size_t matmul_amx_blocking_params_macro_t::l2_matrix_usage(size_t k_chunk_size,
        size_t m_or_n_blk, size_t k_blk, bool is_horizontal, bool force_transform_matrix_to_l2) const {
    ...
    if (force_transform_matrix_to_l2) {
        /* L2 matrix orig size */
             l2_matrix_size += m_or_n_blk * k_blk * k_chunk_size * (is_horizontal? b_dt_sz : a_dt_sz);
    }
...
float matmul_amx_blocking_params_macro_t::evaluate_single_core_blocking(
    ...
    if (l2_matrix_usage(k_chunk_size, m_or_n_blk, k_blk, is_horizontal, force_transform_matrix_to_l2)
            <= L2_threshold()) {
        size_t m_blk, n_blk;
        if (is_horizontal) {
            ...
            for (std::set<dim_t>::reverse_iterator it_n = n_candidates.rbegin();
                ...
                for (std::set<dim_t>::reverse_iterator it_k
                    ...
                    float cur_score = evaluate_single_core_blocking(
                            *it_k, *it_n * n_decomposition, k_blk_h, true,force_b_transform_to_l2);
