...
#else
            block_load(&tmp_buf1[d0_inner * d1_block + d1_inner],
                    src1 + src1_off);
...
__kernel void xe_binary(__global SRC0_DATA_T *src0, __global SRC1_DATA_T *src1,
...
#if WITH_SRC0_SCALE
    unroll_for(unsigned idx = 0; idx < NVECT; ++idx) {
        tmp_src0[idx] *= src0_scale[0];
    }
#endif
#if WITH_SRC1_SCALE
#if BCAST_DIM1 || BCAST_AT_INNERMOST_DIM == 1 || NVECT == 1
    tmp_src1 = tmp_src1 * src1_scale[0];
#else
    unroll_for(unsigned idx = 0; idx < NVECT; ++idx) {
        tmp_src1[idx] *= src1_scale[0];
    }
#endif
#endif
    ...
    unroll_for(unsigned idx = 0; idx < NVECT; ++idx) {
#if BCAST_DIM1 || BCAST_AT_INNERMOST_DIM == 1 || NVECT == 1
        d[idx] = binary_op(BINARY_ALG, tmp_src0[idx], tmp_src1);
#else
        d[idx] = binary_op(BINARY_ALG, tmp_src0[idx], tmp_src1[idx]);
#endif
    }
    ...
    APPLY_POST_OPS_SERIAL(
            d[0], dst_data[0], po_mb, po_oc, dims0[2], dims0[3], dims0[4], dims0[5]);
#else
...
#if IS_SRC0_BLOCKED
        float tmp_src0
                = into_float(src0[local_channel + sub_group_size * idx]);
        float tmp_src1 = into_float(src1[idx]);
#else // IS_SRC1_BLOCKED
        float tmp_src0 = into_float(src0[idx]);
        float tmp_src1
                = into_float(src1[local_channel + sub_group_size * idx]);
#endif
#if WITH_SRC0_SCALE
        tmp_src0 = tmp_src0 * src0_scale[0];
#endif
