...
public:
    ...
    expr_t load_expr(const tile_coord_t &tile_coord, int dim_idx) const {
        ...
        dim_t off = offset_in_bytes<dim_t>(reg_layout_, tile_coord.coord);
...
private:
    ...
    struct c_stage_t {
        ...
        void set_next(
            ...
            if (do_reorder) {
                ...
                dim_t this_off = offset_in_bytes<dim_t>(layout);
                dim_t next_off = offset_in_bytes<dim_t>(next->layout);
    ...
    void build_tile(const tile_coord_t &tile_coord,
        ...
        auto send_op = gemm_schedule_.with_kernel_grid_k_slicing()
                ...
                : send_op_t::store;
        auto offset = offset_in_bytes(c_mem_tile_view.tlayout());
        ...
        int c_off_bytes = offset_in_bytes<int>(c_tile_layout);
