...
bool matmul_amx_blocking_params_macro_t::set_blocking_parameters() {
    ...
    if (is_horizontal) {
        ...
        // use OTF if A is unaligned and reused at least 2 times and not completely write bound.
        use_otf_ = (K % wei_k_blk != 0) && (n_blk_ >= n_decomposition * 3)
                && ((size_t)k_blk_ >= (64 / gemm_dt_sz) * 4);

        extendable_k_ = K % wei_k_blk != 0 && !skip_extendable_k() && !use_otf_;
