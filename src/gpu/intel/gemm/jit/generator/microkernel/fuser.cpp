...
/* Host kernel payload layouts, checked against the microkernel's register
   placement once the kernel hosting it is known. */
struct PayloadCheck {
...
static void fuse(std::vector<uint8_t> &binary,
    ...
    for (auto &entry : textSections) {
        ...
        /* This is the host kernel; validate before touching the binary. */
...
bool fuse(std::vector<uint8_t> &binary, const char *source, int grfBytes) {
    ...
    for (const char *s = std::strstr(source, sigilBinary); s;
        ...
        /* Not a shim sigil; skip rather than decode whatever follows. */
        if (*after != ' ') { s = after; continue; }
        microkernel.clear();
        for (s = after + 1; *s != '\n'; s += 2) {
            if (!s[0] || !s[1]) break;
