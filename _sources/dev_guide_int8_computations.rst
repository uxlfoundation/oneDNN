...
For instance, the ``s8`` average :ref:`pooling <doxid-dev_guide_pooling>` primitive accumulates the ``s8`` input values in a window to an ``s32`` accumulator, then divides the result by the window size, and then stores the result back to the ``s8`` destination:
