...
protected:
    ...
    const dim_t vlen_bias_
            = vlen / (f32_dt_size / static_cast<dim_t>(bias_dt_size_));
    ...
    void generate() override {
        ...
        auto compute_loop = [&](dim_t current_vlen_elems) {
            ...
            L(loop_start_label);
            {
                ...
                compute_vaddps(tmp1_vmm, tmp1_vmm, tmp2_vmm, current_vlen_int);
                if (is_training)
                ...
                if (is_augru) {
                    ...
                    // tmp1 = 1 - G01
                    compute_vsubps(tmp1_vmm, tmp1_vmm, G0, current_vlen_int);
                    // tmp1 = G2 * tmp1
                    compute_vmulps(
                            tmp1_vmm, G2, tmp1_vmm, tmp3_vmm, current_vlen_int);
                    ...
                    to_float(tmp2_vmm, ptr[addr_states_tm1_l_reg], src_data_t,
                            current_vlen_int);
                    compute_vmulps(G0, G0, tmp2_vmm, current_vlen_int);
                    compute_vfmadd231ps(G0, tmp1_vmm, G2, current_vlen_int);
                }
        ...
        L(table_label);
        {
            for (size_t i = 0; i < static_cast<size_t>(vlen) / sizeof(float);
                    i++)
