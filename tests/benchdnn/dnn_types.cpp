...
dnnl_quantization_kind_t attr_t::policy2quantization_kind(policy_t policy) {
    if (policy == policy_t::MX)
        return dnnl_quantization_kind_dynamic_mx;
    else
        return dnnl_quantization_kind_static;
}
...
dnnl_primitive_attr_t create_dnnl_attr(
    ...
    if (!attr.scales.is_def()) {
        ...
        for (const auto &arg : as.scales) {
            ...
            bool is_on_host = e.is_host_scalar();
            ...
            DNN_SAFE_V(dnnl_primitive_attr_set_scales_v3(dnnl_attr, arg_name,
                    ...
                    e.dt, is_on_host,
                    attr_t::policy2quantization_kind(e.policy)));
        }
    }
