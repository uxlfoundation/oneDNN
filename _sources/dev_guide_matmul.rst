...
Attribute   :ref:`Zero-points <doxid-structdnnl_1_1primitive__attr_1a8935d36d48fe5db9476b30b02791d822>`              Sets zero-point(s) for the corresponding tensors                                                                                         
...
* 1, which applies a scale / zero point values along ``k`` -dimension for ``DNNL_ARG_WEIGHTS``. Values could be grouped along this dimension via specifying scales / zero points shapes for the attribute (see the code example :ref:`MatMul Tutorial: Weights Decompression <doxid-weights_decompression_matmul_cpp>`).

* 2, which applies a scale / zero point values per column along the ``n`` -dimension for ``DNNL_ARG_WEIGHTS``.
...
* :ref:`MatMul Tutorial: MXFP8 Inference <doxid-mxfp_matmul_cpp>`
