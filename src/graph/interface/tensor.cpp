...
dnnl_graph_tensor::dnnl_graph_tensor(
    ...
    if (handle == DNNL_MEMORY_ALLOCATE) {
        ...
        pass_by_scalar = true;
        if (lt.data_type == data_type::f32) {
            scalar.f32_value = *static_cast<float *>(handle);
            handle_.reset(&scalar.f32_value, dummy_destructor);
        } else if (lt.data_type == data_type::s32) {
            scalar.s32_value = *static_cast<int32_t *>(handle);
            handle_.reset(&scalar.s32_value, dummy_destructor);
        } else {
