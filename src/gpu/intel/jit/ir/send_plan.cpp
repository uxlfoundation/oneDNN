...
struct send_2d_params_t {
    ...
    expr_t to_base(const layout_t &tlayout, const coord_t &targs) const {
        ...
        auto ret = offset_in_bytes(tlayout, t);
        if (h_vstride != 1) {
            coord_t t(targs.size());
            t[h_tidx] = targs[h_tidx] % h_vstride;
            ret += offset_in_bytes(tlayout, t);
        }
        return ret;
    }
    ...
    int64_t to_addr_inc(const layout_t &vlayout,
        ...
        for (int i = 0; i < nblocks; i++) {
            auto &b = blocks[i];
            if (use_xy) {
                if (w_tdim.has_vidx(b.dim.index())) continue;
                if (h_tdim.has_vidx(b.dim.index())) continue;
            }
...
public:
    split_bounds_t(const layout_t &layout, int factor) {
        ...
        layout.for_each_tile(tile_coord.tile, [&](const icoord_t &start) {
            int off = offset_in_bytes<int>(layout, start);
            offs_.push_back(off);
        });
    }
...
struct layout_2d_wrapper_t {
    ...
    int nblocks(dim_idx_t idx = -1) const {
        ...
        for (auto &b : l.blocks()) {
            if (b.block == 1) continue;
            if (idx == dim_idx::invalid || b.dim.index() == idx) ret++;
        }
        return ret;
    }
...
private:
    ...
    void init_base() {
        if (send_kind_ == send_kind_t::_2d) {
            ...
            addr_base_ = offset_in_bytes(vlayout_);
        }
...
public:
    ...
    bool try_enable(send_op_t send_op, const send_2d_hint_t &hint) {
        ...
        for (auto &b : info_.view().tlayout().blocks()) {
            w_tcount += (b.dim.index() == w_tidx);
            h_tcount += (b.dim.index() == h_tidx);
        }
...
private:
    bool base_alignment_ok(const layout_t &vlayout, const mod_info_t &mod_info,
        ...
        auto offset = offset_in_bytes(info_.view().tlayout());
...
public:
    view_iterator_t(const view_info_t &info)
        ...
        for (int i = 0; i < nblocks(); i++) {
            auto &b = blocks()[i];
            block_dims_[i] = dims[b.dim.index()];
            dims[b.dim.index()] *= (int)b.block;
        }
    }
    ...
    void next(vec_vec_off_t &mask, vec_off_t &addr, int elems, int slots,
        ...
        for (int i = 0; i < nblocks(); i++) {
            auto &b = blocks()[i];
            off_[b.dim.index()] += block_off_[i] * block_dims_[i];
        }
