...
status_t gen_gemm_nocopy_kernel_desc_t::select_kernel(compute::gpu_arch_t arch,
    ...
    auto mod_match = [&](MatchParams &params, bool has_mode, const char *(*match)(Type)) {
        if (!has_mode) return;
    ...
    mod_match(base, true, [](Type dt) -> const char * {
        if (dt.isFP4()) return "E";
        return nullptr;
    });

    if(force_cvt){
        mod_match(base, fpmath_bf16, [](Type dt) -> const char * {
        ...
        mod_match(base, fpmath_f16, [](Type dt) -> const char * {
