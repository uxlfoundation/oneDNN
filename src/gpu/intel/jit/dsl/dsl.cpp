...
struct ctx_t {
    ...
    lval_t def(type_t _type, const std::string &name, expr_t value = {},
    ...
    void load_store(const tensor_t &t, const global_tensor_t &g,
            ...
            const pvar_coord_t<int64_t> &base) {
        ...
        if ((plan.min_alignment >= min_align_2d
            ...
            v2::for_each(g.tile, tile, [&](const pvar_coord_t<int64_t> &coord) {
                ...
                auto width
                        = std::min(tile[w_dim], g.tile[w_dim] - coord[w_dim]);
                auto height
                        = std::min(tile[h_dim], g.tile[h_dim] - coord[h_dim]);
                ...
                auto send_kind = [&]() {
                    switch (op_kind) {
                        ...
                        case load_store_kind_t::store:
                            return send_op_t::store_2d;
                    }
                }();

                auto send_func = send_t::make_2d({}, send_kind, type,
                        tensor_width, tensor_height, tensor_pitch, width,
                        height, count,
                        plan.transform == transform_t::kind_t::vnni,
                ...
                || plan.transform == transform_t::kind_t::block) {
            pvar_tile_t tile = plan.get_block_tile(type);
            v2::for_each(g.tile, tile, [&](const pvar_coord_t<int64_t> &coord) {
                auto buf = is_prefetch
                        ? expr_t()
                        : t.buffer[t.layout.offset_in_bytes(base + coord)];
                ...
                while (width_bytes > 0) {
                    ...
                    auto send_kind = [&]() {
                        switch (op_kind) {
                            ...
                            case load_store_kind_t::load:
                                return send_op_t::load;
                            case load_store_kind_t::store:
                                return send_op_t::store;
                        }
                    }();
    ...
    void mma(const tensor_t &C, const tensor_t &A, const tensor_t &B,
            const pvar_tile_t tile, pvar_coord_t<int64_t> base,
            bool is_systolic) {
        if (is_systolic) {
            int64_t simd = 16;
            int64_t sdepth = 8;
            ...
            pvar_tile_t inst_tile {{dim_simd, simd},
            ...
            v2::for_each(
                    tile, inst_tile, [&](const pvar_coord_t<int64_t> &coord) {
                        ...
                        auto rcount
                                ...
                                / rcount_pack;
                        auto dpas = dpas_t::make(false, simd,
                                into<uint8_t>(sdepth), into<uint8_t>(rcount),
                                C.layout.type(), B.layout.type(),
                                A.layout.type());
            ...
            v2::for_each(
                    tile, inst_tile, [&](const pvar_coord_t<int64_t> &coord) {
                        ...
                        auto mad = mad_t::make(hw_t(), C.layout.type(), simd,
                                A.layout.type(), a_stride, B.layout.type(),
                                b_stride);
    ...
    void load(const tensor_t &t, const global_tensor_t &g,
            const transform_t &plan, const pvar_coord_t<int64_t> &base) {
    ...
    void if_(expr_t cond, stmt_t if_body) { append(if_t::make(cond, if_body)); }
...
lval_t &lval_t::operator=(const expr_t &obj) {
    default_ctx().assign(this->var, obj);
    return *this;
}
