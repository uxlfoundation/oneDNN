...
// carries the mayiuse check for the f16 (zvfh) path. f32/s32/s8/u8 are always
// supported (as on x64); bf16 is not (it needs the Zvfbfmin extension the
// binary kernel does not emit -- x64 supports it via avx512_core/avx2_vnni_2).
...
status_t jit_uni_binary_t::pd_t::init(const engine_t *engine) {
    ...
    VDISPATCH_BINARY(
            post_ops_ok(attr(), src_md(0), dst_md(),
                    conf_.is_src_different_layouts, mayiuse(zvfh) ? zvfh : v),
...
bool jit_uni_binary_t::post_ops_ok(const primitive_attr_t *attr,
    ...
    for (int i = 0; i < p.len(); i++) {
        if (is_binary(i)) {
            ...
            // x64 parity: an i8 dst rejects an xf16 binary post-op rhs (bf16
            // is already excluded by the injector's supported-dtype set).
            if (is_i8 && is_src1_f16) return false;
