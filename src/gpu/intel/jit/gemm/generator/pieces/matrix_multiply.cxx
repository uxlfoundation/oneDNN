                                    ...
                                    const GEMMProblem &problem, const GEMMStrategy &strategy, GEMMState &state)
{
    ...
    int nx1 = (mixedMode || state.broadcast) ? nx : std::min(nx, fmaSIMD);
...
template <HW hw>
void Generator<hw>::outerProductGen9IGEMM(int ha, int hb, const RegisterLayout &A_layout, const RegisterLayout &B_layout,
                                          const GRFMultirange &A_regs, const GRFMultirange &B_regs,
                                          const GEMMProblem &problem, const GEMMStrategy &strategy, GEMMState &state)
{
    auto Ta = problem.Ta, Tb = problem.Tb, Tc = problem.Tc_compute();
    ...
    for (int y = 0; y < ny; y++) {
        for (int x = 0; x < nx;) {
            ...
            int na, nb;
            const RegisterBlock *A_block, *B_block;
            Subregister A = A_layout.find(i, ha, A_regs, &na, &A_block);
            Subregister B = B_layout.find(hb, j, B_regs, &nb, &B_block);

            // Find the appropriate C register. Todo: remainders.
            int nc;
            const RegisterBlock *C_block;
            Subregister C = state.C_layout.find(i, j, state.C_regs[0], &nc, &C_block);
