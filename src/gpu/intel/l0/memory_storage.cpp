...
void *memory_storage_t::malloc_device(size_t size) const {
    ...
    func_zeMemAllocDevice(l0_engine()->context(), &device_mem_alloc_desc, size,
...
void *memory_storage_t::malloc_shared(size_t size) const {
    ...
    func_zeMemAllocShared(l0_engine()->context(), &device_mem_alloc_desc,
...
void memory_storage_t::free(void *ptr) const {
    func_zeMemFree(l0_engine()->context(), ptr);
}
...
status_t memory_storage_t::memcpy(
    ...
    return func_zeCommandListAppendMemoryCopy(
            l0_stream->list(), dst, src, size, nullptr, 0, nullptr);
}

memory_storage_kind_t memory_storage_t::get_pointer_type(
    ...
    func_zeMemGetAllocProperties(l0_engine()->context(), ptr,
            &memory_allocation_properties, nullptr);

    switch (memory_allocation_properties.type) {
    ...
    return memory_storage_kind_t::unknown;
}
