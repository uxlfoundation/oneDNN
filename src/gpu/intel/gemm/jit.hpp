...
struct gen_t : public primitive_t {
    struct pd_t : public jit::pd_t {
        ...
        bool decide_swap_ab(const jit::kernel_config_t &cfg) const override {
            bool check_lda = ((!cfg.trans_a() && cfg.lda == 1) || cfg.trans_a());
        ...
        status_t check_problem(impl::engine_t *engine) const {
            ...
            if (utils::one_of(d->c_type(), s32, f16, bf16, f32, u8, s8)
                ...
                VDISPATCH_GEMM((utils::one_of(d->b_type(), u8, s8)
                                       || wei_decomp()),
                ...
                VDISPATCH_GEMM(utils::one_of(d->a_type(), f64, f32, f16, bf16,
                        ...
                        VERBOSE_UNSUPPORTED_DT);
                VDISPATCH_GEMM((d->b_type() == d->a_type()
                                       || (utils::one_of(d->a_type(), f8_e5m2,
                                                   f8_e4m3)
                                               && utils::one_of(d->b_type(),
                                                       f8_e5m2, f8_e4m3))
                                       || (utils::one_of(d->a_type(), f4_e2m1,
                                                   f4_e3m0)
                                               && utils::one_of(d->b_type(),
                                                       f4_e2m1, f4_e3m0))),
                        VERBOSE_INCONSISTENT_DT, "a", "b");
            ...
            VDISPATCH_GEMM(batch_dims() <= 4, VERBOSE_BAD_DIM, "batch",
                    batch_dims());
        ...
        status_t init(impl::engine_t *engine) {
            ...
            // After apply_swap_ab, cfg_ is in kernel orientation.
            ...
            VDISPATCH_GEMM(IMPLICATION(cfg_.trans_c(), swap),
                    VERBOSE_UNSUPPORTED_TAG);
            ...
            // Kernel-side post-ops are valid only after bias/scale lowering;
            // gate here.
            ...
            auto entries = kernel_desc.select_kernel(*dev_info_, mode, cfg.problem,
                    cfg.alpha(), cfg.beta, cfg.m, cfg.n, cfg.k, cfg.lda, cfg.ldb,
                    cfg.ldc, cfg.batch());
        ...
        // Finalized kernel desc selected in init(); owns the gemmstone
        // strategy/driver_info/aux_params (cfg keeps only the problem).
