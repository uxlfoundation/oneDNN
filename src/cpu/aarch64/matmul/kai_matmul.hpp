...
struct kai_matmul_t : public primitive_t {
    struct pd_t : public cpu::matmul::cpu_matmul_pd_t {
        ...
        std::unique_ptr<kai::ops::IGemmCommon> create_kai_gemm_dequant(
                const kai::ops::DequantizeFloat &, int max_threads = 0) const;
        ...
        // The GemmConfig we use to select and construct the kernel, this may
        // differ from the one returned by kernel after selection.
        ...
        // KAI calls its packed-B transform "pretranspose". Keep this separate
        // from oneDNN layout reorders such as user ba -> kernel-facing ab.
        bool _pack_weights = false;
        bool _pack_transposed_weights = false;
        bool _reorder_src_ba_to_ab = false;
        bool _reorder_weights_ba_to_ab = false;
        bool _reorder_dst_ab_to_ba = false;
        // Compute A^T * B^T as (B * A)^T when one output transpose is cheaper
        // than two input transposes.
        bool _swap_transAB_with_dst_transpose = false;
        ...
        int _reorder_nested_scratchpad_base = 0;
        memory_desc_t src_ab_md_;
        memory_desc_t weights_ab_md_;
        memory_desc_t dst_ab_md_;
        memory_desc_t dst_ba_md_;
        memory_desc_t swapped_src_md_;
        memory_desc_t swapped_weights_md_;
        memory_desc_t swapped_dst_md_;
        ...
        std::shared_ptr<primitive_desc_t> dst_ba_to_ab_reorder_pd_;
...
private:
    ...
    std::shared_ptr<primitive_t> dst_ba_to_ab_reorder_;
