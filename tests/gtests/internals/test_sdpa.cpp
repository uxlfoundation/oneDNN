...
struct sdpa_dims_t {
    ...
    //sdpa_dims_t() = default;
    //sdpa_dims_t(const sdpa_dims_t &other) = default;
    //sdpa_dims_t(sdpa_dims_t &&other) = default;
    //sdpa_dims_t &operator=(const sdpa_dims_t &other) = default;
    //sdpa_dims_t &operator=(sdpa_dims_t &&other) = default;
...
public:
    ...
    void compare() {
        ...
        std::unordered_map<int, memory> s8_args
        ...
        if (!v_is_float && p.qtype != quantize_type::no_quantization) {
            if (p.value.sdt != mdt::undef)
                s8_args[DNNL_ARG_ATTR_SCALES | DNNL_ARG_VALUES]
                        = t.m_value_scales;
            if (p.value.zpdt != mdt::undef)
                s8_args[DNNL_ARG_ATTR_ZERO_POINTS | DNNL_ARG_VALUES]
                        = t.m_value_zp;
        }
...
using sdpa_test_tuple = sdpa_test_t<sdpa_dims_t_tuple>;

// clang-format off

INSTANTIATE_TEST_SUITE_P(DataTypes_f16_s8, sdpa_test_tuple,
...
INSTANTIATE_TEST_SUITE_P(DataTypes_f16_s4, sdpa_test_tuple,
        testing::Combine(testing::Values(1), // mb
                testing::Values(num_heads_t {2, 2}), // hd_num
...
INSTANTIATE_TEST_SUITE_P(DataTypes_bf16_s8, sdpa_test_tuple,
        testing::Combine(testing::Values(1), // mb
                testing::Values(num_heads_t {2, 2}), // hd_num
                testing::Values(seq_len_size_t {384, 384}, seq_len_size_t {1, 385}), // seq_len
                testing::Values(head_group_size_t {128, 128, 128}, head_group_size_t {256, 256, 256}, head_group_size_t {512, 512, 512}), // hd_size
...
INSTANTIATE_TEST_SUITE_P(DataTypes_bf16_s4, sdpa_test_tuple,
        testing::Combine(testing::Values(1), // mb
                testing::Values(num_heads_t {2, 2}), // hd_num
                testing::Values(seq_len_size_t {384, 384}, seq_len_size_t {1, 386}), // seq_len
                testing::Values(head_group_size_t {128, 128, 128}, head_group_size_t {256, 256, 256}, head_group_size_t {512, 512, 512}), // hd_size
...
INSTANTIATE_TEST_SUITE_P(DataTypes_f32, sdpa_test_tuple,
        testing::Combine(testing::Values(1), // mb
                testing::Values(num_heads_t {2, 2}), // hd_num
...
INSTANTIATE_TEST_SUITE_P(AllMaskTypes, sdpa_test_tuple,
        testing::Combine(testing::Values(1), // mb
                testing::Values(num_heads_t {2, 2}), // hd_num
                testing::Values(seq_len_size_t {384, 384}, seq_len_size_t {1, 385}), // seq_len
...
INSTANTIATE_TEST_SUITE_P(GQA, sdpa_test_tuple,
        testing::Combine(testing::Values(1), // mb
...
GPU_TEST_P(sdpa_test_tuple, compare) {
    compare();
}
...
/*
GPU_TEST_P(sdpa_test_tuple, perf) {
    perf();
}
*/
