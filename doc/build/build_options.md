...
* As threadpools are attached to streams which are only passed during primitive
  ...
  balancing the static decomposition from the previous item across available
  worker threads.
...
headers. Additional actions will likely require header files placement to be
followed as they reside inside openXLA project.

For example:
~~~sh
$ cmake -DONEDNN_CPU_RUNTIME=THREADPOOL -D_ONEDNN_TEST_THREADPOOL_IMPL=EIGEN -DCMAKE_PREFIX_PATH=/path/to/eigen/share/eigen3/cmake ..
~~~
