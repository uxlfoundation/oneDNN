Quantized ConvTranspose Fusions {#dev_guide_graph_quantized_convolution_fusions}
===========================================================

## Overview

oneDNN supports various quantized ConvTranspose fusion patterns to optimize
performance and reduce memory bandwidth requirements. This document describes
the supported fusion patterns for quantized ConvTranspose.

## ConvTranspose patterns

oneDNN supports quantized ConvTranspose and its optimization through Graph API [1]
by defining the graph, getting partition from the graph, and optimizing the kernels
underneath. In general, a quantized ConvTranspose pattern is defined as a directional
acyclic graph (DAG) using oneDNN Graph API.

### Quantized ConvTranspose patterns

oneDNN defines quantized ConvTranspose patterns as follows.
The blue parts are required when defining a quantized ConvTranspose pattern while the
...
2. The ConvTranspose performs ConvTranspose between src and weights tensor.
   The bias tensor is optional.
   See [ConvTranspose](@ref dev_guide_op_convtranspose) operation in Graph API.
...
4. The data type conversion subgraph #2 is optional and is used to convert output
   ...
   following subgraphs. See [Quantize](@ref dev_guide_op_quantize) and
...
1. GPU
   - Dequantize and Quantize in data type conversion subgraphs only supports zps
     values as all zeros.
   - Quantize in data type conversion subgraph #2 only supports per_tensor quantization
     type, and its scales values should be all ones.

## Example

oneDNN provides an [quantized ConvTranspose
example](https://github.com/oneapi-src/oneDNN/tree/main/examples/graph/cpu_inference_int8.cpp)
demonstrating how to construct a typical quantized ConvTranspose pattern with oneDNN
Graph API on CPU.
