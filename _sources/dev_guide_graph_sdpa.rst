...
#. The optional Dropout operation takes the normalized output and randomly drops some elements during training. See :ref:`Dropout <doxid-dev_guide_op_dropout>` operation in Graph API.
...
#. The Dropout, TypeCast and MatMul operations after Exp are used to compute the gradients with respect to Value. TypeCast is required for bf16 and f16 training scenarios. See :ref:`TypeCast <doxid-dev_guide_op_typecast>` in Graph API.
...
#. The optional End operation marks the output of SoftMaxBackward as a partition output, representing the gradients with respect to the Mask. Note that the output shape of ``dM`` is (N, H, S, S) and the data type is f32. The library does not perform any reduction or typecast on this output (for example, reducing to (1, 1, S, S), casting to f16).
