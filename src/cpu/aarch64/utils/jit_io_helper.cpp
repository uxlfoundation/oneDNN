...
template <cpu_isa_t isa>
void jit_io_helper_t<isa>::init_saturation(data_type_t dt, bool force) {
    if (!force
...
template <cpu_isa_t isa>
void jit_io_helper_t<isa>::init_tail(bool force) {
...
template <cpu_isa_t isa>
bool jit_io_helper_t<isa>::is_tail_ok(bool is_tail) const {
    return IMPLICATION(is_tail, is_tail_ready_);
}

template <cpu_isa_t isa>
bool jit_io_helper_t<isa>::is_saturation_ok(data_type_t dt) const {
