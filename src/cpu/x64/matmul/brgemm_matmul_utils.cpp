    ...
    if (B_any_layout) {
        if (bgmmc.is_gemv) {
            ...
            const int default_n_block = init_n_tag
                    ...
                    : bgmmc.N_blk;
            // Blocked B sets use_buffer_b=false and a single per-K
            // comp into weights, both of which break per-K-grouped quant
            // (needs runtime copy_b for ZP subtraction or per-group comp).
            const bool blocked_B_breaks_grouped
...
dim_t get_min_per_k_group_size(const brgemm_matmul_conf_t &bgmmc) {
    dim_t sz = 0;
    ...
    if (bgmmc.is_wei_zp_per_k && bgmmc.wei_zp_k_gsize > 0)
        sz = sz == 0 ? bgmmc.wei_zp_k_gsize
                     : nstl::min(sz, bgmmc.wei_zp_k_gsize);
...
status_t compute_blocking_heuristic(brgemm_matmul_conf_t &bgmmc,
    ...
    if (bgmmc.is_amx) {
        ...
        if (bgmmc.with_int8_grouped_quantization) {
            const auto ic_group_sz = get_min_per_k_group_size(bgmmc);
            if (ic_group_sz > 0) {
    ...
    if (!bgmmc.is_amx && bgmmc.with_int8_grouped_quantization) {
        const auto ic_group_sz = get_min_per_k_group_size(bgmmc);
        if (ic_group_sz > 0) {
            const auto target_K_blk
                    = nstl::min(bgmmc.K, static_cast<dim_t>(ic_group_sz));
            if (bgmmc.K_blk != target_K_blk) {
                bgmmc.K_blk = target_K_blk;
                bgmmc.brgemm_batch_size = 1;
                bgmmc.K_chunk_size = 1;
                bgmmc.use_buffer_c
                        = bgmmc.use_buffer_c || bgmmc.K > bgmmc.K_blk;
            }
        }
    }
...
status_t init_brgemm_matmul_conf(cpu_isa_t isa, brgemm_matmul_conf_t &bgmmc,
    ...
    if (bgmmc.is_bf32 || bgmmc.is_bf16_with_int_wei) {
        ...
        bgmmc.wei_dt = one_of(bgmmc.wei_dt, s8, u8) ? bgmmc.wei_dt
                : bgmmc.wei_dt == u4                ? u8
                                                    : s8;
    ...
    // Grouped quantization always accumulates in f32 to perform accuracy.
    bgmmc.acc_dt
    ...
    if (bgmmc.with_wei_scales) {
        ...
        // or grouped quantization (int8 src with int4 weights)
    ...
    if (has_wei_zp) {
        ...
        // Weight ZP dt must match the original weight dt so the byte-wise
        // (wei - zp) shift in copy_b stays in the same numerical range.
        const bool is_grouped_wei_zp
                = bgmmc.is_wei_zp_per_k || bgmmc.is_wei_zp_per_n;
    ...
    // per_tensor grouped ZP on 4D tensors has batch bits in the mask.
    // Compute the stride between ZP planes.
    ...
    // be divisible by tile-size 64 (only grouped quantization path).
    ...
    // layout when needed
    if ((bgmmc.with_wei_decompression || bgmmc.with_int8_grouped_quantization)
            && bgmmc.is_int4_weights)
...
void init_aux_values(brgemm_matmul_conf_t &bgmmc,
    ...
    // Extend per-thread comp scratchpad by `num_K_groups` slabs for per-K
    // grouped quant (one comp per group).
    if (bgmmc.s8s8_compensation_required && bgmmc.with_int8_grouped_quantization
            && bgmmc.K_blk > 0) {
        const dim_t num_K_groups = utils::div_up(bgmmc.K, bgmmc.K_blk);
        bgmmc.s8s8_comp_k_str = bgmmc.s8s8_comp_ithr_str;
        bgmmc.s8s8_comp_ithr_str *= num_K_groups;
    }
