...
struct brgemm_matmul_t<isa>::brg_matmul_exec_ctx_t {
    brg_matmul_exec_ctx_t(
        ...
        if (is_wei_zp_per_oc) {
            zero_point_b_per_oc_buf_ = scratchpad.template get<int32_t>(
            ...
            zero_point_b_per_oc_buf_ = nullptr;
        }
    ...
    */
    ...
    const int32_t *get_zp_b_val_ptr(int n_offset) const {
        if (zero_point_b_per_oc_buf_) {
            return zero_point_b_per_oc_buf_ + n_offset;
        } else {
            return &wei_zp_val_;
        }
    }
    ...
    const void *get_wei_zp_ptr(int n, int k = 0) const {
        ...
        if (bgmmc_.wei_zp_type == brgemm_broadcast_t::per_n) {
            if (zero_point_b_per_oc_buf_) {
                return zero_point_b_per_oc_buf_ + offset;
            }
        }
...
private:
    ...
    int32_t *zero_point_b_per_oc_buf_;
