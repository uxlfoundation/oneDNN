...
private:
    ...
    inline void maybe_apply_src_zp(Vmm vmm_data, int k_idx) {
        ...
        if (is_int4_zp) {
            // Compute absolute element index from base pointer:
...
void jit_brgemm_matmul_copy_a_impl_t<Vmm>::generate() {
    ...
    if (do_apply_src_zp_) {
        ...
        // the index for correct nibble addressing.
...
protected:
    ...
    */
    inline void maybe_apply_per_k_zero_points(const Vmm &vmm_src,
     ...
     * @param inner_n_offs Offset for inner N dimension, used for AVX2
     */
    inline void maybe_apply_per_n_zero_points(const Vmm &vmm_src, bool is_tail,
    ...
    */
    ...
    inline void maybe_apply_zero_points(const Vmm &vmm_src, const int inner_k,
        ...
        maybe_apply_per_n_zero_points(vmm_src, is_tail, ncolumns, inner_n_offs);
        maybe_apply_per_k_zero_points(
...
private:
     ...
     */
    void load_int8_grouped_common_zp() {
        if (!req_int8_zp_b_shift_) return;
...
void jit_brgemm_matmul_copy_b_transposed_t<Vmm>::compute_K_loop(bool is_N_tail,
    ...
    const bool eff_first = comp_per_chunk ? true : is_first_K_iter;
    const bool eff_last = comp_per_chunk ? true : is_last_K_iter;
    ...
    if (req_zp_comp_) {
        ...
        if (!eff_first) vpaddd(vmm_comp_acc, vmm_comp_acc, addr);
        if (eff_last) uni_vpmulld(vmm_comp_acc, vmm_comp_acc, vmm_zp_a_neg_val);
