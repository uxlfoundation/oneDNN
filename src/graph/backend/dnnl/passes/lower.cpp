...
static status_t softmax_handler(
    ...
    // support stats computation: stats = reducemax(src) - log(reducemax(dst))
    ...
    reduce_src_op_out_val->set_data_type(dnnl::impl::data_type::f32);
    ...
    reduce_dst_op->set_attr<int64_t>(op_attr::alg_kind,
            static_cast<int64_t>(dnnl::algorithm::reduction_max));
    reduce_dst_op->add_input(dst);
    dst->add_consumer(*reduce_dst_op, 0);
    ...
    reduce_dst_op_out_val->set_data_type(dnnl::impl::data_type::f32);
    ...
    log_op_out_val->set_data_type(dnnl::impl::data_type::f32);
    ...
    reduce_sum_dst_op->add_input(dst);
    dst->add_consumer(*reduce_sum_dst_op, 0);
