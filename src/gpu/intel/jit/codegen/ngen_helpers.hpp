...
#define CASE(_kind, ngen_enum) \
    ...
    if (type.scalar() == type_t::f4_e3m0()) return ngen_e3m0();
    // Until f4_e2m1 lands in ngen
    if (type.scalar() == type_t::f4_e2m1()) return ngen_e2m1();
...
#define CASE(_kind, ngen_enum) \
    ...
    if (type == ngen_e3m0()) return type_t::f4_e3m0();
    if (type == ngen_e2m1()) return type_t::f4_e2m1();
