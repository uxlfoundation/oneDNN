    ...
    for (int n_l = 0; n_l < CELL_BATCH_THR; n_l++) {
        ...
        for (int c_l = 0; c_l < CELL_DHC_THR; c_l++) {
            ...
            vanilla_lstm_store(gates.ptr, gates.strides.mb, states.ptr,
                    ...
                    states.strides.mb, dims.dhc, n, c, ctx.lstm.tm_cscale, g);
#elif CELL_KIND == VANILLA_RNN
            float g = vanilla_rnn_compute_gates(C[0][n_l][c_l],
                    ctx.rnn.bias[off_ker_bias(dims.dhc, 0, c)], ctx.rnn.alpha,
                    ctx.rnn.tm_scales);
            ...
            lbr_gru_gates_t g = compute_gates_lbr_gru(scratch_gates.ptr,
                    ...
                    ctx.lbr_gru.tm_scales, scratch_gates.strides.mb, dims.dhc,
                    n, c);
            float Wh_b = g.Wh_b;
            float G0 = g.G[0];
            float G1 = g.G[1];
            ...
            if (!RECOMPUTE_GATES && IS_TRAINING) {
                gates.ptr[cell_ws_gates(gates.strides.mb, dims.dhc, n, 0, c)]
                        = G0;
                gates.ptr[cell_ws_gates(gates.strides.mb, dims.dhc, n, 1, c)]
                        = G1;
                gates.ptr[cell_ws_gates(gates.strides.mb, dims.dhc, n, 2, c)]
                        = G2;
