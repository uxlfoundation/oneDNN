...
#ifdef GEMMSTONE_WITH_LEVEL_ZERO_RUNTIME
template <ngen::HW hw>
using ze_gen_t = ir::ir_to_ngen_generator_t<ngen::LevelZeroCodeGenerator<hw>>;
GEMMSTONE_XELP_ISA(
        template void ir::convert_ir_to_ngen<ze_gen_t<ngen::HW::XeLP>>(
                const stmt_t &body, ze_gen_t<ngen::HW::XeLP> &host,
                const walk_order_t *kernel_grid_walk_order));
GEMMSTONE_XEHP_ISA(
        template void ir::convert_ir_to_ngen<ze_gen_t<ngen::HW::XeHP>>(
                const stmt_t &body, ze_gen_t<ngen::HW::XeHP> &host,
                const walk_order_t *kernel_grid_walk_order));
GEMMSTONE_XEHPG_ISA(
        template void ir::convert_ir_to_ngen<ze_gen_t<ngen::HW::XeHPG>>(
                const stmt_t &body, ze_gen_t<ngen::HW::XeHPG> &host,
                const walk_order_t *kernel_grid_walk_order));
GEMMSTONE_XEHPC_ISA(
        template void ir::convert_ir_to_ngen<ze_gen_t<ngen::HW::XeHPC>>(
                const stmt_t &body, ze_gen_t<ngen::HW::XeHPC> &host,
                const walk_order_t *kernel_grid_walk_order));
GEMMSTONE_XE2_ISA(template void ir::convert_ir_to_ngen<ze_gen_t<ngen::HW::Xe2>>(
        const stmt_t &body, ze_gen_t<ngen::HW::Xe2> &host,
        const walk_order_t *kernel_grid_walk_order));
GEMMSTONE_XE3_ISA(template void ir::convert_ir_to_ngen<ze_gen_t<ngen::HW::Xe3>>(
        const stmt_t &body, ze_gen_t<ngen::HW::Xe3> &host,
        const walk_order_t *kernel_grid_walk_order));

std::pair<ze_module_handle_t, ze_kernel_handle_t> make_kernel(
        const kernel_t &ir_kernel, ze_context_handle_t ctx,
        ze_device_handle_t dev) {
    const auto &iface = ir_kernel.iface;
    const auto &options = ir_kernel.options;
    const auto &body = ir_kernel.body;
    const auto &debug_cfg = ir_kernel.debug_cfg;

    ngen::NEOInterfaceHandler interface = generate_ngen_interface(
            iface, options, body);

#define GPU_HW_CASE(hw) \
    ze_gen_t<(hw)> g(iface, options, debug_cfg); \
    g.setInterface(std::move(interface)); \
