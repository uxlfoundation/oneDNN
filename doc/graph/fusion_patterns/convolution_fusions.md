...
### Floating-point Convolution patterns for inference
...
1. The first Convolution performs Convolution between src and weights tensor.
   The bias tensor is optional. See [Convolution](@ref dev_guide_op_convolution)
...
### Floating-point Convolution patterns for training

oneDNN supports limited training fusion patterns.
The floating-point (f32, bf16, or f16) ConvolutionBackwardWeight patterns is defined as follows.
The blue parts are required when defining a ConvolutionBackwardWeights
pattern while the brown parts are optional.

![ConvolutionBackwardWeights pattern](images/conv_bwd_pattern.png)

The [ConvolutionBackwardWeights](@ref dev_guide_op_convolutionbackwardweights) and
[BiasAddBackward](@ref dev_guide_op_biasaddbackward) shares the same input of diff_dst.
