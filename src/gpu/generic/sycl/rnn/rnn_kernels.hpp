...
struct ref_rnn_copy_fwd_t {
    ...
    void do_copy(
        ...
        if (src_ptr()) {
            ...
            if (dst_ptr()) {
                store_float_value(
                        dst_md().data_type(), src, dst_ptr(), dst_offset);
            }
        } else {
            if (dst_ptr()) {
                store_float_value(
                        dst_md().data_type(), 0.0f, dst_ptr(), dst_offset);
            }
        }
    }
};
...
struct ref_rnn_copy_bwd_t {
    ...
    void do_copy(
        ...
        if (src_ptr()) {
            auto src = load_float_value(
                    conf_.src_md.data_type(), src_ptr(), src_offset);
            if (dst_ptr()) {
                store_float_value(
                        conf_.src_md.data_type(), src, dst_ptr(), dst_offset);
            }
        } else {
            if (dst_ptr()) {
                store_float_value(
                        conf_.src_md.data_type(), 0.0f, dst_ptr(), dst_offset);
            }
        }
    }
};
...
struct ref_rnn_bias_bwd {
    ref_rnn_bias_bwd(const sycl_rnn_bias_conf_t &conf,
    ...
    void operator()(::sycl::nd_item<3> item) const {
        ...
        float dsl_val = load_float_value(
                conf_.src_type, diff_state_l, diff_state_l_off);
        float dsi_val = load_float_value(
                conf_.src_type, diff_state_i, diff_state_iter_off);
        ...
        float g = load_float_value(conf_.bias_type, ws_gates, ws_gates_off);

        float tmp = dH * activation_bwd(g);

        store_float_value(conf_.dst_md.data_type(), tmp, scratch_diff_gates,
    ...
    inline float activation_bwd(float in_val) const {
        switch (conf_.activation_kind) {
            case alg_kind::eltwise_relu:
                return (float)(in_val > 0 ? 1.0f : conf_.alpha);
            case alg_kind::eltwise_tanh:
                return (float)((1 - in_val) * (1 + in_val));
            case alg_kind::eltwise_logistic:
                return (float)(in_val - in_val * in_val);
            default: return 0;
        }
    }
