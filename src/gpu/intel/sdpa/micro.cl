...
#define ugemm_slm_size MAX(ugemm_kq_slm_size, ugemm_vs_slm_size)
    ...
    bool block_msk
            = (b1 < MSK_D0 - ceil((float)ugemm_kq_wg_tile_m / MSK_S.array[2]))
            && mask_aligned;
#endif
...
#pragma unroll
    ...
    for (int k0 = 0; k0 < k0end; k0 += ugemm_kq_wg_tile_m) {
        ...
        if (block_msk) {
            tile_load_block(
                    &mask_tile, msk, MSK_S.array[2], 0, k0 + sg_i0_kq, 0);
        } else {
            tile_load(&mask_tile, msk, k, 1, MSK_S.array[2], k0 + sg_i0_kq, 0);
        }
#else
        tile_load_t(&mask_tile, msk, q, k, MSK_S.array[2], sg_j0_kq + wg_j0,
                k0 + sg_i0_kq);
#endif
#endif
