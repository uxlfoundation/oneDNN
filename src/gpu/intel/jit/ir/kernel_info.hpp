...
public:
    ...
    void register_immediate_arg(
            const expr_t &var, const expr_t &value = expr_t()) {
        register_arg(
                var, arg_kind_t::immediate, DNNL_ARG_UNDEF, /*is_input=*/true);
    ...
    void register_user_hostscalar_arg(const expr_t &var, int dnnl_arg) {
    ...
    memory_storage_wrapper_t arg_storage(int idx, const exec_ctx_t &ctx,
        ...
        switch (args_[idx].kind) {
            ...
            case arg_kind_t::immediate: {
                if (!args_[idx].value.is_empty())
                    return memory_storage_wrapper_t();
                return CTX_IN_STORAGE(args_[idx].key);
            }
            default: gpu_error_not_expected();
        }
        return memory_storage_wrapper_t();
