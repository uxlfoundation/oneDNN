...
#define wu_scale_op(x) ((x) * wu_scale)
    ...
    tile_binary(S_WU_tile, S_WG_tile, binary_mul); // <-- DISABLE TO DEBUG
...
#ifdef BLOCK_DST
    tile_store_block_rem_wgu(S_tile_half, tmp_reduce_mem + k_offset, OC, lda,
            wg_i0 + sg_j0_wgu, wg_j0 + sg_i0_wgu);
#else
    tile_store(S_tile_half, tmp_reduce_mem + k_offset, MB, OC, lda,
            wg_i0 + sg_j0_wgu, wg_j0 + sg_i0_wgu);
#endif
}
