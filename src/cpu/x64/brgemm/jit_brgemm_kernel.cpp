...
private:
    ...
    void gemv_microkernel(dim_t bd_block2, bool is_bdb_tail, dim_t ld_block,
            bool is_rd_tail, bool is_ld_tail, dim_t vpad,
            dim_t rows_for_rd_tail);
...
template <typename Wmm>
void jit_brgemm_kernel_t<Wmm>::gemv_microkernel(dim_t bd_block2,
        bool is_bdb_tail, dim_t ld_block2, bool is_rd_tail, bool is_ld_tail,
        ...
        if (brg.is_tmm) {
            ...
            if (brg.rdb > 0) {
                ...
                L_aligned(rdb_loop_label, 64);
                {
                    const bool is_rd_tail = false;
                    if (brg.is_gemv)
                        gemv_microkernel(bd_block2, is_bdb_tail, ld_block2,
...
void jit_brgemm_kernel_t<Wmm>::generate() {
    ...
    if (brg.is_gemv && !isa_has_masks(brg.isa_impl) && brg.rdb_tail) {
        L(avx_rd_tail_mask_);
    ...
    if (!isa_has_masks(brg.isa_impl) && brg.ldb_tail) {
        L(avx_tail_mask_);
