...
status_t stream_t::init_async_tracking(std::string vinfo, double *start_ms) {
    stream_timing_data_ = new async_timing_data_t;
...
status_t stream_t::register_async_tracker(cl_event event) {
    ...
    // Check if there is any event in the queue - for default
    ...
    auto event_timing_data_ = new async_timing_data_t;
    event_timing_data_->start_ms = stream_timing_data_->start_ms;
    event_timing_data_->vinfo = stream_timing_data_->vinfo;

    event_timing_data_->profiler_enabled = is_profiling_enabled();
...
status_t stream_t::check_async_exec_times(std::string vinfo, double *start_ms) {
    ...
    if (!stream_timing_data_->timing_stat) {
        ...
        VPROF(static_cast<int>(stream_timing_data_->start_ms), primitive, exec,
                VERBOSE_profile, stream_timing_data_->vinfo.c_str(),
                duration_ms);
    }

    delete stream_timing_data_;
    stream_timing_data_ = nullptr;
