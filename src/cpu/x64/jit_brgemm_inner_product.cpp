...
status_t brgemm_inner_product_fwd_t<isa>::execute_forward(
    ...
    const auto ker = [= COMPAT_THIS_CAPTURE](dim_t ithr_oc_mb, dim_t nthr_oc_mb,
                             dim_t ithr_ic, dim_t osb, dim_t osb_s, dim_t ocb,
                             dim_t ocb_s, dim_t icc, dim_t icc_s, dim_t kd,
                             dim_t kh, dim_t kw, bool copy_buffer_a,
    ...
    parallel(static_cast<int>(num_threads), [=](const int ithr, const int nthr) {
        ...
        while (work < end) {
            ...
            while (loop_start < loop_end) {
                ...
                for_(dim_t kd = 0; kd < jbgp.kd; kd++)
                for_(dim_t kh = 0; kh < jbgp.kh; kh++)
                for (dim_t kw = 0; kw < jbgp.kw; kw++) {
    ...
    if (jbgp.nthr_ic_b > 1) {
        const auto get_dst_reduced_off = [=](dim_t ithr_ic, dim_t osb, dim_t ocb) {
            ...
            const dim_t ic_buf_idx = ithr_ic - acc_buffer_idx_shift;
        ...
        parallel(static_cast<int>(num_threads),
            ...
            while (start < end) {
                ...
                for (dim_t osb = osb_s; osb < osb_e; ++osb) {
                    ...
                    for (dim_t ic_buf = 0; ic_buf < nthr_ic - 1; ++ic_buf) {
                        const char *c_buffer = c_buffer_global
                                + get_dst_reduced_off(ic_buf + 1, osb, ocb_s);
                        for (dim_t os = 0; os < cur_os_block; ++os) {
                            acc_ker_->accumulate(
...
void brgemm_inner_product_bwd_data_t<isa>::execute_backward_data(
    ...
    const auto get_weights_ptr
            = [=](dim_t icb, dim_t ocb, dim_t kd = 0, dim_t kh = 0,
                      dim_t kw = 0) {
        dim_t fwd_ic_block
    ...
    const auto ker
            = [= COMPAT_THIS_CAPTURE](dim_t ithr_ic_mb, dim_t nthr_ic_mb,
                      dim_t ithr_oc, dim_t nthr_oc, dim_t n, dim_t icb,
                      dim_t occ, dim_t kd, dim_t kh, dim_t kw, bool do_init,
    ...
    parallel(static_cast<int>(num_threads), [=](const int ithr, const int nthr) {
        ...
        while (work < end) {
            ...
            for (dim_t iter = 0; iter < loop_iteration; ++iter) {
    ...
    if (jbgp.nthr_oc_b > 1) {
        parallel(static_cast<int>(num_threads),
            ...
            const dim_t reduce_chunk_size = 64;
            dim_t start {0}, end {0};
            ...
            dim_t oc_buf_idx = !is_f32_out;
            dim_t oc_buf_end = is_f32_out;
            dim_t n_accs = n_oc_bufs - oc_buf_end;
            for (dim_t oc_buf = oc_buf_idx; oc_buf < n_accs; oc_buf++) {
...
char *brgemm_inner_product_bwd_weights_t<isa>::get_wei_acc_ptr(
        ...
        dim_t kw, dim_t reduction_buf_idx) const {
    const auto &jbgp = pd()->jbgp_;

    const dim_t reduction_buf_start_idx = jbgp.wei_dt == f32;
...
void brgemm_inner_product_bwd_weights_t<isa>::compute_diff_weights_and_bias(
    ...
    const auto get_bia_acc_ptr = [&](dim_t oc) {
        const dim_t reduction_buf_start_idx = jbgp.bia_dt == f32;
    ...
    const auto ker = [&](const dim_t sp_icc, const dim_t osc, const dim_t icc,
                             const dim_t occ, const dim_t icb_i,
                             const dim_t ocb_i, const dim_t osc_prev,
                             const dim_t sp_icc_prev, const dim_t occ_prev,
                             dim_t kd, dim_t kh, dim_t kw, int &prev_ker_idx) {
        brgemm_batch_element_t *addr_batch
        ...
        if (nb_os_b > 0 && brg_kernel != nullptr) {
            ...
            for (dim_t os_block = 0; os_block < nb_os_b; os_block++) {
    ...
    dim_t occ_idx = 0, sp_icc_idx = 0, osc_idx = 0;
    ...
    while (loop_idx < loop_end) {
        ...
        for_(dim_t ocb_i = 0; ocb_i < ocb_work; ocb_i++)
        for (dim_t icb_i = 0; icb_i < icb_work; icb_i++) {
...
void brgemm_inner_product_bwd_weights_t<
    ...
    dim_t reduce_buf_idx_start = !is_f32_out;
    ...
    for (dim_t ir = reduce_buf_idx_start; ir < reduce_buf_idx_end; ++ir) {
        dim_t counter = start;
    ...
    if (jbgp.with_bias && ti->ithr_sp_ic_c == 0 && ti->sp_ic_c_work > 0
        ...
        dim_t reduce_buf_idx_start = !is_f32_bias;
