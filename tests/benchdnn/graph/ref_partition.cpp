...
int ref_partition_t::init_graph_mem(
    ...
    for (const auto &id : partition_out_ids_) {
        // For output memories of graph, they need to be in compliance with the
        // reference memories regarding the shapes and memory tags, as the
        // memories of both paths will be reordered to abx for comparison.
