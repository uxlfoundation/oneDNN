...
#else
    block_write(&reduce_temp[group_c_offset + mb_sp_idx * 16], &sum.s0, 1);
...
__kernel void xe_calc_mean(__global DATA_T *src, __global float *reduce_temp,
    ...
    if (sp_block_idx == STAT_SP_TAIL) {
        ...
        while (sp >= 16) {
            ...
            if (is_last_sp && is_last_ic_block) {
                ...
                s0[7] = simd_id < 8 ? into_float(src[7 * IC + simd_id])
                                    : 0.0f;
                LOAD_DATA_Nx16_USING_LOOP_IDX(7, s1, src, 8);
                s1[7] = simd_id < 8 ? into_float(src[15 * IC + simd_id])
                                    : 0.0f;
            } else {
    ...
    block_write(&reduce_temp[group_c_offset + mb_sp_idx * 16], &v_mean, 1);
#endif
}
...
#else
    // reduce_temp is padded to PADDED_IC, no OOB writes
    block_write(&reduce_temp[group_c_offset + mb_sp_idx * 16], &v_var, 1);
#endif
}
...
inline float8 read_src_block(__global DATA_T *src, int c, int sp) {
...
#endif // HAS_SP_TAIL
    {
        ...
        if (is_last_ic_block && is_last_sp_block) {
            ...
            blockS0[7] = simd_id < 8 ? into_float(src[7 * IC + simd_id])
                                     : 0.0f;
        } else {
...
__kernel void xe_bnorm_fwd(__global DATA_T *src, __global float *mean,
    ...
    if (sp == SP_TAIL) {
        for (int k = 0; k < SP - SP_TAIL; ++k) {
            ...
            block_write(&ws[k * IC_BLOCK_STRIDE], &ws_val, 1);
        }
    } else
#endif // HAS_SP_TAIL
    {
    ...
    if (sp == SP_TAIL) {
        for (int k = 0; k < SP - SP_TAIL; ++k) {
            ...
            if (is_last_ic_block) {
                if (simd_id < 8)
                    write(dst + k * IC_BLOCK_STRIDE + simd_id, blockD0[k]);
            } else
#endif
                block_write(&dst[k * IC_BLOCK_STRIDE], &blockD0[k], 1);
        }
    } else
#endif // HAS_SP_TAIL
    {
