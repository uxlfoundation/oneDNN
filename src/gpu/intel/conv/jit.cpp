...
// Returns the number of registers for a config. The pd-time config has no GRF
// mode set yet, so repeating init_regs() logic here.
...
private:
    ...
    static status_t init_reorder(T *pd, const impl::engine_t *engine,
        ...
        // Keep the GRF mode in sync with the convolution kernel to avoid switch.
        primitive_attr_t attr;
