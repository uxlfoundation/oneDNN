...
struct base_settings_t {
    struct settings_attributes_t {
        ...
        void init(const std::vector<attr_t::arg_scales_t> &scales,
                ...
                const std::vector<attr_t::placeholder_t> &placeholder,
            ...
            for_(const auto &pl : placeholder)
            ...
            for (const auto &rm : rounding_mode)
                attrs_.push_back(
                        get_attr(s, zp, pl, po, sm, fm, am, d, dr, rm));
        }
    ...
    std::vector<attr_t::placeholder_t> placeholder {attr_t::placeholder_t()};
    ...
    virtual bool has_single_setup() const {
        return mb.size() == 1 && inplace.size() == 1 && scales.size() == 1
                && zero_points.size() == 1 && placeholder.size() == 1
    ...
    virtual void finalize() {
        attributes.clear();
        attributes.init(scales, zero_points, placeholder, post_ops,
