...
bool flex_rewrite_t::get_inport_shape_stride(const std::string &in_shape,
    ...
    if (delimiter_pos != std::string::npos) {
        ...
        if (all_letters(mtag_or_stride)) // --in-shapes=*abdc
            mtag = mtag_or_stride;
        else if (all_digit_cross(mtag_or_stride)) //--in-shapse=*1x16x4
            stride = mtag_or_stride;
        else
            return false;
        return true;
    } else if (all_letters(in_shape)) { // user provide a new memory tag
        mtag = in_shape;
        return true;
    } else if (all_digit_cross(in_shape)) { // user provide a new shape
        shape = in_shape;
        return true;
    ...
    for (auto &lt : aop.in_lts_) {
        ...
        if (in_shapes_.find(lt.id_) != in_shapes_.end()
            ...
            if (!new_mtag.empty()) {
                ...
                if (!is_dense_memory(new_stride_dims, lt.shape_, tmp_mtag)) {
...
void flex_rewrite_t::update_output_info(deserialized_op_t &aop,
    ...
    std::string dominate_tag = dgraph.lt_2_mtag_[aop.in_lts_.front().id_];
