...
private:
    ...
    bool check_fp4_ok(const context_t &ctx) const {
        auto prb = cfg_.prb();
        if(utils::one_of(prb.src_data_type, data_type_t::dnnl_f4_e2m1, data_type_t::dnnl_f4_e3m0) 
            || utils::one_of(prb.wei_data_type, data_type_t::dnnl_f4_e2m1, data_type_t::dnnl_f4_e3m0)){
            for (auto &d : ctx.blk.iter())
                for (auto t : input_tensors(cfg_.prb())) {
                    if (tensor_conv_dim_index(d, t) == -1) continue;
                    if (inner_stride(cfg_, t, d) == 1 && inner_block(cfg_, d) < 8) return false;
                }
        }
        return true;
    }
