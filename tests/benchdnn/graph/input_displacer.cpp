...
partition_data_displacer_t::partition_data_displacer_t(
    ...
    for (const auto &aop : dg_->ops_) {
        ...
        while (aop.kind_ == "Add") {
            ...
            // The bottom-right implicit casual mask handles future tokens
            // differently compared with the top-left casual mask. To support
            // it, the result of GenIndex on rows should substract query
            // seq_len and add kv seq_len to generate masks such as:
