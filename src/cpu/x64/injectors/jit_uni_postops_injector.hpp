...
dim_t aux_vec_count(const post_ops_t &post_ops, cpu_isa_t isa, bool is_fwd);
...
public:
    ...
    // @rhs_arg_params: see jit_uni_binary_injector description
    virtual void compute_vector_range(dim_t start_idx, dim_t end_idx,
            const binary_injector::rhs_arg_dynamic_params_t &rhs_arg_params)
            = 0;
    virtual void compute_vector_range(dim_t start_idx, dim_t end_idx) = 0;

    // Generates code of post_ops chain injected to host primitive. Applied to
    // a single vector register index.
    // @rhs_arg_params: see jit_uni_binary_injector description
    virtual void compute_vector(dim_t idx,
            const binary_injector::rhs_arg_dynamic_params_t &rhs_arg_params)
            = 0;
    virtual void compute_vector(dim_t idx) = 0;
...
public:
    ...
    // See `jit_uni_postops_injector_base_t::compute_vector_range(...)`
    void compute_vector_range(dim_t start_idx, dim_t end_idx,
            const binary_injector::rhs_arg_dynamic_params_t &rhs_arg_params)
            override;

    void compute_vector_range(dim_t start_idx, dim_t end_idx) override;

    // See `jit_uni_postops_injector_base_t::compute_vector(...)`
    void compute_vector(dim_t idx,
            const binary_injector::rhs_arg_dynamic_params_t &rhs_arg_params)
            override;
    void compute_vector(dim_t idx) override;
