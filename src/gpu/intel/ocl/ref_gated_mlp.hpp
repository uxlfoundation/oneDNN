...
struct ref_gated_mlp_t : public primitive_t {
    ...
    struct pd_t : public gated_mlp_pd_t {
        ...
        status_t init(impl::engine_t *engine) {
            ...
            //VDISPATCH_GATED_MLP(attr()->has_default_values(smask_t::scales),
    ...
    status_t init(impl::engine_t *engine) override {
        ...
        //kernel_ctx.define_int("SIZE_MB", pd()->desc()->src_desc.dims[0]);
        //TODO: STF here in context, or just kernel arg? ^^ above for nregisters, batch size relatively stable
        //kernel_ctx.define_int("SIZE_IC", pd()->desc()->src_desc.dims[1]);
        kernel_ctx.define_int("SIZE_OC", pd()->desc()->W_gate_desc.dims[1]);
