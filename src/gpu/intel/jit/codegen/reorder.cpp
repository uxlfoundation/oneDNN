...
copy_operand_t &copy_operand_t::advance(
    ...
    const auto bit_off = (offset + elems) * type_bit_size * stride;
...
void copy_plan_t::mov(int simd, ngen::InstructionModifier mod,
    ...
    while (simd > 0) {
        ...
        append(phase_, mov, block_simd, mod, block_dst, block_src);
...
reorder_2d_impl_t::reorder_2d_impl_t(ngen::HW hw, tensor_t tile,
...
void reorder_2d_impl_t::emit(
    ...
    for (int i = 0; i < path_len; i++) {
        ...
        auto swizzle = [&](const start_t &start) {
            ...
            auto src_off = src_layout(start) * orig_bits / type_bits;
            auto dst_off = dst_layout(start) * orig_bits / type_bits;
...
void reorder_2d_impl_t::tile_to_2d_dims(const tensor_t &tile, dim_idx_t &a_idx,
...
bool reorder_2d_impl_t::vertex_t::can_reorder(
        const tensor_t &tile, const type_t &type) const {
...
void reorder_impl_t::emit_1d(copy_plan_t &plan, const reorder_operand_t &dst,
    ...
    dst.layout.for_each_tile(tile, [&](const start_t &start) {
        // Tile operands
        auto tile_src = src.buffer, tile_dst = dst.buffer;
        tile_src.stride = (uint8_t)src_stride;
        tile_dst.stride = (uint8_t)dst_stride;
        tile_src.advance(hw_, src.layout(start));
        tile_dst.advance(hw_, dst.layout(start));
...
std::vector<tensor_t> reorder_impl_t::find_2d_dense_tiles(
    ...
    auto equal_tiles = [](const tile_pair_t &p) { return p[0].is_equal(p[1]); };
...
tensor_t reorder_impl_t::find_max_tile_with_fixed_stride(const layout_t &src,
