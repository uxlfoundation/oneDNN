...
status_t micro_horz_t::pd_t::init(impl::engine_t *engine) {
    VDISPATCH_GATED_MLP(!gpu_utils::dev_getenv("skip_gmlp_horz_ukern", false),
    ...
    // init fc_down gemm; to be removed after vertical fusion gets implemented
...
status_t micro_horz_t::pd_t::init_microkernels(impl::engine_t *engine) {
    ...
    problem.Tc_ext = gemmstone::Type::f16;
    ...
    auto ldgu = int(gemm_desc_t::get_ld(*W_gate_mdw.md_)
    ...
    if (with_wts_gate_scales(this) && !wgu_common_scales) {
        ...
        problem_wgu.Ta_scale = gemm::jit::convert_dnnl_to_kernel_type(
        ...
        problem_wgu.A_scale.layout = gemmstone::MatrixLayout::
    ...
    reqs_wgu.push_back(gemmstone::StrategyRequirement::UnrollM
            == config->unroll_m_gwu); //affects A
    reqs_wgu.push_back(gemmstone::StrategyRequirement::UnrollN
            == config->unroll_n_gwu); //affects B
...
status_t micro_horz_t::init(impl::engine_t *engine) {
    ...
    auto lda = gemm_desc_t::get_ld(*dst_mdw.md_)
    ...
    if (with_wts_up_scales(pd()) || with_wts_gate_zp(pd()))
    ...
    //pd()->gemm_gate_up_pkg(), HostLanguage::OpenCL_C, shimOptions);
...
status_t micro_horz_t::execute(const exec_ctx_t &ctx) const {
    ...
    auto wg_tile_OC = gemm_gate_up_pkg.getSetting(
            "wg_tile_m"); //TODO: which? OC along lda should get largest tile dim?
    //auto wg_tile_OC = gemm_gatei_up_pkg.getSetting("wg_tile_n"); //TODO: which? OC along lda should get largest tile dim?
    ...
    //size_t k_split = IC / gemm_gate_up_pkg.getSetting("wg_tile_m");
    //gws[1] *= k_split;

    auto nd_range = compute::nd_range_t(gws, lws);
    //printf("gws[%zu %zu %zu] lws[%zu %zu %zu] \n",gws[0], gws[1], gws[2], lws[0], lws[1], lws[2]);
    status_t gateup_status
            = parallel_for(ctx, nd_range, gemm_gate_up_, arg_list);

    if (gateup_status != status::success) return gateup_status;

    // do fc_down gemm; to be removed after vertical fusion gets implemented
