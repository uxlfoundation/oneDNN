...
template <cpu_isa_t isa>
void jit_io_helper_v2_t<isa>::init_saturation(data_type_t dt, bool force) {
    if (!force
            && (!types::is_integral_dt(dt) || saturation_prepared_dt_ == dt)) {
        return;
    }
    ...
    saturation_prepared_dt_ = dt;
}

template <cpu_isa_t isa>
void jit_io_helper_v2_t<isa>::init_tail(bool force) {
    if (!force && is_tail_ready_) { return; }
...
template <cpu_isa_t isa>
bool jit_io_helper_v2_t<isa>::is_tail_ok(bool is_tail) const {
    return IMPLICATION(is_tail, is_tail_ready_);
}

template <cpu_isa_t isa>
bool jit_io_helper_v2_t<isa>::is_saturation_ok(data_type_t dt) const {
    return IMPLICATION(types::is_integral_dt(dt),
            saturation_prepared_dt_ != data_type::undef);
}
