...
#undef PARAM_PTR
    ...
    void zeroise() {
        ...
        L(label_zeroise);
        {
            ...
            add(reg_off_c_, static_cast<uint32_t>(simd_w * acc_type_size_));
            dec(reg_C_);
            jnz(label_zeroise);
        }
    }
    ...
    void load_stat(bool compute_mean, const int c_blks_to_unroll = 1) {
        ...
        const int step = static_cast<int>(simd_w * acc_type_size_);
    ...
    void compute_stat(bool compute_mean, const int c_blks_to_unroll = 1) {
        ...
        const int step = static_cast<int>(simd_w * data_type_size_);
    ...
    void compute_blocked(bool compute_mean) {
        ...
        L(label_C);
        {
            ...
            L(label_S);
            {
                ...
                add(reg_off_dat_, static_cast<uint32_t>(stride_S_ * data_type_size_));
            ...
            add(reg_off_dat_save_, static_cast<uint32_t>(stride_C_ * data_type_size_));
            add(reg_off_c_, static_cast<uint32_t>(simd_w * acc_type_size_));
        ...
        for (int c_blks_to_unroll = max_of_unrolled_c_blks;
            ...
            L(c_unroll_label[c_blks_to_unroll]);
            {
                ...
                L(label_S);
                {
                    ...
                    add(reg_off_dat_,
                            static_cast<uint32_t>(stride_S_ * data_type_size_));
                ...
                add(reg_off_dat_save_,
                        static_cast<uint32_t>(c_blks_to_unroll * stride_C_
                                * data_type_size_));
    ...
    void compute(bool compute_mean) {
        ...
        L(label_N);
        {
            ...
            add(reg_ptr_src_, static_cast<uint32_t>(stride_N_ * data_type_size_));
            dec(reg_N_);
            jnz(label_N);
        }
    }
    ...
    void normalize() {
        ...
        const int S = static_cast<int>(pd_->D() * pd_->H() * pd_->W());
...
#undef PARAM_PTR
    ...
    void load_two_c_mean_sqrtvar() {
        const int offt = static_cast<int>(simd_w * acc_type_size_);
        jit_tail_.uni_vmovups_maybe_tail(
    ...
    void compute_bnorm_avx2_ne_xf16(
        ...
        if (is_c_blks_tail) {
            ...
            load_c_specifics(true, static_cast<int>(simd_w * acc_type_size_));
            compute_bnorm(vsrc_odd, vmean_odd_, vsqrtvar_odd_,
                    stream_store_allowed, true,
                    static_cast<int>(stride_C_ * data_type_size_));
        }
    }
    ...
    void compute_avx2_ne_xf16(bool stream_store_allowed) {
        ...
        L(label_C);
        {
            ...
            add(reg_off_dat_save_,
                    static_cast<uint32_t>(2 * stride_C_ * data_type_size_));
            add(reg_off_c_, static_cast<uint32_t>(2 * simd_w * acc_type_size_));
    ...
    void compute(bool stream_store_allowed) {
        ...
        L(label_N);
        {
            ...
            add(reg_ptr_src_, static_cast<uint32_t>(stride_N_ * data_type_size_));
            add(reg_ptr_dst_, static_cast<uint32_t>(stride_N_ * data_type_size_));
            if (jit_relu_.with_relu_ && !jit_relu_.with_relu_inf_only_)
...
#undef PARAM_PTR
    ...
    void compute_nspc(bool stream_store_allowed) {
        ...
        L(label_S);
        {
            ...
            L(label_C);
            {
                ...
                add(reg_off_c_, static_cast<uint32_t>(simd_w * acc_type_size_));
                add(reg_off_dat_, static_cast<uint32_t>(stride_C_ * data_type_size_));

                dec(reg_C_);
                jnz(label_C);
            }

            add(reg_off_dat_save_, static_cast<uint32_t>(stride_S_ * data_type_size_));
    ...
    void compute(bool stream_store_allowed) {
        ...
        L(label_N);
        {
            ...
            add(reg_ptr_src_, static_cast<uint32_t>(stride_N_ * data_type_size_));
            add(reg_ptr_diff_src_, static_cast<uint32_t>(stride_N_ * data_type_size_));
            add(reg_ptr_diff_dst_, static_cast<uint32_t>(stride_N_ * data_type_size_));
            add(reg_ptr_ws_, stride_N_ / bits_per_byte);
...
public:
    ...
    static void init_scratchpad(memory_tracking::registrar_t &scratchpad,
        ...
        int C_PADDED = static_cast<int>(get_c_padded(pd));
    ...
    void exec_fwd_step_stats(const dim_t C_blks, const bnorm_dims_t &nthr,
        ...
        parallel(static_cast<int>(nthr.glob),
            ...
            const int ithr_NS
                    = static_cast<int>(ithr.N * nthr.S + ithr.S);
...
private:
    ...
    bnorm_dims_t map_thread(int ithr_glob, const bnorm_dims_t &nthr) {
        ...
        ithr.glob = ithr_glob;
        ithr.C = map_thread_c(static_cast<int>(ithr.glob), nthr);
    ...
    void work_distribution(dim_t C_blks, const bnorm_dims_t &ithr,
        ...
        work_distribution_c(
                C_blks, static_cast<int>(ithr.C), static_cast<int>(nthr.C), start.C, stop.C);
