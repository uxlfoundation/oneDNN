...
#undef scaled_exp
        ...
        if (sg_ij < sg_per_wg_BcD) {
            dV_tile1 = ugemm_vs(dA + q0 * lda, lda, (local FMA_TYPE *)S_slm,
                    ugemm_kq_wg_tile_m, d, k_chunk, q_nchunk, 0, 0, 0, sg_i_vs,
...
#else
        tile_load_packed_src1(&S2_tile_reblock, S_slm, ugemm_kq_sg_tile_n,
...
#if WITH_DS
        p_tile_type_reblock dP_tile_reblock;
        tile_copy_reblock(dP_tile, &dP_tile_reblock);
        tile_store(dP_tile_reblock, dS, k_chunk, q_nchunk, k, k0 + sg_i0_kq,
                q0 + sg_j0_kq);
#endif
...
#pragma unroll
        ...
        barrier(CLK_LOCAL_MEM_FENCE);
#else
        tile_load_packed_src1(&dS_transpose_tile, dS_slm, ugemm_kq_sg_tile_n,
                ugemm_kq_wg_tile_n, sg_i0_kq, sg_j0_kq);
        barrier(CLK_LOCAL_MEM_FENCE);
        tile_store_t_packed_src1(dS_transpose_tile, dS_slm, ugemm_kq_sg_tile_n,
