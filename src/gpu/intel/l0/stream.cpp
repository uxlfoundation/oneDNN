...
status_t stream_impl_t::copy(const impl::memory_storage_t &src,
        const impl::memory_storage_t &dst, size_t size, const xpu::event_t &dep,
        xpu::event_t &out_dep) {
    auto l0_dep
            = utils::downcast<const gpu::intel::l0::event_t *>(&dep)->events;

    auto l0_out_dep = utils::downcast<const gpu::intel::l0::event_t *>(&out_dep)
                              ->events;

    CHECK(func_zeCommandListAppendMemoryCopy(list_, dst.data_handle(),
            src.data_handle(), size, *l0_out_dep.data(),
            static_cast<uint32_t>(l0_dep.size()), l0_dep.data()));
...
status_t stream_impl_t::fill(const impl::memory_storage_t &dst, uint8_t pattern,
    ...
    auto l0_deps
            = utils::downcast<const gpu::intel::l0::event_t *>(&deps)->events;

    auto l0_out_dep = utils::downcast<const gpu::intel::l0::event_t *>(&out_dep)
                              ->events;

    CHECK(func_zeCommandListAppendMemoryFill(list_, dst.data_handle(), &pattern,
            sizeof(pattern), size, *l0_out_dep.data(),
