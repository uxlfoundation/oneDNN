...
double kernel_execute_work(const kai_wino_convolution_fwd_t::pd_t &pd) {
    const auto &args = *pd.wino_impl_->gemm_args;
    return static_cast<double>(args._Msize) * args._Nsize * args._Ksize
            * args._Ksections * args._nbatches * args._nmulti;
}
...
kai_wino_convolution_fwd_t::pd_t::create_kai_gemm(int max_threads) const {
    return kai_utils::create_kai_gemm(*wino_impl_->gemm_args, cfg_.get(),
            src_md()->data_type, weights_md()->data_type, dst_md()->data_type,
            max_threads);
}

status_t kai_wino_convolution_fwd_t::pd_t::init(const engine_t *engine) {
...
status_t execute_wino_forward(const kai_wino_convolution_fwd_t::pd_t *pd,
    ...
    auto *kernel_dst_base = pd->use_dst_reorder_
            ...
            : dst_base;
    auto *post_ops_src = post_ops.has_sum()
    ...
    const int transform_num_threads
            = make_thread_partition(max_threads, window_size).nthr;
    const auto kernel_thread_partition
            = make_thread_partition(requested_kernel_threads, window_size);
    kernel->set_nthreads(kernel_thread_partition.nthr);
    ...
    if (!pd->fixed_format_) {
        parallel(transform_num_threads, [&](int ithr, int nthr) {
    ...
    if (pd->run_weight_reorder_) {
        const double weight_work
                = static_cast<double>(wds.weight_matrix_size_bytes)
                / sizeof(data_t);
        const int reorder_num_threads = threads_for_weight_reorder(
                weight_work, transform_num_threads);
        parallel_pretranspose_B_array(*kernel, pretransposed_weights,
                scratch_wino_weights, static_cast<int>(wds.weight_ld_row),
    ...
    parallel_execute(*kernel, window_size, kernel_thread_partition);
...
status_t kai_wino_convolution_fwd_t::execute(const exec_ctx_t &ctx) const {
    if (pd()->src_md()->data_type == data_type::f32)
        return execute_wino_forward<float>(
                pd(), ctx, src_reorder_, dst_reorder_, post_ops_);
