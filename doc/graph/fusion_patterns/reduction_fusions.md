Reduction Fusions {#dev_guide_graph_reduction_fusions}
===========================================================

## Overview

The Reduction category includes operations such as: [ReduceL1](@ref dev_guide_op_reducel1)
...
[ReduceProd](@ref dev_guide_op_reduceprod), [ReduceSum](@ref dev_guide_op_reducesum).
...
oneDNN supports Reduction and its optimization through Graph API [1] by
defining the graph, getting partition from the graph, and optimizing the kernels
underneath. In general, a Reduction pattern is defined as a directional acyclic
graph (DAG) using oneDNN Graph API.

### Floating-point Reduction patterns

oneDNN defines floating-point (f32, bf16, or f16) Reduction patterns as follows.
The blue parts are required when defining a Reduction pattern while the brown
parts are optional.

![Reduction pattern](images/reduction_pattern.png)

1. The Reduction performs corresponding Reduction operation for src tensor.
...
2. The post-op subgraph is optional and can be constructed with the following operations:
   ...
   1. 1 to 4 Reduction/unary operations are supported.

## Data Types

oneDNN supports the floating-point Reduction pattern with data types f32,
...
in a floating-point Reduction pattern.
