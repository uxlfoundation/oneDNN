...
status_t stream_t::run_verbose_profiler(
    ...
    auto payload = std::make_shared<payload_t>(
            payload_t {this, pd_info, start_ms, last_entry});
        ...
        cgh.host_task([payload]() {
            ...
            if (num_entries == 0) {
                VWARN(primitive, exec,
                        ...
                        "kernel events",
                        payload->info_str.c_str());
            }

            std::vector<uint64_t> timing_data(num_entries);
            payload->s->get_profiling_data(profiling_data_kind::time,
                    &num_entries, timing_data.data());

            double duration_ms = 0.0;
            for (int i = payload->lasti; i < num_entries; i++) {
