...
status_t stream_impl_t::copy(const impl::memory_storage_t &src,
    ...
    utils::downcast<const event_t *>(&deps)->get_l0_events(l0_deps);

    auto l0_engine = utils::downcast<engine_t *>(dst.engine());
    event_wrapper_t out_event(l0_engine->context());

    CHECK(func_zeCommandListAppendMemoryCopy(list_, dst.data_handle(),
            src.data_handle(), size, out_event.get(),
            static_cast<uint32_t>(l0_deps.size()),
            l0_deps.size() ? l0_deps.data() : nullptr));
    utils::downcast<event_t *>(&out_dep)->events_.emplace_back(
            std::move(out_event));
...
status_t stream_impl_t::fill(const impl::memory_storage_t &dst, uint8_t pattern,
    ...
    CHECK(func_zeCommandListAppendMemoryFill(list_, dst.data_handle(), &pattern,
            sizeof(pattern), size, out_event.get(),
            static_cast<uint32_t>(l0_deps.size()),
            l0_deps.size() ? l0_deps.data() : nullptr));
    utils::downcast<event_t *>(&out_dep)->events_.emplace_back(
            std::move(out_event));
