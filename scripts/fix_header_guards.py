...
def get_relpath(fullpath):
    ...
    for segment in path_segments:
        ...
        return segment + tail
    raise ValueError(f"not a source code file: {fullpath}")
...
def fix_file(file, options):
    ...
    if ext not in (".hpp", ".h"):
        return 0
    try:
        relpath = get_relpath(fullpath)
    except ValueError:
        return 0
    if ignore(relpath):
        return 0
    guard = get_file_guard(relpath)
    status = adjust_content(fullpath, guard)
    if not status.ok() or options.verbose:
        print(f"[{status.color(sys.stdout)}] {relpath} ({status!s})")
...
def adjust_content(fullpath: str, guard: str):
    ...
    try:
        ...
        return status.add(FileStatus.WARN, str(e))
    dirname = os.path.dirname(relpath)
    path_parts = dirname.split("/")
    basename, ext = os.path.splitext(os.path.basename(relpath))
    ...
    if options.pedantic and common_parts:
        ...
        new_relpath = dirname + "/" + "_".join(new_name_parts) + ext
        message = f"consider renaming {relpath}"
        if new_name_parts:
            message += f" to {new_relpath}"
        elif path_parts[-1] in common_parts:
            message += f" to {dirname}{ext}"
        status.add(FileStatus.WARN, message)
    ...
    for start, end, raw_line in continuations(lines):
        ...
        if kind == "endif":
            try:
                ...
                return status.add(FileStatus.FAIL, "mismatched #ifs/#endifs")
    ...
    if len(blocks) != 1:
        return status.add(FileStatus.FAIL, "mismatched #ifs/#endifs")
    force_write, has_pragma_once = False, False
    ...
    if len(guards) == 1:
        if guard not in defines:
            ...
            if not options.inplace:
                return status.add(FileStatus.FAIL, "missing define")
            message = "added missing define"
        message = "correct guard found"
    elif len(guards) > 1:
        return status.add(FileStatus.FAIL, "too many guards")
    elif root is None:
        start = license_ends
        for block in blocks[0]:
            ...
            if no_code(lines[start:]):
                if not status.ok():
                    return status
                return status.add(FileStatus.OK, "no content")
        ...
        if not options.inplace:
            if has_pragma_once:
                status.add(FileStatus.FAIL, "uses #pragma once")
            return status.add(FileStatus.FAIL, "missing guard")
    elif root.open.args is not None:
        old_guard = ifndef_argument(root.open)
        if old_guard is None:
            return status.add(FileStatus.FAIL, "broken guard")
        ...
        if old_guard != guard:
            ...
            if not options.inplace:
                return status.add(FileStatus.FAIL, f"wrong guard: {old_guard}")
            message = f"fixed incorrect guard {old_guard}"
    else:
        return status.add(FileStatus.FAIL, "broken top-level guard")
    if options.inplace or force_write:
        with open(fullpath, "w") as fd:
            fd.write(content)
    if not status.ok():
        return status
    return status.add(FileStatus.OK, message)
...
options:
    ...
    -p    warn about repetitive parts in file names
