...
status_t brgemm_desc_set_postops(brgemm_desc_t *brg,
    ...
    bool wei_scales_are_set = one_of(true, brg->is_single_wei_scale,
            brg->is_oc_wei_scales, brg->is_ic_wei_scales);
    if (brg->with_wei_scales && !wei_scales_are_set) {
        ...
        brg->is_oc_wei_scales = (wei_scales_mask & 1 << /*ndims - 1*/ 1) > 0;
        brg->is_ic_wei_scales = (wei_scales_mask & 1 << /*ndims - 2*/ 0) > 0;
