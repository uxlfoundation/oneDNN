...
dim_t jit_uni_eltwise_injector_t<Wmm>::get_stack_vmm_space() {
...
void jit_uni_eltwise_injector_t<Wmm>::log_compute_vector_fwd(
    ...
    auto gather_table_values
        ...
        if (has_avx512_core_) {
            ...
            for (int i = 0; i < vlen_ / static_cast<dim_t>(sizeof(float));
                    ++i) {
                h->mov(reg_tmp.cvt32(),
...
void jit_uni_eltwise_injector_t<Wmm>::pow_compute_vector_fwd(
    ...
    if (beta_ == -1) { // alpha / x
        ...
        for (int i = 0; i < vlen_ / static_cast<dim_t>(sizeof(float)); ++i) {
