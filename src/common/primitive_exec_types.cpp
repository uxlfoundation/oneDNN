...
exec_ctx_t::exec_ctx_t(const exec_ctx_t &other, exec_args_t &&args)
    : impl_(std::make_shared<exec_ctx_impl_t>(other.stream(), std::move(args),
...
void *exec_ctx_t::host_ptr(const memory_storage_t *mem_storage,
        bool use_mem_storage_handle) const {
    return impl_->host_ptr(mem_storage, use_mem_storage_handle);
}
...
void *exec_ctx_impl_t::host_ptr(const memory_storage_t *mem_storage,
        bool use_mem_storage_handle) const {
    ...
    if (memory_mapping_.count(handle) > 0) {
        ...
        base_ptr = use_mem_storage_handle ? handle : nullptr;
    }
    return base_ptr;
}
