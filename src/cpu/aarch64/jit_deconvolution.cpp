...
status_t create_fwd_convolution_desc(
    ...
    VDISPATCH_DECONVOLUTION_IC(
            deconv_desc->strides[0] == 1 && deconv_desc->strides[1] == 1,
     ...
     */
    padding_l[0]
            = (deconv_pd.KH() - 1) * (deconv_pd.KDH() + 1) - deconv_pd.padT();

    padding_r[0]
            = (deconv_pd.KH() - 1) * (deconv_pd.KDH() + 1) - deconv_pd.padB();
     ...
     */
    padding_l[1]
            = (deconv_pd.KW() - 1) * (deconv_pd.KDW() + 1) - deconv_pd.padL();

    padding_r[1]
            = (deconv_pd.KW() - 1) * (deconv_pd.KDW() + 1) - deconv_pd.padR();

    CHECK(conv_desc_init(conv_desc, deconv_desc->prop_kind,
            alg_kind::convolution_direct, deconv_pd.src_md(),
            deconv_pd.weights_md(), &deconv_desc->bias_desc, deconv_pd.dst_md(),
            deconv_desc->strides, deconv_desc->dilates, padding_l, padding_r));
 ...
 */
bool is_compatible_convolution(const jit_deconvolution_fwd_t::pd_t &deconv_pd,
    ...
    if (candidate_src_md == nullptr || candidate_weights_md == nullptr
            || candidate_dst_md == nullptr)
        return false;

    if (memory_desc_wrapper(*deconv_pd.src_md())
            != memory_desc_wrapper(*candidate_src_md))
        return false;

    if (memory_desc_wrapper(*deconv_pd.weights_md())
            != memory_desc_wrapper(*candidate_weights_md))
        return false;

    if (memory_desc_wrapper(*deconv_pd.dst_md())
            != memory_desc_wrapper(*candidate_dst_md))
        return false;

    if (deconv_pd.with_bias()) {
        const memory_desc_t *candidate_bias_md = candidate->weights_md(1);

        if (candidate_bias_md == nullptr) return false;

        if (memory_desc_wrapper(*deconv_pd.invariant_bia_md())
                != memory_desc_wrapper(*candidate_bias_md))
            return false;
    }

    /*
     * gemm_convolution_fwd_t does GEMM (beta = 1) sum then bias. Deconv needs
     * bias then sum. So for this case just skip the gemm_convolution_fwd_t candidate
     */

    if (deconv_pd.with_bias() // has bias
            && deconv_pd.attr()->post_ops_.find(primitive_kind::sum)
                    != -1 // has sum post-op
            && dynamic_cast<const gemm_convolution_fwd_t::pd_t *>(
                       candidate.get())
                    != nullptr) // is a gemm_convolution_fwd_t candidate
        return false;
...
bool needs_spatial_inversion(const memory_desc_t *weights_md) {
    ...
    constexpr int kh_dim = 2; // [OC, IC, KH, KW]
 ...
 *
 *     [OC, IC, KH, KW] see commons deconv line 125
 *
 * Channel coordinates are preserved while the two spatial coordinates are
 * reversed:
 *
 *     dst[oc, ic, kh, kw] = src[oc, ic, KH - 1 - kh, KW - 1 - kw]
 *
...
status_t invert_weights(
        const memory_desc_t *weights_md, const void *src, void *dst) {

    if (weights_md == nullptr || src == nullptr || dst == nullptr)
        return status::invalid_arguments;

    if (weights_md->ndims != 4) return status::invalid_arguments;

    if (memory_desc_matches_one_of_tag(*weights_md, format_tag::hwio)
            != format_tag::hwio)
        return status::invalid_arguments;

    const size_t element_size = types::data_type_size(weights_md->data_type);

    if (element_size == 0) return status::invalid_arguments;
    ...
    const dim_t OC = weights_md->dims[0];
    const dim_t IC = weights_md->dims[1];
    ...
    const size_t spatial_plane_size
            = static_cast<size_t>(IC) * static_cast<size_t>(OC) * element_size;
        ...
        for (dim_t kw = 0; kw < KW; ++kw) {
            ...
            std::memcpy(dst_bytes
                            + static_cast<size_t>(dst_offset)
                                    * element_size, // destination address
                    src_bytes
...
bool regular_swd_ok(const jit_deconvolution_fwd_t::pd_t &pd) {
    ...
    return (src_dt == f32 && wei_dt == f32 && dst_dt == f32)
            || (src_dt == bf16 && wei_dt == bf16 && one_of(dst_dt, bf16, f32))
            || (src_dt == f16 && wei_dt == f16 && one_of(dst_dt, f16, f32));
}

bool bias_ok(const jit_deconvolution_fwd_t::pd_t &pd) {
 ...
 * [OC, IC, KH, KW] dimensions while HWIO determines their physical order.
 *
 ...
 */
status_t jit_deconvolution_fwd_t::pd_t::init(const engine_t *const_engine) {

    auto *engine = const_cast<engine_t *>(
            const_engine); // pd requires const engine but verbose and iterator require non-const engine...
    ...
    VDISPATCH_DECONVOLUTION(regular_swd_ok(*this), VERBOSE_UNSUPPORTED_DT_CFG);
    ...
    VDISPATCH_DECONVOLUTION(attr()->has_default_values(
     ...
     */
    VDISPATCH_DECONVOLUTION(KSH() == 1 && KSW() == 1,
            VERBOSE_UNSUPPORTED_FEATURE, "non-unit stride");
    ...
    CHECK(attr_.set_default_formats(
            &dst_md_)); // set default formats for post-ops dst memory descriptors

    /*
     * Bias is optional. When present, check datatype and density.
     */
    VDISPATCH_DECONVOLUTION(bias_ok(*this), VERBOSE_UNSUPPORTED_DT_CFG);

    if (with_bias()) {
        VDISPATCH_DECONVOLUTION(impl::is_dense_format_kind({&bias_md_}),
                VERBOSE_UNSUPPORTED_SPARSE_CFG);
    }
    ...
    primitive_desc_iterator_t iterator(engine,
            reinterpret_cast<const op_desc_t *>(&conv_desc), attr(), nullptr);
    ...
    VDISPATCH_DECONVOLUTION(conv_pd_ != nullptr,
     ...
     * descriptor.
     */
    VDISPATCH_DECONVOLUTION(memory_desc_wrapper(src_md_)
                    == memory_desc_wrapper(*conv_pd_->src_md()),
            VERBOSE_UNSUPPORTED_TAG_S, "nested src");

    VDISPATCH_DECONVOLUTION(memory_desc_wrapper(weights_md_)
                    == memory_desc_wrapper(*conv_pd_->weights_md()),
            VERBOSE_UNSUPPORTED_TAG_S, "nested weights");

    if (with_bias()) {
        const memory_desc_t *nested_bias_md = conv_pd_->weights_md(1);

        VDISPATCH_DECONVOLUTION(nested_bias_md != nullptr
                        && memory_desc_wrapper(bias_md_)
                                == memory_desc_wrapper(*nested_bias_md),
                VERBOSE_UNSUPPORTED_TAG_S, "nested bias");
    }

    VDISPATCH_DECONVOLUTION(memory_desc_wrapper(dst_md_)
                    == memory_desc_wrapper(*conv_pd_->dst_md()),
            VERBOSE_UNSUPPORTED_TAG_S, "nested dst");
...
status_t jit_deconvolution_fwd_t::init(engine_t *engine) {
    if (!pd()->conv_pd_) return status::invalid_arguments;
 ...
 */
status_t jit_deconvolution_fwd_t::execute(const exec_ctx_t &ctx) const {

    if (!conv_p_) return status::runtime_error;

    const auto *deconv_pd = pd();

    const bool invert = needs_spatial_inversion(deconv_pd->weights_md());
     ...
     */
    if (!invert) return execute_nested(std::move(conv_args));
     ...
     */
    const void *original_weights = CTX_IN_MEM(const void *, DNNL_ARG_WEIGHTS);

    uint8_t *inverted_weights = scratchpad.get<uint8_t>(
            memory_tracking::names::key_conv_permuted_weights);

    if (inverted_weights == nullptr) return status::runtime_error;

    CHECK(invert_weights(
            deconv_pd->weights_md(), original_weights, inverted_weights));
    ...
    inverted_weights_mem.reset(
            new memory_t(engine, deconv_pd->conv_pd_->weights_md(),
                    use_runtime_ptr, inverted_weights));

    /*
     * Replace the original weights argument.
     */
    conv_args[DNNL_ARG_WEIGHTS] = {inverted_weights_mem.get(), true};
