...
Root Mean Square Normalization (RMSNorm)[[#1]][1] has emerged as a critical component in
modern large language models (LLMs) and transformer architectures. RMSNorm
provides a simpler and more efficient alternative to Layer Normalization by
normalizing inputs using only the root mean square statistic, eliminating the
need for mean subtraction and variance computation.
...
- **rrms** (optional): Reciprocal of RMS (`1/RMS(x)`). Can be cached for reuse
  in backward pass to avoid redundant computation. Shape is the same as src with
  normalized dimensions reduced to size 1.
...
3. The `rrms` output is optional and typically used when backward pass is needed.
   It stores the reciprocal of RMS for each sample, enabling efficient gradient
   computation without recomputing RMS.
...
1. Clear semantics with one-to-one mapping to framework operations.
2. Simpler parameter set than LayerNorm (no mean, no beta).
3. Distinguishes RMSNorm from LayerNorm at the API level, making fusion
   patterns explicit.
4. Easier for frameworks to integrate.

**Cons:**

1. Adds another normalization op to maintain.
...
Add a new attribute `rms_norm` (default=false) to LayerNorm. When `rms_norm=true`,
the operation performs RMS normalization (no mean subtraction, no variance computation).
The `beta` parameter is ignored and `mean`/`variance` outputs are not generated when
`rms_norm=true`.

**Pros:**

1. Reuses existing LayerNorm infrastructure.
2. Fewer operations to maintain.
3. Single operation covers both LayerNorm and RMSNorm use cases.

**Cons:**

1. Overloaded semantics - LayerNorm becomes ambiguous.
2. Framework integration is less intuitive (need to set flags).

## Decision

**Choose Option 1** - Add RMSNorm as a dedicated operation.

**Reasoning:**

1. **Clear Semantics**: RMSNorm has fundamentally different semantics from
   LayerNorm (no mean subtraction, no bias). A dedicated op makes this explicit.
...
5. **Growing Usage**: RMSNorm is becoming the standard for new LLMs (LLaMA
   family, Mistral, Gemma). This trend justifies a first-class operation.

6. **Simpler API**: Fewer attributes and inputs than trying to overload
   LayerNorm. No optional mean/variance outputs needed.

## Proposal for RMSNorm Backward

RMSNorm backward operation computes gradients with respect to input (src) and
...
| input     | input                    | dst (optional)                 | diff_dst                       |
| input     | weight                   | diff_dst                       | gamma                          |
| input     | rrms (cached 1/RMS)      | gamma                          | rrms (optional)                |
| attribute | normalized_shape         |                                | axis                           |
| attribute | eps                      | epsilon                        | epsilon                        |
| output    | grad_input               | diff_src                       | diff_src                       |
| output    | grad_weight              | diff_gamma                     | diff_gamma (optional)          |

### Option 1: Proposed RMSNorm Backward Operation (Recommended)
...
- **gamma** (required): Scale parameter from forward pass.

- **rrms** (optional): Cached reciprocal of RMS from forward pass
  (`1/RMS(x)`). If provided, avoids recomputation. If not provided, will be
  recomputed from src.
...
- **diff_gamma** (optional): Gradient with respect to scale parameter. Only computed
  if gamma was provided in the forward pass.

**Optimization Note:**

The forward pass can optionally output `rrms` (reciprocal RMS) for reuse in
backward pass, eliminating redundant computation. This is a common optimization
in training frameworks.

### Option 2: Extend LayerNormBackward with RMSNorm support
...
Add a new attribute `rms_norm` (default=false) to LayerNormBackward. When `rms_norm=true`,
the operation computes gradients for RMS normalization (no mean/variance inputs needed).
The `diff_beta` output is not generated when `rms_norm=true`.

**Pros:**

1. Reuses existing LayerNormBackward infrastructure.
2. Fewer backward operations to maintain.
3. Consistent with Option 2 in forward pass.

**Cons:**

1. More complex implementation with conditional logic.
2. Mean and variance inputs become conditionally required.
3. Different computation paths based on `rms_norm` flag add complexity.
4. Less clear semantics compared to dedicated operation.

## Decision for Backward

**Choose Option 1** - Add dedicated RMSNormBackward operation.

Same reasoning as forward pass: clear semantics, easier framework integration,
and alignment with major frameworks treating RMSNorm as a distinct operation.
