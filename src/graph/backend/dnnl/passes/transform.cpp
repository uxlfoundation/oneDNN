...
status_t fuse_sdpa_bwd(std::shared_ptr<subgraph_t> &sg) {
    ...
    for (auto &cur_op : sg->get_ops()) {
        ...
        for (auto &c : consumers_of(exp_op)) {
            if (c->get_kind() == op_kind::dnnl_permute)
                permute_p = c;
        ...
        if (!permute_p || permute_p->get_kind() != op_kind::dnnl_permute)
            continue;
        ...
        op_ptr reduce_dv = nullptr;
        {
            ...
            if (next && next->get_kind() == op_kind::dnnl_reduction)
                reduce_dv = next;
        }
        ...
        if (matmul_vt_do->get_input_value(1)->has_producer()) {
            permute_v = matmul_vt_do->get_input_value(1)
                                ->get_producer()
                                .shared_from_this();
            if (permute_v->get_kind() != op_kind::dnnl_permute) continue;
        }
        ...
        auto classify_sbwd_consumers = [&](const std::vector<op_ptr> &cs) {
            for (auto &c : cs) {
                if (is_binary(c, dnnl::algorithm::binary_mul)
                    ...
                    matmul_dq = c;
                else if (c->get_kind() == op_kind::dnnl_permute)
                    permute_ds = c;
            }
        };
        ...
        if (permute_ds && !matmul_dk) {
            ...
            if (next && next->get_kind() == op_kind::dnnl_matmul)
                matmul_dk = next;
        }
        ...
        op_ptr permute_k = nullptr;
        {
            ...
            if (dq_in1->has_producer()) {
                ...
                if (prod->get_kind() == op_kind::dnnl_permute) permute_k = prod;
            }
        }
        ...
        op_ptr transpose_dk = nullptr;
        {
            ...
            if (next && next->get_kind() == op_kind::dnnl_transpose)
                transpose_dk = next;
        }
