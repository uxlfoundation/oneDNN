        ...
        for (int ki = 0; ki < jcp.kw; ki++) {
            ...
            for (int ic = 0; ic
                ...
                for (dim_t oi = ow_start; oi < ow_end; oi++) {
                    ...
                    auto vmm_in = vmm_src(static_cast<int>(oi),
                            jcp.nb_oc_blocking);
                ...
                for (int kk = 0; kk < jcp.nb_oc_blocking; kk++) {
                    ...
                    for (dim_t oi = ow_start; oi < ow_end; oi++) {
                        ...
                        auto src = vmm_src(static_cast<int>(oi),
                                jcp.nb_oc_blocking);
                        if (isa_has_bf16(jcp.isa)) {
...
status_t jit_avx512_core_bf16_bwd_data_kernel_t::init_conf(jit_conv_conf_t &jcp,
    ...
    if (is_iw_threading_available(jcp)) {
        ...
        float block_overhead
                = nstl::max<dim_t>(0.0f, 1.0f - iw_block_cost / iw_block);
...
status_t jit_avx512_core_bf16_conv_bwd_weights_kernel_f32_t::init_conf(
    ...
    jcp.ic_tail = is_data_layout_nxc
            ? static_cast<int>(jcp.ic % jcp.ic_block)
            : 0;
    jcp.oc_tail = is_data_layout_nxc
            ? static_cast<int>(jcp.oc % jcp.oc_block)
            : 0;
...
void jit_avx512_core_bf16_conv_bwd_weights_kernel_f32_t::balance(
    ...
    auto calc_mem_cost = [&](int nthr_mb, int nthr_oc_b, int nthr_ic_b) {
        ...
        auto get_src_coef = [oi_channels_ratio, wei_compensation_scale]() {
            float src_coef = nstl::max<dim_t>(1.0f / oi_channels_ratio, 1.0f);
        ...
        auto get_dst_coef = [oi_channels_ratio]() {
            return nstl::max<dim_t>(oi_channels_ratio, 1.0f);
        };

        auto get_wei_coef = [wei_compensation_scale]() {
            return nstl::max<dim_t>(wei_compensation_scale, 1.0f);
        };
