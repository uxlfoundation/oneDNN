...
function formatDocBlock_font(block, context, token)
	...
	if not string.match(text, "[\n`*]") and not text == '' then -- single line, no inline markup
...
function formatDocBlock_simplesect(block, context)
	...
	if block.simpleSectionKind == "note" then
		s = "\n.. note:: \n"
		text = "\n" .. text
		text = text:gsub( "\n", "\n   ")
		s = s .. text .. "\n"
	elseif block.simpleSectionKind == "warning" then
		s = "\n.. warning:: \n"
		text = "\n" .. text
		text = text:gsub( "\n", "\n   ")
		s = s .. text .. "\n"
	elseif block.simpleSectionKind == "return" then
...
function formatDocBlock_htmlonly(block, context)
	return ''
end
...
{
	["htmlonly"]             = formatDocBlock_htmlonly,
