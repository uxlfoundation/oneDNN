...
void jit_prelu_reduction_kernel_t::generate(bool tail) {
    ...
    L(unroll_loop);
    {
        ...
        cmp(reg_reduction_blocks_, static_cast<uint32_t>(unrolling_factor));
        jl(unroll_loop_tail, T_NEAR);
        compute_dst(static_cast<int>(unrolling_factor), tail);
        sub(reg_reduction_blocks_, static_cast<uint32_t>(unrolling_factor));
    ...
    L(unroll_loop_tail);
    {
        ...
        add(reg_offset_, static_cast<uint32_t>(scratchpad_c_block_offset_));
...
void jit_uni_prelu_reduction_kernel_t<Vmm>::compute_dst(
    ...
    const int vmm_begin = static_cast<int>(number_reserved_vmms_);
