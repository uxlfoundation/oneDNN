...
struct ref_gated_mlp_t : public primitive_t {
    ...
    struct pd_t : public gated_mlp_pd_t {
        ...
        status_t init(impl::engine_t *engine) {
            //bool enable_ref = gpu_utils::dev_getenv("enable_ref_gmlp", false);
            ...
            auto gate_attr = move_attr(DNNL_ARG_WEIGHTS_GATE, DNNL_ARG_WEIGHTS);
            gate_attr.post_ops_.append_eltwise(1.f, activation(), 1.f, 0.f);
            gate_attr.post_ops_.append_binary(alg_kind::binary_mul, &inter_md);
            VDISPATCH_GATED_MLP_SC(
                    create_matmul(gemm_gate, engine, gate_attr,
                            arg_md(DNNL_ARG_SRC), arg_md(DNNL_ARG_WEIGHTS_GATE),
                            &inter_md),
                    "internal error in gemm_gate");

            VDISPATCH_GATED_MLP_SC(
                    create_matmul(gemm_up, engine,
                            move_attr(DNNL_ARG_WEIGHTS_UP, DNNL_ARG_WEIGHTS),
                            arg_md(DNNL_ARG_SRC), arg_md(DNNL_ARG_WEIGHTS_UP),
                            &inter_md),
                    "internal error in gemm_up");

            VDISPATCH_GATED_MLP_SC(
                    create_matmul(gemm_down, engine,
                            move_attr(DNNL_ARG_WEIGHTS_DOWN, DNNL_ARG_WEIGHTS),
                            &inter_md, arg_md(DNNL_ARG_WEIGHTS_DOWN),
                            arg_md(DNNL_ARG_DST)),
                    "internal error in gemm_down");

            auto scratchpad = scratchpad_registry().registrar();
            const memory_desc_wrapper inter_mdw(inter_md);
            scratchpad.book(memory_tracking::names::key_matmul_src_trans,
                    inter_mdw.size(), 1, OCL_BUFFER_ALIGNMENT);
        ...
        status_t get_inter_md(memory_desc_t &retn) const {
            ...
            CHECK(memory_desc_init_by_tag(retn, int(dims.size()), dims.data(),
                    dnnl_f32, format_tag::ab));
            return status::success;
        }

        std::shared_ptr<primitive_desc_t> gemm_gate, gemm_up, gemm_down;

    private:
        primitive_attr_t move_attr(int from, int to) {
            primitive_attr_t retn;
            retn.scales_.set(to, attr()->scales_.get(from));
            retn.zero_points_.set(to, attr()->zero_points_.get(from));
            return retn;
        }
    ...
    status_t init(impl::engine_t *engine) override {
        CHECK(create_nested_primitive(gemm_gate, pd()->gemm_gate, engine));
        CHECK(create_nested_primitive(gemm_up, pd()->gemm_up, engine));
        CHECK(create_nested_primitive(gemm_down, pd()->gemm_down, engine));
        return status::success;
    }
    ...
    status_t execute(const exec_ctx_t &ctx) const override {
        ...
        do {
            ...
            CHECK(prep_weights_and_run(args, DNNL_ARG_WEIGHTS_UP, gemm_up));
        } while (false);
        do {
            exec_args_t args;
            args[DNNL_ARG_SRC] = ctx.args().at(DNNL_ARG_SRC);
            ...
            CHECK(prep_weights_and_run(args, DNNL_ARG_WEIGHTS_GATE, gemm_gate));
        } while (false);
        do {
            exec_args_t args;
            args[DNNL_ARG_SRC] = memory_arg_t {inter_wei_mem.get(), true};
            args[DNNL_ARG_DST] = ctx.args().at(DNNL_ARG_DST);
            CHECK(prep_weights_and_run(args, DNNL_ARG_WEIGHTS_DOWN, gemm_down));
        } while (false);
...
private:
    ...
    std::shared_ptr<impl::primitive_t> gemm_gate, gemm_up, gemm_down;
};
