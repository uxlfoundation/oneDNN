        ...
        status_t init(impl::engine_t *engine) {
            ...
            VDISPATCH_INNER_PRODUCT_SC(
                    create_matmul_pd(matmul_pd_, engine, &a_md, &b_md,
                            &matmul_bias_md, &c_md, &matmul_attr, true),
...
struct matmul_bwd_weights_t : public primitive_t {
    ...
    struct pd_t : public bwd_weights_pd_t {
        ...
        status_t init(impl::engine_t *engine) {
            ...
            bool matmul_ok = status::success
                    == create_matmul_pd(matmul_pd_, engine, &a_md, &b_md,
                            nullptr, &c_md, attr(), true,
            ...
            if (with_bias() && !matmul_ok) {
                ...
                int grf_per_thread;
                auto status = matmul_pd_->query(query::preferred_gpu_grf_per_thread,
                        0, &grf_per_thread);
