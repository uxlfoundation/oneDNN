...
kernel_cache_t::~kernel_cache_t() {
    utils::lock_read_t lock_write(mutex_);
    auto it = kernels_.find(std::this_thread::get_id());
    if (it != kernels_.end() && it->second != main_kernel_) {
        func_zeKernelDestroy(it->second);
        kernels_.erase(it);
    }
}
...
status_t kernel_t::make(compute::kernel_t &compute_kernel,
        std::shared_ptr<module_wrapper_t> &module_ptr,
        ze_kernel_handle_t kernel, const compute::program_src_t &src) {
    compute_kernel = compute::kernel_t(
            std::make_shared<kernel_compat_t>(module_ptr, kernel, src));
...
status_t kernel_t::parallel_for(impl::stream_t &stream,
    ...
    auto l0_engine = utils::downcast<engine_t *>(stream.engine());
    auto l0_context = l0_engine->context();
            ...
            if (!mem_storage->is_null()) {
                auto memory_storage_ctx
                        = utils::downcast<engine_t *>(mem_storage->engine())
                                  ->context();
                if (l0_context != memory_storage_ctx) {
    ...
    event_wrapper_t event(l0_context);
    ...
    if (l0_stream->flags() & stream_flags::out_of_order) {
        ...
        auto events_size = l0_deps.size();
        auto event_data = events_size ? l0_deps.data() : nullptr;

        CHECK(func_zeCommandListAppendLaunchKernel(l0_stream->list(), *kernel,
                &group_count, event.get(), events_size, event_data));

        event_t::from(out_dep).events_ = {event};
    } else {
        bool save_event = save_events_ || stream.is_profiling_enabled();

        CHECK(func_zeCommandListAppendLaunchKernel(l0_stream->list(), *kernel,
                &group_count, save_event ? event.get() : nullptr, 0, nullptr));
    }
...
kernel_t::kernel_t(const std::shared_ptr<module_wrapper_t> module,
    ...
    cache_ = std::make_shared<kernel_cache_t>(*(module_.get()), kernel_);
}

kernel_t::~kernel_t() {
    if (kernel_) func_zeKernelDestroy(kernel_);
    module_.reset();
}
