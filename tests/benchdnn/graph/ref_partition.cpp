...
void ref_partition_t::exec_ops(res_t *res) {
    for (const auto &par_op_ref : partition_ops_ref_) {
        ...
        bool cond_accept_parent_output
        ...
        for (size_t i = 0; i < op.in_lts_.size(); i++) {
            ...
            int arg = get_prim_arg_name_from_graph_op_input_offset(
                    ref_prim->get_kind(), i, use_dst);
            if (cond_accept_parent_output && i == 0) {
                ...
                assert(dst_i.nelems() == src_i.nelems());

                const int64_t chunk_size = 64;
                const int64_t n_chunks = div_up(src_i.nelems(), chunk_size);
                benchdnn_parallel_nd(n_chunks, [&](int64_t idx_chunk) {
                    int64_t idx_start = idx_chunk * chunk_size;
                    int64_t idx_end
                            = MIN2(idx_start + chunk_size, src_i.nelems());
                    for (int64_t idx = idx_start; idx < idx_end; ++idx) {
                        float e = src_i.get_elem(idx);
                        dst_i.set_elem(idx, static_cast<int8_t>(e));
                    }
                });
            } else
