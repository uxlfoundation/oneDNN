...
#if HAS_TAIL
    // Element-wise writes with linear bounds check to prevent OOB.
    // The DST_D* decomposition preserves DST_OFF == mid_dim (block-aligned),
    // but the rounded mixed_dim dispatch can produce positions past the
    // end of the buffer. Guard each lane with a flat-index check.
    {
        unsigned dst_base = DST_OFF(
                dims0[0], dims0[1], dims0[2], dims0[3], dims0[4], dims0[5]);
        unroll_for(unsigned idx = 0; idx < NVECT; ++idx) {
            if (dst_base + idx * SUB_GROUP_SIZE + local_id < TOTAL_ELEMS) {
