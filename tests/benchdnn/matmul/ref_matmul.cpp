...
//   wei_ab(k, n) = wei_ab_base + k * N + n (for scales and zps)
//   wei_ba(k, n) = wei_ba_base + k * wei_k_stride + n * wei_n_stride
...
static void compute_ref_matmul_chunk(const chunk_params_t &p, int64_t M,
        ...
        int64_t wei_ab_base, int64_t wei_ba_base, int64_t wei_k_stride,
        int64_t wei_n_stride, int64_t dst_row_base, int64_t bia_base,
        int64_t bia_m_stride, int64_t bia_n_stride, const attr_t &attr,
        const args_t &args) {
            ...
            ++n)
    {
        ...
        for (int64_t gK = 0; gK < p.n_k_groups; gK++) {
            ...
            const auto wei_gK_off
                    = wei_ab_base + gK * p.smallest_k_group * N + n;
            ...
            for (int64_t k = 0; k < p.smallest_k_group; ++k) {
                ...
                const auto wei_off
                        = wei_ba_base + kk * wei_k_stride + n * wei_n_stride;
...
void compute_ref_grouped_matmul(const prb_t *prb, const args_t &args) {
    ...
    benchdnn_parallel_nd(group_count, [&](int64_t g) {
        ...
        const int64_t wei_ab_base = g * prb->k * prb->n;
        const int64_t wei_ba_base = g * prb->k * prb->n; // abc: k-major
        const int64_t wei_k_stride = prb->n;
        const int64_t wei_n_stride = 1;
        const int64_t dst_row_base = group_offsets[g];
        const int64_t bia_base = g * prb->n;
        const int64_t bia_m_stride = 0; // bias is per-group, not per-row
        const int64_t bia_n_stride = 1;
        ...
        for (int64_t mc = 0; mc < M_blocks; mc++)
            for (int64_t nc = 0; nc < N_blocks; nc++)
                compute_ref_matmul_chunk(params, M_g, prb->n, prb->k, mc, nc,
                        src_row_base, wei_ab_base, wei_ba_base, wei_k_stride,
                        wei_n_stride, dst_row_base, bia_base, bia_m_stride,
                        bia_n_stride, prb->attr, args);
    });
}
...
void compute_ref_matmul(const prb_t *prb, const args_t &args) {
    ...
    benchdnn_parallel_nd(
        ...
        const int64_t bia_base
                = dst_m.get_idx(dst_off_f(prb, mb, 0, 0), bias_broadcast_mask);
        const int64_t bia_m_stride
                = dst_m.get_idx(dst_off_f(prb, mb, 1, 0), bias_broadcast_mask)
                - bia_base;
        const int64_t bia_n_stride
                = dst_m.get_idx(dst_off_f(prb, mb, 0, 1), bias_broadcast_mask)
                - bia_base;

        // Precompute offsets for this chunk
        const int64_t src_row_base = src_mb * M;
        const int64_t wei_ab_base = wei_mb * K * N;
        const int64_t wei_ba_base = wei_mb * N * K;
        const int64_t wei_k_stride = 1;
        const int64_t wei_n_stride = K;
        const int64_t dst_row_base = mb * M;

        compute_ref_matmul_chunk(params, M, N, K, mc, nc, src_row_base,
                wei_ab_base, wei_ba_base, wei_k_stride, wei_n_stride,
