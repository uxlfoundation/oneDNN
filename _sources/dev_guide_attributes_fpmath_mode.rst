...
.. index:: pair: page; Floating-point Math Mode
...
.. note:: 
   ...
   The ``tf32`` mode is accelerated on Intel GPUs only. Intel CPUs do not implement ``tf32`` arithmetic; setting ``tf32`` on a CPU engine is accepted but has no effect, and computations are performed in ``f32``.
