...
private:
    ...
    void pre_process_k_tail_otf(brgemm_iteration_t &bi, int bdb, const Tmm &t1,
            reg64_t reg_base, size_t offset_src, size_t offset_dst,
            bool mem_advice_A);

    void maybe_tileloadd_nt(
            brgemm_iteration_t &bi, matrix_kind_t mk, int xdb, size_t offset);

    void maybe_otf_nt(brgemm_iteration_t &bi, int xdb);
...
size_t jit_brgemm_amx_uker_base_t::A_offset_wsp(
    ...
    // a layout [bs][k/rd_block][m][k = rd_block]
...
void jit_brgemm_amx_uker_base_t::maybe_otf_nt(brgemm_iteration_t &bi, int bdb) {
    ...
    if (bi.ldi->pos(0) == 0) {
        ...
        // OTF: load from A orig and store it aligned in the wsp buffer
        if (bi.rdi->is_tail(0)) {
            pre_process_k_tail_otf(bi, bdb, t1, reg_A, src_offset,
...
void jit_brgemm_amx_uker_base_t::pre_process_k_tail_otf(brgemm_iteration_t &bi,
        int bdb, const Tmm &t1, reg64_t reg_base, size_t offset_src,
...
void jit_brgemm_amx_uker_base_t::generate() {
    ...
    if (brg.is_input_convert() || brg.amx_wary_k_tail() || brg.otf) {
