...
void brgemm_1x1_convolution_fwd_t<isa>::execute_os_blocking(
        const exec_ctx_t &ctx, brgemm_batch_element_t *const brg_batch_global,
        const float *dst_scales, const float *oscales, int32_t src_zp_vals,
        int32_t *src_zp_comp, int32_t *dst_zp_vals, int32_t *s8s8_compensation,
        char *const c_buffer_global, char *inp_buffer_base,
        uint8_t *inp_buffer_mask_base, char *const wsp_tile_global) const {
    parallel(pd()->jcp_.nthr, [=, &ctx](const int ithr, const int nthr) {
        const auto &jcp = pd()->jcp_;
