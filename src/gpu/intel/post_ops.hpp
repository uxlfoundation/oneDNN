...
struct relative_md_t {
    ...
    relative_md_t(int mask, int ndims, data_type_t dt,
            const ndim_normalizer_t &ndim_normalizer)
        : dt(dt), broadcast_mask(~0), inner_dim(0) {
        for (int i = 0; i < ndims; i++) {
            auto rmd_idx = from_md_idx(i, ndims, ndim_normalizer);
            broadcast_mask &= ~(((~mask >> i) & 1) << rmd_idx.as_int());
        }
    }
    static status_t make(relative_md_t &rmd, const memory_desc_t &md,
        ...
        for (dim_t i = 0; i < blocking.inner_nblks; i++) {
            rmd.inner_layout.idxs[i] = from_md_idx(
                    blocking.inner_idxs[i], ndims, ndim_normalizer);
...
struct binary_t {
    ...
    static status_t make(binary_t &b, const post_ops_t::entry_t::prelu_t &op,
            ...
            const ndim_normalizer_t &ndim_normalizer) {
        if (s.src1_desc_layout.is_inlined()) {
            b.src1_desc = relative_md_t(
                    op.mask, dst_md.ndims(), data_type::f32, ndim_normalizer);
        } else {
