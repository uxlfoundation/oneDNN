...
private:
    ...
    void _visit(const ir::binary_op_t &obj) override {
        auto it = expr_locations.emplace(obj, expr_data_.size());
        if (!it.second) return;
...
dsl::expr_t calculate_buffer_offset(const gws_bin_mapping_t &gws_map,
    ...
    for (size_t gws_idx = 0; gws_idx < range_t::max_ndims; gws_idx++) {
        ...
        for (size_t i = 0; i < gws_blocks.size(); i++) {
            ...
            bool is_inner = false;
            for (size_t j = i + 1; j < gws_blocks.size(); j++) {
                if (gws_blocks[j].dim_idx == block.dim_idx) {
                    is_inner = true;
                    break;
                }
            }
            ...
            dsl::expr_t inner_stride = registry.add(int64_t(block.gws_stride),
                    "stride_dim" + dim_suffix, is_inner);
            dsl::expr_t size = registry.add(block.block, size_name, is_inner);
...
std::string dispatch_compile_params_t::str() const {
    ...
    for (size_t bit_idx = 0; bit_idx < 8 * sizeof(name_id_t); bit_idx++) {
        ...
        ss << to_string(name_id)
           << " - [ type: " << dnnl_dt2str(buffer_types[buf_idx]) << "offset : "
           << expr_encoder_t::decode(exprs, buffer_off_expr[buf_idx]) << "]";
        buf_idx++;
    }
    ss << "}";
