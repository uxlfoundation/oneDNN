...
#define BRGEMM_CONV_NDHWGC_ORDER(jcp) \
    n, (jcp).mb, odb, (jcp).nb_od, ohb, (jcp).nb_oh, owb, (jcp).nb_ow, g, \
            (jcp).ngroups, ocb, (jcp).nb_oc
#define BRGEMM_CONV_NGCDHW_ORDER(jcp) \
    n, (jcp).mb, g, (jcp).ngroups, ocb, (jcp).nb_oc, odb, (jcp).nb_od, ohb, \
            (jcp).nb_oh, owb, (jcp).nb_ow
#define BRGEMM_CONV_GCNDHW_ORDER(jcp) \
    g, (jcp).ngroups, ocb, (jcp).nb_oc, n, (jcp).mb, odb, (jcp).nb_od, ohb, \
            (jcp).nb_oh, owb, (jcp).nb_ow

#define BRGEMM_CONV_ITERATOR_INIT(jcp) \
    if ((jcp).loop_order == loop_ndhwgc) \
        nd_iterator_init(start, BRGEMM_CONV_NDHWGC_ORDER(jcp)); \
    else if ((jcp).loop_order == loop_ngcdhw) \
        nd_iterator_init(start, BRGEMM_CONV_NGCDHW_ORDER(jcp)); \
    else if ((jcp).loop_order == loop_gcndhw) \
        nd_iterator_init(start, BRGEMM_CONV_GCNDHW_ORDER(jcp)); \
    else \
        assert(!"Unknown loop order");

#define BRGEMM_CONV_ITERATOR_STEP(jcp) \
    if ((jcp).loop_order == loop_ndhwgc) \
        nd_iterator_step(BRGEMM_CONV_NDHWGC_ORDER(jcp)); \
    else if ((jcp).loop_order == loop_ngcdhw) \
        nd_iterator_step(BRGEMM_CONV_NGCDHW_ORDER(jcp)); \
    else if ((jcp).loop_order == loop_gcndhw) \
        nd_iterator_step(BRGEMM_CONV_GCNDHW_ORDER(jcp)); \
    else \
        assert(!"Unknown loop order");
