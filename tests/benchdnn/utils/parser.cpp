...
bool parse_grouped(std::vector<sparse_options_t> &sparse_options,
    ...
    static const std::string help
            = "DIM_IDX:NUM_GROUPS:size0,size1,...,sizeN[:MAX_VAR_DIM]\n   "
              ...
              "(comma-separated)\n"
              "    MAX_VAR_DIM is the optional upper bound on per-group size "
              "(default: 0 = unknown)\n"
    ...
    // Split remaining string at ':' to separate sizes from optional max_variable_dim
    std::string sizes_str = s.substr(start_pos);
    dnnl_dim_t max_variable_dim = 0;
    auto colon_pos = sizes_str.rfind(':');
    if (colon_pos != std::string::npos) {
        max_variable_dim
                = parser_utils::stoll_safe(sizes_str.substr(colon_pos + 1));
        sizes_str = sizes_str.substr(0, colon_pos);
    }
