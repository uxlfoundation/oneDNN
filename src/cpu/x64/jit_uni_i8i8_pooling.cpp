...
struct jit_uni_i8i8_pooling_fwd_ker_t : public jit_generator_t {
    ...
    // Narrows a register index to the int expected by XByak register
    // constructors, with a bounds check.
    static int xbyak_register_index(dim_t v) {
        assert(v >= 0 && v <= INT_MAX);
        return static_cast<int>(v);
    }

    // Narrows to the XByak 8-bit immediate boundary, with a bounds check.
    static uint8_t to_imm8(dim_t v) {
...
void jit_uni_i8i8_pooling_fwd_ker_t<avx2>::load_vreg_mask_q(dim_t ll) {
    ...
    vpblendd(vreg_mask_q, vreg_zeros, vreg_mask,
            to_imm8(0x3 << 2 * ll)); // 0x3 - mask for 2 x DWORD

    // Move mask from ll-th pos to 0-th pos
    if (ll > 0) vpermq(vreg_mask_q, vreg_mask_q, to_imm8(ll));
}
    ...
    if (jpp.src_dt == s32) {
        ...
        if (masked) {
            ...
            for (int i = 0; i < copy_range; i++)
                pinsrb(vr_src, ptr[aux_reg_src_w + offset + i], to_imm8(i));
...
void jit_uni_i8i8_pooling_fwd_ker_t<sse41>::store_dst_avg_op(
    ...
    if (jpp.src_dt == s32) {
        ...
        for (int i = 0; i < copy_range; i++)
            pextrb(ptr[reg_ptr_dst_i8 + offset + i], vr_dst, to_imm8(i));
    } else
        assert(!"unsupported src data type");
}
