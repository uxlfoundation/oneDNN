...
impl::data_type_t get_accum_datatype(brgemm_desc_t *brg) {
    ...
    assert(brg->is_int8 || brg->is_bf16 || brg->is_f32 || brg->is_f16
            || brg->is_fp8 || brg->is_bf16_fp8 || brg->is_f16_fp8);
...
status_t init_kernel_datatype(
    ...
    if (one_of(dt_b, data_type::f8_e5m2, data_type::f8_e4m3)) {
        brg->is_bf16_fp8 = dt_a == data_type::bf16;
        brg->is_f16_fp8 = dt_a == data_type::f16;
    }
    if (utils::everyone_is(false, brg->is_int8, brg->is_bf16, brg->is_f32,
                brg->is_f16, brg->is_fp8, brg->is_bf16_fp8, brg->is_f16_fp8)) {
        assert(!"Unsupported data type");
        return status::unimplemented;
    }
...
void set_isa_impl(brgemm_desc_t *brg) {
    ...
    if (brg->is_tf32) {
        ...
        brg->isa_impl = utils::map(true, isa_undef, is_isa_ok(avx10_2_amx_2),
                ...
                avx2);
    } else if (one_of(true, brg->is_fp8, brg->is_bf16_fp8, brg->is_f16_fp8)) {
        brg->isa_impl = utils::map(true, isa_undef, is_isa_ok(avx10_2_amx_2),
...
status_t init_brgemm_conf(brgemm_desc_t *brg, cpu_isa_t isa,
    ...
    brg->is_fp8_tmm
            = one_of(true, brg->is_fp8, brg->is_bf16_fp8, brg->is_f16_fp8)
