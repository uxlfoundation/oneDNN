...
struct transform_t {
    ...
    ir::v2::layout_t get_layout(const ir::tile_t &sizes, ir::type_t type,
...
struct kloop_iterator_t {
    ...
    virtual void inc_A_prefetch(int k_block) = 0;
    virtual void inc_A_load(int k_block) = 0;

    virtual void inc_B_prefetch(int k_block) = 0;
    virtual void inc_B_load(int k_block) = 0;
...
transform_t get_plan(const MatrixAddressingStrategy &matrix_strategy,
...
void apply_post_ops(const dnnl::impl::gpu::intel::gpu_post_ops_t &ops,
        const tensor_t &C, const std::vector<ir::expr_t> &idxs,
    ...
    for (size_t i = 0; i < ops.len(); i++) {
        if (ops[i].is_eltwise()) {
            ...
            auto src_g = [&]() -> global_tensor_t {
                ...
                pvar_map_t<expr_t> g_idxs;
                pvar_map_t<expr_t> g_strides;
                pvar_map_t<expr_t> g_sizes;
                for (int i = 0; i < ndims; i++) {
            ...
            v2::layout_t src_layout = {C.layout.desc(), src_g.type};
            ...
            std::cout << "src_g: " << src_g << "\n";
            std::cout << "src: " << src << "\n";
            load(src, src_g);

            switch (e.alg) {
                case dnnl::impl::alg_kind::binary_add:
                    binary(op_kind_t::_add, C, C, src);
                    break;
...
struct basic_iterator_t : kloop_iterator_t {
    basic_iterator_t(const global_tensor_t &A, int A_prefetch_k_blk,
        ...
        , k_idx_ {A.idxs[k_var]}
        , k_ {A.sizes[k_var]}
        , A_prefetch_ {A.buffer, A.type, A.base_offset, A.idxs, A.strides,
                  A.sizes, {{m_var, C.tile[m_var]}, {k_var, A_prefetch_k_blk}}}
        , A_load_ {A.buffer, A.type, A.base_offset, A.idxs, A.strides, A.sizes,
                  {{m_var, C.tile[m_var]}, {k_var, A_load_k_blk}}}
        , B_prefetch_ {B.buffer, B.type, B.base_offset, B.idxs, B.strides,
                  B.sizes, {{k_var, B_prefetch_k_blk}, {n_var, C.tile[n_var]}}}
        , B_load_ {B.buffer, B.type, B.base_offset, B.idxs, B.strides, B.sizes,
    ...
    void inc_kloop(int k_block) override {
        ...
        inc_A_prefetch(-k_block);
        inc_B_prefetch(-k_block);
        inc_A_load(-k_block);
        inc_B_load(-k_block);
...
struct gemm_ir {
    ...
    ir::stmt_t build(ir::kernel_iface_t iface, ir::ir_context_t &ctx) {
        ...
        auto A_prefetch_plan = get_plan(strategy.A_prefetch, A_vars, true);
        auto A_load_plan = get_plan(strategy.A, A_vars);

        auto B_prefetch_plan = get_plan(strategy.B_prefetch, B_vars, true);
        auto B_load_plan = get_plan(strategy.B, B_vars);

        ir::tile_t C_dims {{{m_var, m_blk}, {n_var, n_blk}}};
        auto C_store_plan = get_plan(strategy.C, C_vars);
        ...
        std::vector<ir::expr_t> C_idxs = {m_idx, n_idx};
        if (problem.batch == BatchMode::Strided) {
            struct info_t {
                info_t(ir::expr_t size, ir::expr_t idiv_magic)
                    : size(std::move(size))
                    , idiv_magic(std::move(idiv_magic)) {}
                ir::expr_t size;
                ir::expr_t idiv_magic;
            };
        ...
        global_tensor_t A_base {arg("A"), into_ir(problem.Ta_ext), offset_A,
                ...
                {{m_var, m}, {k_var, k}}, {}};
        global_tensor_t B_base {arg("B"), into_ir(problem.Ta_ext), offset_B,
        ...
        auto store_C = [&]() {
            ...
            store(kloop_it.C_store(), C, {}, {C_store_plan.cache_hint});
        };
    ...
    void build_k_loop(const k_loop_config_t &cfg) {
        ...
        auto prefetch_A = [&](int k_unroll_idx) {
            ...
            prefetch(kloop_it.A_prefetch(), {{k_var, 0}},
                    {cfg.A_prefetch_plan.cache_hint});
            kloop_it.inc_A_prefetch(prefetch_A_blk);
        };
        ...
        auto load_A = [&](int k_unroll_idx) {
            ...
            load(A.sub_tensor(cfg.A_load.layout, {{k_var, idx}}),
                    kloop_it.A_load(), {{k_var, 0}},
                    {cfg.A_load.transform.cache_hint});
            kloop_it.inc_A_load(load_A_blk);
        };
        ...
        auto prefetch_B = [&](int k_unroll_idx) {
            ...
            prefetch(kloop_it.B_prefetch(), {{k_var, 0}},
                    {cfg.B_prefetch_plan.cache_hint});
            kloop_it.inc_B_prefetch(prefetch_B_blk);
        };
        ...
        auto load_B = [&](int k_unroll_idx) {
            ...
            load(B.sub_tensor(cfg.B_load.layout, {{k_var, idx}}),
                    kloop_it.B_load(), {{k_var, 0}},
                    {cfg.B_load.transform.cache_hint});
            kloop_it.inc_B_load(load_B_blk);
        };
...
ir::stmt_t build_ir(const gemm_ir_desc_t &desc, ir::constraint_set_t cset) {
    ...
    stmt = ir::eliminate_common_subexprs(
            stmt, ctx, desc.strategy.GRFs * ctx.hw().grf_size());
    return stmt;
}
