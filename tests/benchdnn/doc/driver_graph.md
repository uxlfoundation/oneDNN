...
* [graph-knobs] can have the following attributes:
  ...
  - `--in-shapes=ID:SHAPE[*STRIDES][+ID:SHAPE*STRIDES+...]` -- Override a shape
    and strides of a graph input tesnor that includes `ID` in a graph with
    ...
    `STRIDES` can be specified in two different formats: 
      - Fully specified through a vector, of which the size should be the smae
      as that of `SHAPE`.
      ...
      indicates the first, second, third and fourth dimension respectively. The
      `Tag` is used as as a convenience shortcut to the alternative, full
    ...
    Some notes:
      ...
      shape modification.
      - The shape of internal tensors and graph output tensors are inferredby
