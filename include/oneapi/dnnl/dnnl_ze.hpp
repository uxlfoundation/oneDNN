...
inline memory make_memory(const memory::desc &memory_desc,
    ...
    error::wrap_c_api(
            dnnl_ze_interop_memory_create(&c_memory, memory_desc.get(),
...
inline void *get_mem_object(const memory &amemory) {
    void *mem_object;
    error::wrap_c_api(
            dnnl_ze_interop_memory_get_mem_object(amemory.get(), &mem_object),
            "could not get a memory object");
    return mem_object;
}
...
inline void set_mem_object(memory &amemory, void *mem_object) {
    error::wrap_c_api(
            dnnl_ze_interop_memory_set_mem_object(amemory.get(), mem_object),
            "could not set a memory object");
}
...
inline ze_event_handle_t execute(const dnnl::primitive &aprimitive,
    ...
    error::wrap_c_api(dnnl_ze_interop_primitive_execute(aprimitive.get(),
