...
#if ALG_MAX
    FLT_ACC_DATA_T dst_val = load(dst_val, src, SRC_OFF(mb, oc, 0, 0, 0));
#if IS_TRAINING
    off_t max_idx = 0;
#endif
#else
    FLT_ACC_DATA_T dst_val = 0;
#endif
    ...
    for (off_t id = 0; id < ID; id++) {
        for (off_t ih = 0; ih < IH; ih++) {
            for (off_t iw = 0; iw < IW; iw++) {
                ...
                FLT_ACC_DATA_T val = load(val, src, src_off);
#if ALG_MAX
                if (val > dst_val) {
                    dst_val = val;
#if IS_TRAINING
...
#endif
#else
    write(dst + dst_off, dst_val / (ID * IH * IW));
#endif
}
#endif // IS_FWD
...
__kernel void xe_global_pooling_bwd(__global DATA_T *diff_src, __global int *ws,
    ...
    const FLT_ACC_DATA_T dst_val = load((FLT_ACC_DATA_T)0, diff_dst, dst_off);
#endif // ALG_AVG
...
#else // ALG_MAX
            val_to_write = dst_val / SPATIAL_DIM;
#endif // ALG_MAX
        }
