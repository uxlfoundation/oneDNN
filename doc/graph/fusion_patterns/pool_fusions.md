Pool Fusions {#dev_guide_graph_pool_fusions}
===========================================================

## Overview

The Pool category includes operations such as: [AvgPool](@ref dev_guide_op_avgpool)
and [MaxPool](@ref dev_guide_op_maxpool).
...
fusion patterns for Pool.

## Pool patterns

oneDNN supports Pool and its optimization through Graph API [1] by
defining the graph, getting partition from the graph, and optimizing the kernels
underneath. In general, a Pool pattern is defined as a directional acyclic
graph (DAG) using oneDNN Graph API.

### Floating-point Pool patterns

oneDNN defines floating-point (f32, bf16, or f16) Pool patterns as follows.
The blue parts are required when defining a Pool pattern while the brown
parts are optional.

![Pool pattern](images/pool_pattern.png)

1. The Pool performs corresponding Pool operation for src tensor.
   See [AvgPool](@ref dev_guide_op_avgpool)
   and [MaxPool](@ref dev_guide_op_maxpool) in Graph API.
2. The post-op subgraph is optional and can be constructed with the following operations:
   ...
   1. 1 to 4 Pool/unary operations are supported.

## Data Types

oneDNN supports the floating-point Pool pattern with data types f32,
...
in a floating-point Pool pattern.
...
1. AvgPool
   - the combination of rounding_type equal to ceil and exclude_pad equal to false is
     not supported.
