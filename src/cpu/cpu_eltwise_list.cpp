...
const std::map<pk_impl_key_t, std::vector<impl_list_item_t>> &impl_list_map() {
    static const std::map<pk_impl_key_t, std::vector<impl_list_item_t>> the_map = REG_ELTWISE_P({
        {{forward}, {
            ...
            CPU_INSTANCE_AARCH64(jit_uni_eltwise_fwd_t<f32>)
            CPU_INSTANCE_AARCH64(jit_uni_eltwise_fwd_t<bf16>)
            CPU_INSTANCE_AARCH64(jit_uni_eltwise_fwd_t<f16>) 
        ...
        {{backward}, REG_BWD_PK({
            ...
            CPU_INSTANCE_AARCH64(jit_uni_eltwise_bwd_t<f32>)
            CPU_INSTANCE_AARCH64(jit_uni_eltwise_bwd_t<f32>)
            CPU_INSTANCE_AARCH64(jit_uni_eltwise_bwd_t<f32>)
