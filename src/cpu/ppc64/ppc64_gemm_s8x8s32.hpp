...
inline int pack_T16_8bit_V2(dim_t K_dim, dim_t M_dim, const int8_t *A,
    ...
    VecType vmask = reinterpret_cast<VecType>(vec_splats(uint8_t(0)));
    while (M_dim >= 16) {
    ...
    if (M_dim > 12 && M_dim < 16) {
        ...
        if (y >= 1 && y <= 3) {
            ...
            VecType va4 = reinterpret_cast<VecType>(vec_splats(uint8_t(0)));
...
inline int pack_T8_8bit_V2_signed(dim_t k, dim_t n, const b_type *b, dim_t ldb,
    ...
    for (i = 0; i < k8; i += 8) {
        for (j = 0; j < n8; j += 8) {
            ...
            *(vec_t *)&dest[16]
                    = reinterpret_cast<vec_t>(vec_add(D0, vmask)); // D1;
            *(vec_t *)&dest[32]
                    = reinterpret_cast<vec_t>(vec_add(D0, vmask)); //D2;
            *(vec_t *)&dest[48]
                    = reinterpret_cast<vec_t>(vec_add(D0, vmask)); //D3;
        }
    }
