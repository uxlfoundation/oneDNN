...
struct kernel_config_t {
    ...
    // C zp -> cOffset==Post (init_attrs); covers tensor and host-scalar c-zp.
    ...
    dim_t batch() const {
        dim_t b = 1;
        for (int i = 0; i < problem.batchDims; i++) b *= c_batch_sizes[i];
        return b;
    }
    // Swap-folded orientation defaults (N/T, exchanged under swap_ab).
    gemmstone::MatrixLayout ab_layout_N() const {
        return swap_ab ? gemmstone::MatrixLayout::T : gemmstone::MatrixLayout::N;
    }
    gemmstone::MatrixLayout ab_layout_T() const {
        return swap_ab ? gemmstone::MatrixLayout::N : gemmstone::MatrixLayout::T;
    }
