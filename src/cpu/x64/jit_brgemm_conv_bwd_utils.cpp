...
status_t pick_tags(jit_brgemm_conv_conf_t &jcp, memory_desc_t &diff_dst_md,
    ...
    if (jcp.wei_plain) {
        ...
        const bool prefer_vnni_format = jcp.isa == avx10_2
                && one_of(jcp.wei_dt, f8_e5m2, f8_e4m3);
        const bool req_emulation = jcp.isa == avx10_2;
        const auto vnnl_block_dt = get_mac_emu_data_type(
                jcp.wei_dt, jcp.isa, req_emulation);
        const auto vnni_block = data_type_vnni_granularity(vnnl_block_dt);
        if (jcp.ic_block == 64) {
            if (is_3d) {
                if (no_vnni_format)
