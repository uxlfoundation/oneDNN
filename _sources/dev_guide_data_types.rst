...
#. ``s4`` / ``u4`` data types are only supported as a storage data type for weights argument in case of weight-only quantization. For more details, refer to :ref:`Matmul Tutorial: weight-only quantization <doxid-matmul_with_weight_only_quantization_cpp>`.
